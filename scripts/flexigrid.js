...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 usepager: false, //
			 nowrap: true, //
			 halaman: 1, //current halaman
			 total: 1, //total pages
			 useRp: true, //use the results per halaman select box
			 rp: 15, // results per halaman
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				if (p.dataType=='xml')
					p.halaman = +$('rows halaman',data).text();
				else
					p.halaman = data.halaman;
			...
			$('.pcontrol input').val(p.halaman);
			$('.pcontrol span').html(p.pages);
			
			var r1 = (p.halaman-1) * p.rp + 1; 
			var r2 = r1 + p.rp - 1; 
			...
			populate: function () { //get latest data
				...
				if (p.halaman>p.pages) p.halaman = p.pages;
				var param = {halaman:p.newp, rp: p.rp, sortname: p.sortname, sortorder: p.sortorder, query: p.query, qtype: p.qtype};
			...
			changePage: function (ctype){ //change halaman
				...
				switch(ctype)
				{
					...
					case 'prev': if (p.halaman>1) p.newp = p.halaman - 1; break;
					case 'next': if (p.halaman<p.pages) p.newp = p.halaman + 1; break;
				...
				if (p.newp==p.halaman) return false;
