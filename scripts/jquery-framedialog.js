...
(function(jQuery) {
	...
	jQuery.FrameDialog = jQuery.FrameDialog || {};

	//array for return values, placeholder
	jQuery.FrameDialog._results = jQuery.FrameDialog._results || {};
	...
	buttons[Cancel] = function() {
				...
				jQuery.FrameDialog.clearResult(frame.attr("id"));
				frame.dialog("close");
			}

	//default options
	jQuery.FrameDialog._defaultOptions = {
		modal: true,
		closeOnEscape: false,
		position: 'center',
		//buttons: buttons,
	...
	var retry = false;
	jQuery.FrameDialog.create = function(options) {
		try	{
			//generate unique id
            ...
            var $loadingPane = jQuery('<div></div>').addClass(opts.loadingClass);            
			...
			var iframe = jQuery("<iframe frameborder='0' scrolling='auto' background='transparent' />")
				.attr("id", uid + "-VIEW")
				.attr("name", uid + "-VIEW")
				.attr("src", url)
				.css("margin", "0")
			...
			var ret = jQuery("<div />")
				...
				.bind("dialogbeforeclose", function(event, ui) {
					...
					if (event && event.originalTarget && event.originalTarget.nodeName && event.originalTarget.nodeName == "SPAN")
						jQuery.FrameDialog.clearResult(uid);
					...
					var result = jQuery.FrameDialog._results[uid] || null; //result or an explicit null
				...
				.bind('dialogclose', function(event, ui) {
					...
					window.setTimeout(
						function() {
							...
							try {
								delete jQuery.FrameDialog._results[uid];
			...
			if (err instanceof TypeError && !retry) {
				retry = true;
				return jQuery.FrameDialog.create(options); //retry once only
			}
	...
	jQuery.FrameDialog._getUid = function() {
	...
	jQuery.FrameDialog.current = function() {
		if (window.parent && window.parent.jQuery)
			return window.parent.jQuery("#" + jQuery.FrameDialog._getUid());
	...
	jQuery.FrameDialog.clearResult = function(uid) {
		if (uid) {
			//clear child's value
			try {
				delete jQuery.FrameDialog._results[uid];
			...
			var uid = jQuery.FrameDialog._getUid();
	...
	jQuery.FrameDialog.setResult = function(value, uid) {
		if (uid) {
			//set child value
	...
	jQuery.FrameDialog.closeDialog = function(uid) {
		if (uid) {
			//close child
	...
	jQuery.FrameDialog.cancelDialog = function(uid) {
		jQuery.FrameDialog.clearResult(uid);
		jQuery.FrameDialog.closeDialog(uid);
		return false;
	}


	//extension methods for shortcuts in dealing with a framedialog handle
	jQuery.fn.setResult = function(result) {
		return this.each(function() {
			jQuery.FrameDialog.setResult(result, jQuery(this).attr("id"));
		});
	}

	jQuery.fn.clearResult = function() {
		return this.each(function() {
			jQuery.FrameDialog.clearResult(jQuery(this).attr("id"));
		});
	}
	...
	jQuery.fn.cancelDialog = function() {
