...
(function(jQuery) {
	...
	$jx.FrameDialog = $jx.FrameDialog || {};

	//array for return values, placeholder
	$jx.FrameDialog._results = $jx.FrameDialog._results || {};
	...
	if (document.body !== null) {
		var winSize = { w:$jx(window).width(), h:$jx(window).height() }
	}
	
	buttons[OK] = function() {
				$jx(this).dialog("close");
			}
	buttons[Cancel] = function() {
				var frame = $jx(this);
				$jx.FrameDialog.clearResult(frame.attr("id"));
				frame.dialog("close");
			}

	//default options
	$jx.FrameDialog._defaultOptions = {
		modal: true,
		closeOnEscape: false,
		position: 'center',
		//buttons: buttons,
	...
	var retry = false;
	$jx.FrameDialog.create = function(options) {
		try	{
			//generate unique id
            ...
            var $loadingPane = $jx('<div></div>').addClass(opts.loadingClass);            
			...
			var iframe = $jx("<iframe frameborder='0' scrolling='auto' background='transparent' />")
				.attr("id", uid + "-VIEW")
				.attr("name", uid + "-VIEW")
				.attr("src", url)
				.css("margin", "0")
                ...
                .load(function(e){
                    $jx(this).css('visibility', 'visible');
                    $loadingPane.hide();
                })
				;

			var overlay = $jx("<div>&nbsp;</div>")
			...
			var ret = $jx("<div />")
				...
				.bind("dialogbeforeclose", function(event, ui) {
					...
					if (event && event.originalTarget && event.originalTarget.nodeName && event.originalTarget.nodeName == "SPAN")
						$jx.FrameDialog.clearResult(uid);
					...
					var result = $jx.FrameDialog._results[uid] || null; //result or an explicit null
				...
				.bind('dialogclose', function(event, ui) {
					...
					window.setTimeout(
						function() {
							...
							try {
								delete $jx.FrameDialog._results[uid];
			...
			if (err instanceof TypeError && !retry) {
				retry = true;
				return $jx.FrameDialog.create(options); //retry once only
			}
	...
	$jx.FrameDialog._getUid = function() {
	...
	$jx.FrameDialog.current = function() {
		if (window.parent && window.parent.jQuery)
			return window.parent.jQuery("#" + $jx.FrameDialog._getUid());
	...
	$jx.FrameDialog.clearResult = function(uid) {
		if (uid) {
			//clear child's value
			try {
				delete $jx.FrameDialog._results[uid];
			...
			var uid = $jx.FrameDialog._getUid();
	...
	$jx.FrameDialog.setResult = function(value, uid) {
		if (uid) {
			//set child value
	...
	$jx.FrameDialog.closeDialog = function(uid) {
		if (uid) {
			//close child
	...
	$jx.FrameDialog.cancelDialog = function(uid) {
		$jx.FrameDialog.clearResult(uid);
		$jx.FrameDialog.closeDialog(uid);
		return false;
	}


	//extension methods for shortcuts in dealing with a framedialog handle
	$jx.fn.setResult = function(result) {
		return this.each(function() {
			$jx.FrameDialog.setResult(result, $jx(this).attr("id"));
		});
	}

	$jx.fn.clearResult = function() {
		return this.each(function() {
			$jx.FrameDialog.clearResult($jx(this).attr("id"));
		});
	}
	
	$jx.fn.closeDialog = function() {
		return this.dialog('close');
	}

	$jx.fn.cancelDialog = function() {
		return this.clearResult().closeDialog();
	}
	
	//get the window context for the object/iframe in question
	$jx.fn.window = function() {
