<?php
class tarifLabRujukan extends SimakConf
{   
	/**
	...
	*/
	...
	protected function getDataRows($offset,$rows,$order,$sort,$pil,$cariNama,$cariID,$tipeCari,$cariKategori,$cariKel,$cariLab)
	{
		if($pil == "1")
		{
			$sql = "SELECT  b.id AS id,
							b.id_tdk_lab AS id_tdk_lab,
							a.nama AS nama,
							c.nama AS nm_lab_rujukan,
							b.tarif AS tarif,
							b.tarif1 AS tarif1,
							b.tarif2 AS tarif2,
							b.tarif3 AS tarif3,
							b.tarif4 AS tarif4,
							b.tarif5 AS tarif5
						FROM tbm_lab_rujukan_tarif b,
							 tbm_lab_tindakan a,
							 tbm_lab_rujukan c
						WHERE	 
							 b.id_tdk_lab=a.kode
							 AND b.id_lab_rujukan = c.id ";
			...
			if($cariLab <> '')			
				$sql .= "AND b.id_lab_rujukan = '$cariLab' ";	
			//$sql .= " GROUP BY a.kode";
    ...
    */
	...
	{				
		...
		{	
			...
			$cariByLab=$this->getViewState('cariByLab');		
			
			$this->UserGrid->VirtualItemCount=count($this->getDataRows($offset,$limit,$orderBy,'','2',$cariByNama,$cariByID,$elemenBy,$cariByKategori,$cariByKel,$cariByLab));
			// fetches all data account information 
			$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','2',$cariByNama,$cariByID,$elemenBy,$cariByKategori,$cariByKel,$cariByLab);
    ...
    public function deleteButtonClicked($sender,$param)
    {
        if ($this->User->IsAdmin)
		{
			...
			RadTdkRecord::finder()->deleteByPk($ID);	
	...
	public function editItem($sender,$param)
    {
		 ...
		 if ($this->User->IsAdmin)
		{
			...
			$this->UserGrid->VirtualItemCount = count($this->getDataRows($offset,$limit,$orderBy,'','2',$cariByNama,$cariByID,$elemenBy,$cariByKategori,$cariByKel,$cariByLab));
			// fetches all data account information 
			$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','1',$cariByNama,$cariByID,$elemenBy,$cariByKategori,$cariByKel,$cariByLab);
			// binds the data to interface components
			$this->UserGrid->dataBind();					
		}	
    }
    ...
    {       
		if ($this->User->IsAdmin)
		{
			...
			$tarifRcd = LabRujukanTarifRecord::finder()->findByPk($ID);
	...
	{		
		...
		$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','1',$this->cariNama->Text,$this->ID->Text,$this->Advance->Checked,$this->DDRadKateg->SelectedValue,$this->DDRadKel->SelectedValue,$this->DDlabRujuk->SelectedValue);
		$this->UserGrid->dataBind();	
	}
	...
	{		
		...
		//$this->UserGrid->DataSource=KabupatenRecord::finder()->findAll();
		$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','2',$this->cariNama->Text,$this->ID->Text,$this->Advance->Checked,$this->DDRadKateg->SelectedValue,$this->DDRadKel->SelectedValue,$this->DDlabRujuk->SelectedValue);
		$this->UserGrid->dataBind();
	}	
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Lab.tarifLabRujukanBaru'));		
	}
	...
	{		
		...
		if($this->DDlabRujuk->SelectedValue) {
			$this->setViewState('cariByLab', $this->DDlabRujuk->SelectedValue);
		}else{
			$this->clearViewState('cariByLab');	
		}
		
		$this->UserGrid->CurrentPageIndex = 0;
		
		$this->UserGrid->VirtualItemCount = count($this->getDataRows($offset,$limit,$orderBy,'','2',$this->cariNama->Text,$this->ID->Text,$this->Advance->Checked,$this->DDRadKateg->SelectedValue,$this->DDRadKel->SelectedValue,$this->DDlabRujuk->SelectedValue));
