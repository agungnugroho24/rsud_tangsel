...
class cetakLapTrans extends SimakConf
{
	...
	{	
		...
		if($jnsPasien == '0')
			{
				$tblTbtLab= 'tbt_lab_penjualan';
				$tblTbtRawat= 'tbt_rawat_jalan';
				
				 $sql = "SELECT 
					  $tblTbtLab.no_trans,
					  tbd_pasien.nama,
					  tbm_lab_tindakan.nama AS tindakan,
					  $tblTbtRawat.cm,
					  $tblTbtLab.harga,
					  tbm_kelompok.nama AS kelompok
					FROM
					  $tblTbtLab
					  INNER JOIN $tblTbtRawat ON ($tblTbtLab.cm = $tblTbtRawat.cm)
					  INNER JOIN tbd_user ON ($tblTbtLab.operator = tbd_user.nip)
					  INNER JOIN tbd_pasien ON ($tblTbtRawat.cm = tbd_pasien.cm)
					  INNER JOIN tbm_lab_tindakan ON ($tblTbtLab.id_tindakan = tbm_lab_tindakan.kode)
					  INNER JOIN tbm_poliklinik ON ($tblTbtLab.klinik = tbm_poliklinik.id)
					  INNER JOIN tbd_pegawai ON ($tblTbtLab.dokter = tbd_pegawai.id)
					  INNER JOIN tbm_kelompok ON (tbd_pasien.kelompok = tbm_kelompok.id) ";	
			}
			elseif($jnsPasien == '1')
			{
				$tblTbtLab = 'tbt_lab_penjualan_inap';
			...
			elseif($jnsPasien == '2')
			{
				$tblTbtLab = 'tbt_lab_penjualan_lain';
				
				 $sql = "SELECT 
					  $tblTbtLab.no_trans,
					  $tblTbtLab.nama AS nama,					  
					  tbm_lab_tindakan.nama AS tindakan,
					  $tblTbtLab.harga
					...
					FROM
					  $tblTbtLab					  
					  INNER JOIN tbd_user ON ($tblTbtLab.operator = tbd_user.nip)					  
					  LEFT JOIN tbm_lab_tindakan ON ($tblTbtLab.id_tindakan = tbm_lab_tindakan.kode)";
			}
           				  			
			if($jnsPasien == '2')
			{
				$sql .= "WHERE $tblTbtLab.no_trans <> '' ";	
			}
			else
			{
			...
			if($cariNama <> '')
				$sql .= "AND $tblTbtLab.operator = '$cariNama' ";		
						
			if($poli <> '')			
				$sql .= "AND tbm_poliklinik.id = '$poli' ";	
			
			if($dokter <> '')			
				$sql .= "AND $tblTbtLab.dokter = '$dokter' ";
			...
			if($tgl <> '')			
				$sql .= "AND $tblTbtLab.tgl = '$tgl' ";
			
			if($tglawal <> '' AND $tglakhir <> '')			
				$sql .= "AND $tblTbtLab.tgl BETWEEN '$tglawal' AND '$tglakhir' ";
			
			if($bln <> '' AND $thn <> '')			
				$sql .= "AND MONTH ($tblTbtLab.tgl)='$bln' AND YEAR($tblTbtLab.tgl)='$thn' ";	
			
			$sql .= " GROUP BY $tblTbtLab.id ";			
		...
		*/		
		...
		if($jnsPasien == '0')
		{
			$pdf->Cell(0,7,'LAPORAN TRANSAKSI LABORATORIUM - PASIEN RAWAT JALAN','0',0,'C');
		}
		elseif($jnsPasien == '1')
		{
			$pdf->Cell(0,7,'LAPORAN TRANSAKSI LABORATORIUM - PASIEN RAWAT INAP','0',0,'C');
		}
		elseif($jnsPasien == '2')
		{
			$pdf->Cell(0,7,'LAPORAN TRANSAKSI LABORATORIUM - PASIEN LUAR','0',0,'C');
		}
		...
		$pdf->Cell(450,8,'('.$this->User->IsUserName.')',0,0,'C','',$this->Service->constructUrl('Lab.LapTrans'));
