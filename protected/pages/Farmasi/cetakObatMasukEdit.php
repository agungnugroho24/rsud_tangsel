<?php
class cetakObatMasukEdit extends SimakConf
{
	...
	{	
		...
		$pdf->Cell(0,10,'           Telp. (021) 74718440','0',0,'C');
			...
			foreach($arrData as $row)
			{
				...
				if($row['disc'] > 0) //jika ada => discount harga ppn discount
				{
					$total = floor($row['jml_terima'] * $row['hrg'] * ($row['disc']/100));
					//$total = floor($row['jumlah'] * $row['hrg_ppn_disc']);
				
					$pdf->SetFont('Arial','',9);						
					//$pdf->Cell(15,5,$j.'.','1',0,'C');
					$pdf->Cell(30,5,$row['nofak'],'1',0,'C');
					$pdf->Cell(65,5,ObatRecord::finder()->findByPk($row['kode'])->nama,'1',0,'L');
					$pdf->Cell(20,5,$row['jml_terima'],'1',0,'C');
					$pdf->Cell(30,5,'Rp ' .number_format($row['hrg']*($row['disc']/100),2,',','.'),1,0,'R');
				...
				{
					$total = floor($row['jml_terima'] * $row['hrg']);
			...
			}elseif ($mat==2)
			{
				$pdf->Cell(110,8,'',0,0,'L');
				$pdf->Cell(35,5,'Materai ',0,0,'R');		
				$pdf->Cell(30,5,'Rp ' .number_format( '6000',2,',','.'),1,0,'R');
				$pdf->Ln(5);	
			}
			...
			//$pdf->Cell(60,8,'( '.$operator.' )',0,0,'C','',$this->Service->constructUrl('Farmasi.ObatMasuk1'));	
			$pdf->Cell(60,8,'( '.$operator.' )',0,0,'C','',$this->Service->constructUrl('Farmasi.ObatMasuk1',array('ode'=>'1','noPO'=>$noPO,'tgl'=>$tglFaktur,'noFak'=>$noFak,'mat'=>$cek,'ppn'=>$ppn,'pot'=>$pot,'tab'=>$nmTable)));
