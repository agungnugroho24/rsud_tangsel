<?php
class PbfObat extends SimakConf
{   
	/**
	...
	*/
	protected function getDataRows($offset,$rows,$order,$sort,$pil,$cariNama,$cariID,$tipeCari,$cariAlamat,$tipeCari2,$cariNpwp,$cariNpkp)
	{
		if($pil == "1")
		{
			$sql = "SELECT id, 
			...
			if($cariNpwp <> '')
				if($sqlAwal==$sql){
					$sql .= "WHERE npwp = '$cariNpwp' ";
				}
				else
				{
					$sql .= "AND npwp = '$cariNpwp' ";
				}
			
			if($cariNpkp <> '')
				if($sqlAwal==$sql){
					$sql .= "WHERE Npkp = '$cariNpkp' ";
				}
				else
				{
					$sql .= "AND Npkp = '$cariNpkp' ";
				}
		...
		$page=PbfObatRecord::finder()->findAllBySql($sql);
    ...
    */   
	...
	{				
		...
		{					
			...
			$cariByNpwp=$this->getViewState('cariByNpwp');
			$cariByNpkp=$this->getViewState('cariByNpkp');		
			
			$jmlData=count($this->getDataRows($offset,$limit,$orderBy,'','2',$cariByNama,$cariByID,$elemenBy,$cariByAlamat,$elemen2By,$cariByNpwp,$cariByNpkp));
			
			$this->jmlData->Text=$jmlData;
			$this->UserGrid->VirtualItemCount=$jmlData;
			
			// fetches all data account information 
			$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','2',$cariByNama,$cariByID,$elemenBy,$cariByAlamat,$elemen2By,$cariByNpwp,$cariByNpkp);
    ...
    public function deleteButtonClicked($sender,$param)
    {
        if ($this->User->IsAdmin)
		{
			...
			PbfObatRecord::finder()->deleteByPk($ID);	
	...
	{		
		...
		$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','1',$cariByNama,$cariByID,$elemenBy,$cariByAlamat,$elemen2By,$cariByNpwp,$cariByNpkp);
		$this->UserGrid->dataBind();		
	} 
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Farmasi.AdminFar.PbfObatBaru'));		
	}
	...
	{		
		...
		if($this->cariNpwp->Text){ 
			$this->setViewState('cariByNpwp', $this->cariNpwp->Text);	
		}else{
			$this->clearViewState('cariByNpwp');	
		}
		
		if($this->cariNpkp->Text){ 
			$this->setViewState('cariByNpkp', $this->cariNpkp->Text);	
		}else{
			$this->clearViewState('cariByNpkp');	
		}
		
		$jmlData=count($this->getDataRows($offset,$limit,$orderBy,'','1',$this->cariNama->Text,$this->ID->Text,$this->Advance->Checked,$this->cariAlamat->Text,$this->Advance2->Checked,$this->cariNpwp->Text,$this->cariNpkp->Text));
			
		$this->jmlData->Text=$jmlData;
		$this->UserGrid->VirtualItemCount=$jmlData;
		$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','1',$this->cariNama->Text,$this->ID->Text,$this->Advance->Checked,$this->cariAlamat->Text,$this->Advance2->Checked,$this->cariNpwp->Text,$this->cariNpkp->Text);	
		$this->UserGrid->dataBind();
	}
