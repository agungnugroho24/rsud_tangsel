<?php
class SumberObatEdit extends SimakConf
{
	...
	public function onPreLoad($param)
	{
		...
		{
			$SumberRecord=$this->SbrObtRecord;
			
			// Populates the input controls with the existing user data			
            $this->ID->Text=$SumberRecord->id;
			$this->nama->Text=$SumberRecord->nama;			
	...
	public function simpanClicked($sender,$param)	{			
		...
		if($this->IsValid)  // when all validations succeed
        {
           ...
           // $temp=$this->Request['ID'];
            ...
            $SumberRecord->id=ucwords($this->ID->Text);
		    $SumberRecord->nama=ucwords($this->nama->Text);
	...
	*/
	...
	protected function getSbrObtRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$SbrObtRecord=SumberObatRecord::finder()->findByPk($ID);		
		if(!($SbrObtRecord instanceof SumberObatRecord))
