<?php
class VisiteTarif extends SimakConf
{   	
	private $sortExp = "nm_kel_dokter";
    private $sortDir = "ASC";
    private $offset = 0;
    private $pageSize = 10;	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
						tbm_inap_visite.id,
						tbm_inap_visite.kel_dokter,
						tbm_inap_visite.kelas,
						(SELECT tbm_kamar_kelas.nama FROM tbm_kamar_kelas WHERE tbm_kamar_kelas.id = tbm_inap_visite.kelas ORDER BY tbm_kamar_kelas.id DESC LIMIT 0,1) AS nm_kelas,
						tarif,
						if(kel_dokter='0','Non Spesialis',if(kel_dokter='1','Spesialis','Sub Spesialis')) AS nm_kel_dokter
					FROM tbm_inap_visite WHERE id <> ''  ";
			...
			*/
			if($this->DDKelas->SelectedValue != '')
			{
				...
				$sql .= "AND tbm_inap_visite.kelas = '$DDKelas' ";
			} 
			
			if($this->DDKelompok->SelectedValue != '')
			{
				$DDKelompok = $this->DDKelompok->SelectedValue;
				$sql .= "AND tbm_inap_visite.kel_dokter = '$DDKelompok' ";
			} 
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $sql = "DELETE FROM tbm_inap_visite WHERE id = '$ID'";
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Tarif Visite Dokter telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.VisiteTarifBaru'));		
	}
