...
{   	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
					...
					FROM
					  tbm_emergency
					WHERE tbm_emergency.id <> ''  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_emergency.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_emergency.nama LIKE '$nama%' ";
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $sql = "DELETE FROM tbm_emergency WHERE id = '$ID'";
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Jasa Emergency <b>'.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.EmergencyBaru'));		
	}
