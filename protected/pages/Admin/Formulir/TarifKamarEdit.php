<?php
class TarifKamarEdit extends SimakConf
{
	...
	public function onPreLoad($param)
	{
		...
		{
			$dataRuangPasien=$this->KamarHargaRecord;		
			
			// Populates the input controls with the existing user data			
            $this->ID->Text=$dataRuangPasien->id_kmr;
			if($dataRuangPasien->id_opr)
				$this->DDtindakan->SelectedValue=$dataRuangPasien->id_opr;
	...
	public function simpanClicked($sender,$param)	{			
		...
		if($this->IsValid)  // when all validations succeed
        {
           ...
           // $temp=$this->Request['ID'];
            ...
            $dataRuangPasien->id_kmr=$this->ID->Text;
			$dataRuangPasien->id_opr=$this->DDtindakan->SelectedValue;
	...
	*/
	...
	protected function getKamarHargaRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$KamarHargaRecord=KamarHargaRecord::finder()->findByPk($ID);		
		if(!($KamarHargaRecord instanceof KamarHargaRecord))
			throw new THttpException(500,'id tidak benar.');
		return $KamarHargaRecord;
	}	
