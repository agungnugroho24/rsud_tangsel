<?php
class PddkPasienEdit extends SimakConf
{
	...
	public function onPreLoad($param)
	{
		...
		{
			$dataPendidikan=$this->PendidikanRecord;
			
			// Populates the input controls with the existing user data			
            $this->ID->Text=$dataPendidikan->id;
			$this->nama->Text=$dataPendidikan->nama;			
	...
	public function simpanClicked($sender,$param)	{			
		...
		if($this->IsValid)  // when all validations succeed
        {
           ...
           // $temp=$this->Request['ID'];
            ...
            $dataPendidikan->id=ucwords($this->ID->Text);
		    $dataPendidikan->nama=ucwords($this->nama->Text);
	...
	*/
	...
	protected function getPendidikanRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$PendidikanRecord=PendidikanRecord::finder()->findByPk($ID);		
		if(!($PendidikanRecord instanceof PendidikanRecord))
			throw new THttpException(500,'id tidak benar.');
		return $PendidikanRecord;
	}	
