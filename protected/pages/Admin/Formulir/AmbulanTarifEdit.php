<?php
class AmbulanTarifEdit extends SimakConf
{
	...
	public function onPreLoad($param)
	{
		...
		{
			$data = $this->TarifAmbulanRecord;
			
			// Populates the input controls with the existing user data	
			$this->nama->Text = $data->nama;	
			
			$this->tarifOperator->Text = floatval($data->harga);
	...
	{	
		if($this->IsValid)  // when all validations succeed
        {
			...
			{
				$sql = "SELECT nama FROM tbm_ambulan WHERE LOWER(nama) = '$nama' ";
				...
				if($arr)
				{
					...
					('','unmaskContent();
						jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_error">Tarif Ambulan dengan tujuan :<br/> <b>'.mysql_escape_string(ucwords($this->nama->Text)).'</b><br/> sudah ada dalam database !</p>\',timeout: 4000,dialog:{
							modal: true
						}});');	
	...
	protected function prosesSimpan()
	{
		...
		$data->harga = $tarifStandar;
		$data->save();	
		
		$this->getPage()->getClientScript()->registerEndScript
		('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Tarif Ambulan dengan tujuan  <b>'.ucwords($this->getViewState('namaAwal')).'</b> telah sukses diperbaharui<br/><br/></p>\',timeout: 600000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	*/
	...
	protected function getTarifAmbulanRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$TarifAmbulanRecord=TarifAmbulanRecord::finder()->findByPk($ID);		
		if(!($TarifAmbulanRecord instanceof TarifAmbulanRecord))
			throw new THttpException(500,'id tidak benar.');
		return $TarifAmbulanRecord;
	}	
