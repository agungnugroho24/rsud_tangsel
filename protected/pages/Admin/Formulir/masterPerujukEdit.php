<?php
class masterPerujukEdit extends SimakConf
{
	...
	public function onPreLoad($param)
	{
		...
		{
			$BPRecord=$this->BPRecord;
			
			// Populates the input controls with the existing user data			
            $this->alamat->Text=$BPRecord->alamat;
			$this->nama->Text=$BPRecord->nama;	
			$this->ID->Text=$BPRecord->id;			
	...
	public function simpanClicked($sender,$param)	{			
		...
		if($this->IsValid)  // when all validations succeed
        {
           ...
           // $temp=$this->Request['ID'];
			...
			$KelurahanRecord=$this->BPRecord;
			
            $KelurahanRecord->alamat=ucwords($this->alamat->Text);
	...
	*/
	...
	protected function getBPRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$BPRecord=BidanPerujukRecord::finder()->findByPk($ID);		
		if(!($BPRecord instanceof BidanPerujukRecord))
