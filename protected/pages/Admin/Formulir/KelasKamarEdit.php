<?php
class KelasKamarEdit extends SimakConf
{
	...
	public function onPreLoad($param)
	{
		...
		{
			$data = $this->KelasKamarRecord;
			
			// Populates the input controls with the existing user data	
			$this->nama->Text = $data->nama;
			$this->persentase->Text = $data->persentase;
	...
	{	
		if($this->IsValid)  // when all validations succeed
        {
			...
			{
				$sql = "SELECT nama FROM tbm_kamar_kelas WHERE LOWER(nama) = '$nama' ";
				...
				if($arr)
				{
					...
					('','unmaskContent();
						jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_error">Kelas dengan nama dengan nama :<br/> <b>'.ucwords($this->nama->Text).'</b><br/> sudah ada dalam database !</p>\',timeout: 4000,dialog:{
							modal: true
						}});');	
	...
	protected function prosesSimpan()
	{
		...
		$data->persentase = floatval($this->persentase->Text);
		$data->save();	
		
		$this->getPage()->getClientScript()->registerEndScript
		('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Kelas dengan nama <b>'.ucwords($this->getViewState('namaAwal')).'</b> telah sukses diperbaharui<br/><br/></p>\',timeout: 600000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	*/
	...
	protected function getKelasKamarRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$KelasKamarRecord=KelasKamarRecord::finder()->findByPk($ID);		
		if(!($KelasKamarRecord instanceof KelasKamarRecord))
			throw new THttpException(500,'id tidak benar.');
		return $KelasKamarRecord;
	}	
