...
{   	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
					  tbm_sewa_alat_penunjang.id,
					  tbm_sewa_alat_penunjang.nama,
					  tbm_sewa_alat_penunjang.tarif
					FROM
					  tbm_sewa_alat_penunjang
					WHERE tbm_sewa_alat_penunjang.id <> ''  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_sewa_alat_penunjang.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_sewa_alat_penunjang.nama LIKE '$nama%' ";
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $sql = "DELETE FROM tbm_sewa_alat_penunjang WHERE id = '$ID'";
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Jasa Sewa Alat Penunjang <b>'.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.SewaAlatPenunjangBaru'));		
	}
