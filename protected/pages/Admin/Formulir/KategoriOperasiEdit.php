<?php
class KategoriOperasiEdit extends SimakConf
{
	...
	public function onPreLoad($param)
	{
		...
		{
			$data = $this->OperasiKategoriRecord;
		...
		if($this->IsValid)  // when all validations succeed
        {
			...
			if($this->getViewState('namaAwal') != $nama)
			{
				...
				if($arr)
				{
					...
					('','unmaskContent();
						jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_error">Kategori Operasi dengan nama dengan nama :<br/> <b>'.ucwords($this->nama->Text).'</b><br/> sudah ada dalam database !</p>\',timeout: 4000,dialog:{
							modal: true
						}});');	
	...
	protected function prosesSimpan()
	{
		...
		$this->getPage()->getClientScript()->registerEndScript
		('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Kategori Operasi dengan nama <b>'.ucwords($this->getViewState('namaAwal')).'</b> telah sukses diperbaharui<br/><br/></p>\',timeout: 600000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	*/
	...
	protected function getOperasiKategoriRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$OperasiKategoriRecord=OperasiKategoriRecord::finder()->findByPk($ID);		
		if(!($OperasiKategoriRecord instanceof OperasiKategoriRecord))
			throw new THttpException(500,'id tidak benar.');
		return $OperasiKategoriRecord;
	}	
