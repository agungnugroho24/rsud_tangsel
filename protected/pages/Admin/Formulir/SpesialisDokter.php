<?php
class SpesialisDokter extends SimakConf
{   
	/**
	...
	*/
	...
	protected function getDataRows($offset,$rows,$order,$sort,$pil,$cariNama,$cariID,$tipeCari,$cariKlinik)
	{
		if($pil == "1")
		{
			$sql = "SELECT a.id,
			...
			if($cariKlinik <> '')			
				$sql .= "AND a.id_klinik = '$cariKlinik' ";
		...
		$page=SpesialisRecord::finder()->findAllBySql($sql);
    ...
    */
	...
	{				
		...
		{	
			...
			$this->UserGrid->VirtualItemCount=SpesialisRecord::finder()->count();
			// fetches all data account information 
			$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','2',$cariByNama,$cariByID,$elemenBy,$cariByKlinik);
    ...
    public function deleteButtonClicked($sender,$param)
    {
        if ($this->User->IsAdmin)
		{
			...
			SpesialisRecord::finder()->deleteByPk($ID);	
	...
	{		
		...
		$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','1',$cariByNama,$cariByID,$elemenBy,$cariByKlinik);
		$this->UserGrid->dataBind();		
	} 
	...
	{		
		...
		$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','1',$this->cariNama->Text,$this->ID->Text,$this->Advance->Checked,$this->DDKlinik->SelectedValue);
		$this->UserGrid->dataBind();	
	}
	...
	{		
		...
		//$this->UserGrid->DataSource=KabupatenRecord::finder()->findAll();
		$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','2',$this->cariNama->Text,$this->ID->Text,$this->Advance->Checked,$this->DDKlinik->SelectedValue);
		$this->UserGrid->dataBind();
	}	
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.SpesialisDokterBaru'));		
	}
