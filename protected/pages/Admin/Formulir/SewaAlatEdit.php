<?php
class SewaAlatEdit extends SimakConf
{
	...
	protected function prosesSimpan()
	{
		...
		$this->getPage()->getClientScript()->registerEndScript
		('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Jasa Sewa Alat  <b>'.ucwords($this->getViewState('namaAwal')).'</b> telah sukses diperbaharui<br/><br/></p>\',timeout: 600000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	*/
	...
	protected function getSewaAlatRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$SewaAlatRecord=SewaAlatRecord::finder()->findByPk($ID);		
		if(!($SewaAlatRecord instanceof SewaAlatRecord))
			throw new THttpException(500,'id tidak benar.');
		return $SewaAlatRecord;
	}	
