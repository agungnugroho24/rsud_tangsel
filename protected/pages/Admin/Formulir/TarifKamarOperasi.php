<?php
class TarifKamarOperasi extends SimakConf
{   	
	private $sortExp = "nm_operasi ASC, nm_kategori_operasi";
    private $sortDir = "ASC";
    private $offset = 0;
    private $pageSize = 10;	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
					  tbm_operasi_kamar_tarif.id,
					  tbm_operasi_kamar_tarif.id_kelas,
					  tbm_operasi_kamar_tarif.id_kamar_operasi,
					  tbm_operasi_kamar_nama.nama AS nm_operasi,
					  tbm_operasi_kamar_tarif.tarif AS tarif_operator,
					  tbm_operasi_kategori.id AS id_kategori_operasi,
					  tbm_operasi_kategori.nama AS nm_kategori_operasi
					FROM
					  tbm_operasi_kamar_tarif
					  INNER JOIN tbm_operasi_kamar_nama ON (tbm_operasi_kamar_tarif.id_kamar_operasi = tbm_operasi_kamar_nama.id)
					  LEFT JOIN tbm_operasi_kategori ON (tbm_operasi_kamar_tarif.id_kategori_operasi = tbm_operasi_kategori.id) 
					WHERE tbm_operasi_kamar_tarif.id_kamar_operasi <> ''  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_operasi_kamar_nama.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_operasi_kamar_nama.nama LIKE '$nama%' ";
			...
			if($this->DDKamar->SelectedValue != '')
			{
				...
				$sql .= "AND tbm_operasi_kamar_tarif.id_kamar_operasi = '$DDKamar' ";
			} 
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $idKamar = KamarOperasiTarifRecord::finder()->findByPk($ID)->id_kamar_operasi;
		  //$nama = KamarOperasiNamaRecord::finder()->findByPk($ID)->nama;
		  
		  $sql = "DELETE FROM tbm_operasi_kamar_tarif WHERE id_kamar_operasi='$idKamar'";
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Tarif Kamar Operasi <b>'.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.TarifKamarOperasiBaru'));		
	}
