<?php
class NamaKamarOperasi extends SimakConf
{   	
	private $sortExp = "nm_operasi ASC, nm_kelas";
    private $sortDir = "ASC";
    private $offset = 0;
    private $pageSize = 10;	
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
					...
					FROM
					  ...
					  INNER JOIN tbm_kamar_kelas ON (tbm_operasi_kamar_tarif.id_kelas = tbm_kamar_kelas.id) 
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $sql = "DELETE FROM tbm_operasi_kamar_nama WHERE id = '$ID'";
		  $this->queryAction($sql,'C');
		  
		  $sql = "DELETE FROM tbm_operasi_kamar_tarif WHERE id_kamar_operasi='$ID'";
		  $this->queryAction($sql,'C');
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Kamar Tindakan dan Tarif Kamar Operasi <b>'.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
