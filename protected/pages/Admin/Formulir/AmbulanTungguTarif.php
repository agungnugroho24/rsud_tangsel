<?php
class AmbulanTungguTarif extends SimakConf
{   
	private $sortExp = "id";
    private $sortDir = "ASC";
    private $offset = 0;
	...
	private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT * FROM tbm_ambulan_tunggu ";				
	...
	public function itemCreated($sender,$param)
    {
		...
		if($item->ItemType==='EditItem')
        {
		   ...
		   $item->tarif->TextBox->Columns=3;
		   $item->tarif2->TextBox->Columns=3;
        }       
	...
	public function saveItem($sender,$param)
    {
		...
		if(floatval($item->tarif->TextBox->Text))
		{
			$update = TarifAmbulanTungguRecord::finder()->findByPk($id);
			$update->tarif_jam_pertama = $item->tarif->TextBox->Text; 
			$update->tarif_jam_berikutnya = $item->tarif2->TextBox->Text; 
