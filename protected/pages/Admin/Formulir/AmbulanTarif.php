<?php
class AmbulanTarif extends SimakConf
{   	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
					  tbm_ambulan.id,
					  tbm_ambulan.nama,
					  tbm_ambulan.harga AS tarif
					FROM tbm_ambulan 
					WHERE tbm_ambulan.id <> ''  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_ambulan.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_ambulan.nama LIKE '$nama%' ";
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $sql = "DELETE FROM tbm_ambulan WHERE id = '$ID'";
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Tarif Ambulan <b>'.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.AmbulanTarifBaru'));		
	}
