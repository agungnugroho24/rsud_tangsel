<?php
class BhpInap extends SimakConf
{   	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
					  tbm_bhp_tindakan_inap.id,
					  tbm_bhp_tindakan_inap.nama,
					  tbm_bhp_tindakan_inap.tarif,
					  tbm_bhp_unit.nama AS nm_kateg
					FROM
					  tbm_bhp_tindakan_inap
					  INNER JOIN tbm_bhp_unit ON(tbm_bhp_unit.id = tbm_bhp_tindakan_inap.id_kateg)
					WHERE tbm_bhp_tindakan_inap.id <> ''  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_bhp_tindakan_inap.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_bhp_tindakan_inap.nama LIKE '$nama%' ";
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $sql = "DELETE FROM tbm_bhp_tindakan_inap WHERE id = '$ID'";
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Jasa Bhp Rawat Inap <b>'.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.BhpInapBaru'));		
	}
