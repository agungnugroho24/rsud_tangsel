<?php
class KategoriOperasi extends SimakConf
{   	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT * FROM tbm_operasi_kategori WHERE tbm_operasi_kategori.id <> ''  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_operasi_kategori.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_operasi_kategori.nama LIKE '$nama%' ";
			...
			{
				$ID = $this->ID->Text;
				$sql .= "AND tbm_operasi_kategori.id = '$ID' ";
			}
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $nama = KategoriOperasiRecord::finder()->findByPk($ID)->nama;
		  
		  $sql = "DELETE FROM tbm_operasi_kategori WHERE id = '$ID'";
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Kelas dengan nama <b>'.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.KategoriOperasiBaru'));		
	}
