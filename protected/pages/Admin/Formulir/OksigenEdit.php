<?php
class OksigenEdit extends SimakConf
{
	...
	public function onPreLoad($param)
	{
		...
		{
			$data = $this->InapTarifOksigenRecord;
	...
	{	
		if($this->IsValid)  // when all validations succeed
        {
			...
			{
				$sql = "SELECT nama FROM tbm_inap_tarif_oksigen WHERE LOWER(nama) = '$nama' ";
				...
				if($arr)
				{
					...
					('','unmaskContent();
						jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_error">Pemakaian Oksigen dengan nama :<br/> <b>'.ucwords($this->nama->Text).'</b><br/> sudah ada dalam database !</p>\',timeout: 4000,dialog:{
							modal: true
						}});');	
	...
	protected function prosesSimpan()
	{
		...
		$this->getPage()->getClientScript()->registerEndScript
		('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Pemakaian Oksigen  <b>'.ucwords($this->getViewState('namaAwal')).'</b> telah sukses diperbaharui<br/><br/></p>\',timeout: 600000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	*/
	...
	protected function getInapTarifOksigenRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$InapTarifOksigenRecord=InapTarifOksigenRecord::finder()->findByPk($ID);		
		if(!($InapTarifOksigenRecord instanceof InapTarifOksigenRecord))
			throw new THttpException(500,'id tidak benar.');
		return $InapTarifOksigenRecord;
	}	
