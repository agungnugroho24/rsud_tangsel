...
{   	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
					  ...
					  tbm_askep.nama,
						tbm_askep.kelas,
						(SELECT tbm_kamar_kelas.nama FROM tbm_kamar_kelas WHERE tbm_kamar_kelas.id = tbm_askep.kelas ORDER BY tbm_kamar_kelas.id DESC LIMIT 0,1) AS nm_kelas,
					  tbm_askep.tarif
					FROM
					  tbm_askep
					WHERE tbm_askep.id <> ''  ";
			...
			if($this->cariNama->Text != '')
			{
				...
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_askep.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_askep.nama LIKE '$nama%' ";
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $nama = AskepRecord::finder()->findByPk($ID)->nama;
		  
		  $sql = "DELETE FROM tbm_askep WHERE id = '$ID'";
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Jasa Asuhan Keperawatan <b>'.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.AskepBaru'));		
	}
