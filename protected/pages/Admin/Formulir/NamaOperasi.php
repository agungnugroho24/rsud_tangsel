<?php
class NamaOperasi extends SimakConf
{   	
	private $sortExp = "nm_operasi";
    private $sortDir = "ASC";
    private $offset = 0;
    private $pageSize = 10;	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
					  tbm_operasi_tarif.id_kelas,
					  tbm_operasi_tarif.id_operasi,
					  tbm_operasi_nama.nama AS nm_operasi,
					  tbm_operasi_tarif.js_dok_obgyn AS tarif_operator,
					  tbm_operasi_nama.index_operator,
					  tbm_operasi_nama.index_anastesi,
					  tbm_operasi_nama.index_ast_anastesi,
					  tbm_operasi_nama.index_ast_instrumen,
					  tbm_operasi_nama.index_paramedis,
					  tbm_operasi_nama.index_resusitasi,
					  tbm_operasi_nama.index_pengembang,
					  tbm_operasi_nama.index_penyulit
					FROM
					  tbm_operasi_tarif
					  INNER JOIN tbm_operasi_nama ON (tbm_operasi_tarif.id_operasi = tbm_operasi_nama.id)
					  INNER JOIN tbm_operasi_kategori ON (tbm_operasi_kategori.id = tbm_operasi_nama.id_kategori_operasi)
					WHERE tbm_operasi_tarif.id_operasi <> ''  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_operasi_nama.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_operasi_nama.nama LIKE '$nama%' ";
			...
			{
				$ID = $this->ID->Text;
				$sql .= "AND tbm_operasi_tarif.id_operasi = '$ID' ";
			}
			
			if($this->DDKelas->SelectedValue != '')
			{
				$DDKelas = $this->DDKelas->SelectedValue;
				$sql .= "AND tbm_operasi_tarif.id_kelas = '$DDKelas' ";
			} 
			
			if($this->DDKateg->SelectedValue != '')
			{
				$DDKateg = $this->DDKateg->SelectedValue;
				$sql .= "AND tbm_operasi_nama.id_kategori_operasi = '$DDKateg' ";
			} 
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $nama = OperasiNamaRecord::finder()->findByPk($ID)->nama;
		  
		  $sql = "DELETE FROM tbm_operasi_nama WHERE id = '$ID'";
		  $this->queryAction($sql,'C');
		  
		  $sql = "DELETE FROM tbm_operasi_tarif WHERE id_operasi='$ID'";
		  $this->queryAction($sql,'C');
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Tindakan dan Tarif Operasi <b>'.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.NamaOperasiBaru'));		
	}
