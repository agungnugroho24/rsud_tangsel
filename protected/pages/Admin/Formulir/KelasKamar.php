<?php
class KelasKamar extends SimakConf
{   	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT * FROM tbm_kamar_kelas WHERE tbm_kamar_kelas.id <> ''  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_kamar_kelas.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_kamar_kelas.nama LIKE '$nama%' ";
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $nama = KelasKamarRecord::finder()->findByPk($ID)->nama;
		  
		  $sql = "DELETE FROM tbm_kamar_kelas WHERE id = '$ID'";
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.KelasKamarBaru'));		
	}
