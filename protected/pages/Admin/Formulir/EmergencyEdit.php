<?php
class EmergencyEdit extends SimakConf
{
	...
	{	
		if($this->IsValid)  // when all validations succeed
        {
			...
			{
				$sql = "SELECT nama FROM tbm_emergency WHERE LOWER(nama) = '$nama' ";
				...
				if($arr)
				{
					...
					('','unmaskContent();
						jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_error">Jasa Emergency dengan nama :<br/> <b>'.ucwords($this->nama->Text).'</b><br/> sudah ada dalam database !</p>\',timeout: 4000,dialog:{
							modal: true
						}});');	
	...
	protected function prosesSimpan()
	{
		...
		$this->getPage()->getClientScript()->registerEndScript
		('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Jasa Emergency  <b>'.ucwords($this->getViewState('namaAwal')).'</b> telah sukses diperbaharui<br/><br/></p>\',timeout: 600000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	*/
	...
	protected function getEmergencyRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$EmergencyRecord=EmergencyRecord::finder()->findByPk($ID);		
		if(!($EmergencyRecord instanceof EmergencyRecord))
