<?php
class TindakanKhusus extends SimakConf
{   	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
					  tbm_tindakan_khusus.id,
					  tbm_tindakan_khusus.nama,
					  tbm_tindakan_khusus.tarif
					FROM
					  tbm_tindakan_khusus
					WHERE tbm_tindakan_khusus.id <> ''  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_tindakan_khusus.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_tindakan_khusus.nama LIKE '$nama%' ";
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $sql = "DELETE FROM tbm_tindakan_khusus WHERE id = '$ID'";
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Jasa Tindakan Khusus <b>'.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Admin.Formulir.TindakanKhususBaru'));		
	}
