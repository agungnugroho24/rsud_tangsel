...
class cetakStokFilm extends SimakConf
{
	...
	public function onInit($param)
	 {		
		parent::onInit($param);
		$tmpVar2=$this->authApp('6');
		if($tmpVar == "False" )//Bila tidak ada hak utk modul aplikasi pendaftaran
			$this->Response->redirect($this->Service->constructUrl('login'));
		
		 if (!isset($this->Session['cetakStokSql'])) 				
		{
			$this->Response->redirect($this->Service->constructUrl('Rad.MasterStokFilm'));
		}
	 }
	...
	{	
		...
		$pdf->Cell(0,5,'LAPORAN STOK RADIOLOGI','0',0,'C','',$this->Service->constructUrl('Rad.MasterStokFilm'));		
		...
		*/
		...
		$pdf->Cell(125,8,'',0,0,'L');
		$pdf->Cell(80,8,'('.$operator.')',0,0,'C','',$this->Service->constructUrl('Rad.MasterStokFilm'));		
		$pdf->Ln(5);
		$pdf->SetFont('Arial','',9);	
		$pdf->Cell(125,8,'',0,0,'L');
