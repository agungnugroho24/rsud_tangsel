...
{   
	...
	protected function getDataRows($offset,$rows,$order,$sort,$pil,$cariNama,$cariID,$tipeCari,$cariKategori,$cariKel,$cariRad)
	{
		if($pil == "1")
		{
			$sql = "SELECT  b.id AS id,
							b.id_tdk_rad AS id_tdk_rad,
							a.nama AS nama,
							c.nama AS nm_rad_rujukan,
							b.tarif AS tarif,
							b.tarif1 AS tarif1,
							b.tarif2 AS tarif2,
							b.tarif3 AS tarif3,
							b.tarif4 AS tarif4,
							b.tarif5 AS tarif5
						FROM tbm_rad_rujukan_tarif b,
							 tbm_rad_tindakan a,
							 tbm_rad_rujukan c
						WHERE	 
							 b.id_tdk_rad=a.kode
							 AND b.id_rad_rujukan = c.id ";
			...
			if($cariRad <> '')			
				$sql .= "AND b.id_rad_rujukan = '$cariRad' ";	
			//$sql .= " GROUP BY a.kode";
    ...
    */
	...
	{				
		...
		{	
			/*
			$sql = "SELECT * FROM tbm_rad_rujukan";
			$arr = $this->queryAction($sql,'S');
			
			foreach($arr as $row)
			{
				$id_rad_rujukan = $row['id'];
				
				
				$sql2 = "SELECT * FROM tbm_rad_tarif ";
				...
				foreach($arr2 as $row2)
				{
					...
					$sql3 = "INSERT INTO tbm_rad_rujukan_tarif (id_tdk_rad,id_rad_rujukan,tarif,tarif1,tarif2,tarif3,tarif4,tarif5) VALUE ('$id_tdk_rad','$id_rad_rujukan','$tarif','$tarif1','$tarif2','$tarif3','$tarif4','$tarif5')";
    ...
    {       
		if ($this->User->IsAdmin)
		{
			...
			$tarifRcd = RadRujukanTarifRecord::finder()->findByPk($ID);
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Rad.tarifRadRujukanBaru'));		
	}
