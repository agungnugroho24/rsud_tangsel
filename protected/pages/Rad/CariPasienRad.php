...
class CariPasienRad extends SimakConf
{   
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			if($jnsPasien == '0') //pasien rawat jalan
			{
				$sql = "SELECT 
						...
						WHERE 
							tbt_rad_penjualan.st_cetak_hasil = '0'";
			}
			elseif($jnsPasien == '1') //pasien rawat inap
			{
				$sql = "SELECT tbd_pasien.cm AS cm,
							...
							WHERE 
								...
								AND tbt_rad_penjualan_inap.st_cetak_hasil = '0'";
			}
			...
			if(trim($this->tglMsk->Text) != '')
			{
				...
				if($jnsPasien == '0') //pasien rawat jalan
				{
					$sql .= "AND tbt_rad_penjualan.tgl = '$tgl' ";
				}
				elseif($jnsPasien == '1') //pasien rawat inap
				{
					$sql .= "AND tbt_rad_penjualan_inap.tgl = '$tgl' ";
				}
				elseif($jnsPasien == '2') //pasien luar
				{
					$sql .= "AND tbt_rad_penjualan_lain.tgl = '$tgl' ";
				}
			}	
			...
			if($this->DDBulan->SelectedValue != '')
			{
				...
				if($jnsPasien == '0') //pasien rawat jalan
				{
					$sql .= "AND MONTH(tbt_rad_penjualan.tgl) = $bln";	
				}
				elseif($jnsPasien == '1') //pasien rawat inap
				{
					$sql .= "AND MONTH(tbt_rad_penjualan_inap.tgl) = $bln";	
				}
				elseif($jnsPasien == '1') //pasien rawat inap
				{
					$sql .= "AND MONTH(tbt_rad_penjualan_lain.tgl) = $bln";	
				}
			}
			
			if($jnsPasien == '0') //pasien rawat jalan
			{
				$sql .= " GROUP BY tbt_rad_penjualan.no_reg ";
			}
			elseif($jnsPasien == '1') //pasien rawat inap
			{
				$sql .= " GROUP BY tbt_rad_penjualan_inap.no_reg ";
			}
			elseif($jnsPasien == '1') //pasien rawat inap
			{
				$sql .= " GROUP BY tbt_rad_penjualan_lain.no_reg ";
			}
				...
				*/
				...
				$sql .= "FROM
						...
						WHERE
						  ...
						  AND tbt_rad_penjualan.st_cetak_hasil = '0'
	...
	public function pilihClicked($sender,$param)
	{
		...
		if($jnsPasien == '0') //pasien rawat jalan
		{
			$sql = "SELECT * FROM tbt_rad_penjualan WHERE no_reg = '$noReg' ";	
			$noTrans = RadJualRecord::finder()->findBySql($sql)->no_trans_rwtjln;	
			$cm = RadJualRecord::finder()->findBySql($sql)->cm;	
			//$cm = RwtjlnRecord::finder()->findByPk($noTrans)->cm;
		}
		elseif($jnsPasien == '1') //pasien rawat inap
		{
			$sql = "SELECT * FROM tbt_rad_penjualan_inap WHERE no_reg = '$noReg' ";	
			$noTrans = RadJualInapRecord::finder()->findBySql($sql)->no_trans_inap;	
			$cm = RadJualInapRecord::finder()->findBySql($sql)->cm;	
			//$cm = RwtInapRecord::finder()->findByPk($noTrans)->cm;
		}
		...
		if($notransUpdate != '' )
		{
			$this->msg->Text = '    
				<script type="text/javascript">
					window.opener.location="index.php?page=Rad.beritaRad&cm='.$cm.'&noTrans='.$noTrans.'&noReg='.$noReg.'&jnsPasien='.$jnsPasien.'"; 
					self.close();
				</script>';			
		}
