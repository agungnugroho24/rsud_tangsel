...
{   
	...
	{
		//$orderBy=$this->getViewState('orderBy');
		$cek=substr($this->noReg->Text,6,2);//Cek dari no registrasi 07=rawat jalan, 12	= rawat inap
		//if($cek=='07')
		if($cek=='01')
		{
			if(RadJualRecord::finder()->find('no_trans_rwtjln = ? AND flag = ?',$this->noReg->Text,'1'))
			{
				$this->CM->text=RadJualRecord::finder()->find('no_trans_rwtjln = ? AND flag = ?',$this->noReg->Text,'1')->cm;
				$this->show->Visible=true;
				$this->namaPasien->text=PasienRecord::finder()->findByPk($this->CM->Text)->nama;				
				$dok=RadJualRecord::finder()->find('no_trans_rwtjln = ?',$this->noReg->Text)->dokter;
		...
		elseif($cek=='02')
		{
			if(RadJualInapRecord::finder()->find('no_trans_inap = ?AND flag = ?',$this->noReg->Text,'1'))
			{
				$this->CM->text=RadJualInapRecord::finder()->find('no_trans_inap = ?',$this->noReg->Text)->cm;
				$this->show->Visible=true;
				$this->namaPasien->text=PasienRecord::finder()->findByPk($this->CM->Text)->nama;				
				$dok=RadJualInapRecord::finder()->find('no_trans_inap = ?',$this->noReg->Text)->dokter;
				$this->namaDokter->text=PegawaiRecord::finder()->findByPk($dok)->nama;
			}
		}else{
			if(RadJualLainRecord::finder()->find('no_trans_rwtjln_lain = ?AND flag = ?',$this->noReg->Text,'1'))
			{
				$this->CM->text='-';//RadJualInapRecord::finder()->find('no_trans = ?',$this->noReg->Text)->cm;
				$this->show->Visible=true;
				$this->namaPasien->text=RadJualLainRecord::finder()->find('no_trans_rwtjln_lain = ?',$this->noReg->Text)->nama;		
				//$dok=RadJualInapRecord::finder()->find('no_trans = ?',$this->noReg->Text)->dokter;
				$this->namaDokter->text='-';
				//$this->namaDokter->text=RadJualLainRecord::finder()->find('no_trans_rwtjln_lain = ?',$this->noReg->Text)->dokter;		
			...
			if (!$this->getViewState('nmTable'))
			{
				...
				if($cek=='01')
				{
					$sql="SELECT b.id,a.nama, b.catatan, b.film_size 
						FROM tbm_rad_tindakan a,tbt_rad_penjualan b 
				...
				{
					$sql="SELECT b.id,a.nama, b.catatan, b.film_size 
						FROM tbm_rad_tindakan a,tbt_rad_penjualan_inap b 
				...
				{
					$sql="SELECT b.id,a.nama, b.catatan, b.film_size 
						FROM tbm_rad_tindakan a,tbt_rad_penjualan_lain b 
				...
				foreach($arr as $row)
				{
					...
					$sql="INSERT INTO $nmTable (id_rad,nama,catatan,film_size) VALUES ('$id_rad','$nama','$catatan','$film')";
...
//		$this->UserGrid->DataSource=$this->getDataRows($offset,$limit,$orderBy,'','2',$this->noReg->Text);	
	...
	{		
		...
		$cek=substr($this->noReg->Text,6,2);//Cek dari no registrasi 07=rawat jalan, 11= rawat inap
		...
		foreach($arr as $row)
		{
			...
			if($cek=='01')
			{
				$sql="UPDATE tbt_rad_penjualan SET catatan='$cat' WHERE no_trans_rwtjln='$noTrans' AND id='$id_rad' ";			
				$this->queryAction($sql,'C');	
			}
			//elseif($cek=='12')
			elseif($cek=='02')
			{
				$sql="UPDATE tbt_rad_penjualan_inap SET catatan='$cat' WHERE no_trans_inap='$noTrans' AND id='$id_rad' ";			
				$this->queryAction($sql,'C');
			}else{
				$sql="UPDATE tbt_rad_penjualan_lain SET catatan='$cat' WHERE no_trans_rwtjln_lain='$noTrans' AND id='$id_rad' ";			
				$this->queryAction($sql,'C');
			}
		}
		
		//$this->Response->Reload();			
		$this->Response->redirect($this->Service->constructUrl('Rad.cetakBeritaRad',array('notrans'=>$noTrans,'cm'=>$cm,'nama'=>$nama,'dokter'=>$dokter,'table'=>$nmTable)));		
	}
