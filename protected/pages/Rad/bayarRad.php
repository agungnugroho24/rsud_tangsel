<?php
class bayarRad extends SimakConf 
{   
	...
	public function ambilDataTdkRad()
	{
		...
		{
			$sql .= " WHERE tbm_rad_tarif.tarif <> '0' ";
			...
			{
				$id_tdk = $row['id_tdk'];
				$sql .= " AND tbm_rad_tindakan.kode <> '$id_tdk' ";
    ...
    {	
		...
		elseif($jnsPasien=='1')
		{			
			$nmTable = 'tbt_rad_penjualan_inap';
		...
		{			
			$nmTable = 'tbt_rad_penjualan_lain';
			$this->konfNoCm->Text = '-';
		...
		{	
			$sql="SELECT 
				   ...
				   tbm_rad_kelompok.nama AS kelompok,
				   tbm_rad_kategori.jenis AS kategori  
				FROM
				  tbm_rad_tindakan
				  INNER JOIN tbm_rad_kelompok ON (tbm_rad_tindakan.kelompok = tbm_rad_kelompok.kode)
				  INNER JOIN $nmTable ON (tbm_rad_tindakan.kode = $nmTable.id_tindakan)
    ...
    {	
		...
		else //---------------- JIKA DIRUJUK KE LAB LUAR -----------------------
		{
			...
			{	
				...
				$sql .= " FROM tbm_rad_rujukan_tarif a, 
							  tbm_rad_tindakan b, tbm_rad_rujukan c
						 WHERE a.id_tdk_rad='$item' AND a.id_tdk_rad=b.kode AND a.id_rad_rujukan=c.id AND a.id_rad_rujukan='$id_rad_rujukan' ";	
			...
			{			
				...
				if($kelas == '1')//kelas VIP
				{
					...
					$sql .= " FROM tbm_rad_rujukan_tarif a, 
						 ...
						 WHERE 
						 	a.id_tdk_rad='$item' 
							AND a.id_tdk_rad=b.kode 
							AND a.id_rad_rujukan=c.id
							AND a.id_rad_rujukan='$id_rad_rujukan' ";
				}
				elseif($kelas == '2' || $kelas == '3' || $kelas == '4' || $kelas == '5')//kelas I,II,IIIA,IIIB
				{
					$sql="SELECT b.nama AS id, ";
    ...
    {		
		...
		$id_rad_rujukan=$this->DDradRujuk->SelectedValue;
		
		if($this->getViewState('stAsuransi') == '0')//pasien umum
		{
			$tmpTarif = $this->ambilTarifRs($jnsPasien,$item,$st_rujukan,$id_rad_rujukan);
		}
		elseif($this->getViewState('stAsuransi') == '1')//pasien penjamin
		{
			...
			$sql = "SELECT id,id_paket,tarif FROM tbm_rad_tindakan_paket_asuransi_detail WHERE id_paket > '0' AND id_asuransi = '$idPerus' AND id_tindakan = '$item' AND st_rawat = '$jnsPasien' ";
			if($this->queryAction($sql,'S'))
			{
				...
				$sql = "SELECT id,tarif FROM tbm_rad_tindakan_paket_asuransi WHERE id='$idPaket' AND id_asuransi = '$idPerus' AND st_rawat = '$jnsPasien' ";
				$tmpTarif = RadTdkPaketAsuransiRecord::finder()->findBySql($sql);
			}
			else
			{
				//cek tarif non paket di tbm_rad_tindakan_paket_asuransi_detail
				$sql = "SELECT tarif FROM tbm_rad_tindakan_paket_asuransi_detail WHERE id_paket = '0' AND id_asuransi = '$idPerus' AND id_tindakan = '$item' AND st_rawat = '$jnsPasien' ";
				if($this->queryAction($sql,'S'))
				{
    ...
    {		
		...
		if (!$this->getViewState('nmTable'))
		{
			...
			$sql="CREATE TABLE $nmTable (id INT (2) auto_increment,
					 ...
					 pengali TINYINT(4) DEFAULT '1',									 
					 total INT(11) NOT NULL,	
					 st_rujukan char(1) DEFAULT '0',
					 id_rad_rujukan char(2) DEFAULT NULL,	
			...
			{			
				$jml2=5000;
				$sql="INSERT INTO $nmTable (nama,total,id_tdk,st_rujukan,id_rad_rujukan) VALUES ('PENDAFTARAN',$jml2,'PDT','0','')";
		...
		$sql="INSERT INTO $nmTable (id_film,nm_film,nama,total,id_tdk,pengali,st_rujukan,id_rad_rujukan,id_paket_tdk_asuransi) VALUES ('$film','$nmFilm','$nama','$total','$item','$pengali','$st_rujukan','$id_rad_rujukan','$idPaketTdkAsuransi')";
		$this->queryAction($sql,'C');//Insert new row in tabel bro...
		...
		//---------- Tabel Temp u/ tbt_rad_jual_sisa --------
	...
	public function prosesCetak()
	{
		...
		if($jnsPasien == '0')
		{
			...
			$noReg = $this->numRegister('tbt_rad_penjualan',RadJualRecord::finder(),'07');
			...
			foreach($arrData as $row)
			{
				...
				$transRwtJln->id_rad_rujukan=$row['id_rad_rujukan'];
				$transRwtJln->Save();
		...
		elseif($jnsPasien == '1')
		{
			$noReg = $this->numRegister('tbt_rad_penjualan_inap',RadJualInapRecord::finder(),'12');
		...
		else
		{
			...
			$noReg = $this->numRegister('tbt_rad_penjualan_lain',RadJualLainRecord::finder(),'15');
			
			//INSERT tbt_rad_penjualan_lain
			foreach($arrData as $row)
			{
				$notransTmp = $this->numCounter('tbt_rad_penjualan_lain',RadJualLainRecord::finder(),'15');
		...
		if($this->getViewState('sisaTmpTable'))
		{
			...
			if($jmlSisa > 0) //jika ada sisa pembulatan
			{
				...
				foreach($arrData as $row)
				{
					$ObatJualSisa= new RadJualSisaRecord();
		...
		$this->errMsg->Text = '    
		<script type="text/javascript">
			alert("Data Rekam Billing Radiologi Telah Masuk Dalam Database.");
			window.location="index.php?page=Rad.bayarRad"; 
		</script>';
		
		//$this->Response->redirect($this->Service->constructUrl('Rad.bayarRadSukses'));
