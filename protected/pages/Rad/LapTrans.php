...
{   
    ...
    public function onLoad($param)
	{
		...
		{	
			...
			$sql = "SELECT 
				...
				FROM
				  tbm_rad_tindakan ORDER BY nama  ";
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT * FROM view_lap_rad WHERE flag='1' ";
	...
	public function prosesAmbil($noReg)
	{
		...
		$sql = "SELECT id FROM tbt_rad_hasil WHERE no_trans = '$noReg' ";
		$arr = $this->queryAction($sql,'S');
		foreach($arr as $row)
		{
			$id = $row['id'];
		...
		if ($jnsPasien=='0')
		{
			$sql="UPDATE tbt_rad_penjualan SET st_ambil = '1' WHERE no_reg = '$noReg' ";
		}
		elseif ($jnsPasien=='1')
		{
			$sql="UPDATE tbt_rad_penjualan_inap SET st_ambil = '1' WHERE no_reg = '$noReg' ";
		}
		elseif ($jnsPasien=='2')
		{
			$cm = '';
			$sql="UPDATE tbt_rad_penjualan_lain SET st_ambil = '1' WHERE no_reg = '$noReg' ";
		}
