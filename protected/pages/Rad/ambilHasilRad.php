<?php
class ambilHasilRad extends SimakConf
{   
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT * FROM view_ambil_hasil_rad WHERE no_reg <> '' ";
	...
	public function ambilClicked($sender,$param)
	{
		...
		{	
			...
			if($jmlData > 0) //jika ada hasil rad yg diambil
			{
				...
				jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Pengambilan Hasil Radiologi Sukses <br/><br/></p>\',timeout: 6000,dialog:{
					modal: true,
					buttons: {
						"OK": function() {
							jQuery( this ).dialog( "close" );
	...
	{	
		...
		if(substr($noReg,6,2) == '07')
		{
			//$sql="UPDATE tbt_rad_penjualan SET film_size = '$idFilm',jml_film = '$jmlFilm',st_ambil = '1',operator_ambil = '$operator',tgl_ambil = '$tgl',wkt_ambil = '$wkt' WHERE no_reg = '$noReg' AND id_tindakan='$idTdk'";
			$sql="UPDATE tbt_rad_penjualan SET jml_film = '1',st_ambil = '1',operator_ambil = '$operator',tgl_ambil = '$tgl',wkt_ambil = '$wkt' WHERE no_reg = '$noReg' AND id_tindakan='$idTdk'";
		}
		elseif(substr($noReg,6,2) == '12')
		{
			//$sql="UPDATE tbt_rad_penjualan_inap SET film_size = '$idFilm',jml_film = '$jmlFilm',st_ambil = '1',operator_ambil = '$operator',tgl_ambil = '$tgl',wkt_ambil = '$wkt' WHERE no_reg = '$noReg' AND id_tindakan='$idTdk' ";
			$sql="UPDATE tbt_rad_penjualan_inap SET jml_film = '1',st_ambil = '1',operator_ambil = '$operator',tgl_ambil = '$tgl',wkt_ambil = '$wkt' WHERE no_reg = '$noReg' AND id_tindakan='$idTdk' ";
		}
		elseif(substr($noReg,6,2) == '15')
		{
			$cm = '';
			//$sql="UPDATE tbt_rad_penjualan_lain SET film_size = '$idFilm',jml_film = '$jmlFilm',st_ambil = '1',operator_ambil = '$operator',tgl_ambil = '$tgl',wkt_ambil = '$wkt' WHERE no_reg = '$noReg' AND id_tindakan='$idTdk' ";
			$sql="UPDATE tbt_rad_penjualan_lain SET jml_film = '1',st_ambil = '1',operator_ambil = '$operator',tgl_ambil = '$tgl',wkt_ambil = '$wkt' WHERE no_reg = '$noReg' AND id_tindakan='$idTdk' ";
		}
