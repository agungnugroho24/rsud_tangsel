<?php
class cetakFilmBeli extends SimakConf
{
	...
	{	
		...
		if($this->Request['stRkbu'] == '0')
			$url = $this->Service->constructUrl('Rad.FilmBeli');
		...
		*/
		...
		if($stRkbu == '0')
			$pdf->Cell(0,5,'Permintaan Pengadaan Barang Radiologi ','0',0,'C','',$url);
		elseif($stRkbu == '1')	
			$pdf->Cell(0,5,'Permintaan Pengadaan Barang Radiologi  ','0',0,'C','',$url);
		...
		*/
		...
		$sql="SELECT * FROM tbt_film_beli WHERE no_po='$noPO' ";
		...
		foreach($arrData as $row)
		{
			...
			$pdf->Cell(95,5,FilmRecord::finder()->findByPk($row['kode'])->nama,'1',0,'L');
