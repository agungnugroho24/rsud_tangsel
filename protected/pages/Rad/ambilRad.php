...
{   
	...
	public function checkRegister($sender,$param)
    {
		...
		$cek=substr($this->notrans->Text,6,2);//Cek dari no registrasi 07=rawat jalan, 12	= rawat inap
		//if ($cek=='07')
		if ($cek=='01')
		{
			if(RadJualRecord::finder()->find('no_trans_rwtjln = ? AND st_ambil=?', $this->notrans->Text,'0'))
			{				
				$sql ="SELECT 
						...
						WHERE tbt_rad_penjualan.no_trans_rwtjln='$tmp' AND tbt_rad_penjualan.st_ambil='0' ";
		...
		elseif($cek=='02')
		{
			if(RadJualInapRecord::finder()->find('no_trans_inap = ? AND st_ambil=?', $this->notrans->Text,'0'))
			{				
				$sql ="SELECT 
						  tbt_rad_penjualan_inap.cm AS cm,
						  tbd_pasien.nama as nama,
						  tbd_pegawai.nama as dokter
						FROM
						  tbt_rad_penjualan_inap
						  INNER JOIN tbd_pasien ON (tbt_rad_penjualan_inap.cm = tbd_pasien.cm)
						  INNER JOIN tbd_pegawai ON (tbt_rad_penjualan_inap.dokter = tbd_pegawai.id)
						WHERE tbt_rad_penjualan_inap.no_trans_inap='$tmp' AND tbt_rad_penjualan_inap.st_ambil='0' ";
				$arr = $this->queryAction($sql,'R');
			}	
		}
		else
		{
			if(RadJualLainRecord::finder()->find('no_trans_rwtjln_lain = ? AND st_ambil=?', $this->notrans->Text,'0'))
			{				
				$sql ="SELECT 
						  '-' AS cm,	
						  tbt_rad_penjualan_lain.nama AS nama,
						  '-' AS dokter
						FROM
						  tbt_rad_penjualan_lain
						  INNER JOIN tbm_rad_tindakan ON (tbt_rad_penjualan_lain.id_tindakan = tbm_rad_tindakan.kode)
						WHERE tbt_rad_penjualan_lain.no_trans_rwtjln_lain='$tmp' AND tbt_rad_penjualan_lain.st_ambil='0' ";
				$arr = $this->queryAction($sql,'R');
			}	
		}
			...
			if (!$this->getViewState('nmTable'))
			{
				...
				if ($cek=='01')
				{
					$sql="SELECT 
						...
						WHERE tbt_rad_penjualan.no_trans_rwtjln='$noTrans' AND cm='$cm'";
					$arr=$this->queryAction($sql,'R');
				}elseif ($cek=='02')
				{
					$sql="SELECT 
						...
						WHERE tbt_rad_penjualan_inap.no_trans_inap='$noTrans' AND cm='$cm'";
					$arr=$this->queryAction($sql,'R');
				}else{
					$sql="SELECT 
						  tbm_rad_tindakan.nama,
						...
						FROM
						  tbt_rad_penjualan_lain
						  INNER JOIN tbm_rad_tindakan ON (tbt_rad_penjualan_lain.id_tindakan = tbm_rad_tindakan.kode)
						WHERE tbt_rad_penjualan_lain.no_trans_rwtjln_lain='$noTrans' ";
					$arr=$this->queryAction($sql,'R');
				}
				foreach($arr as $row)
				{
				...
				if ($cek=='01')
				{
					$sql="SELECT 
						...
						FROM
						  tbt_rad_penjualan
						  INNER JOIN tbm_rad_tindakan ON (tbt_rad_penjualan.id_tindakan = tbm_rad_tindakan.kode)
						WHERE tbt_rad_penjualan_inap.no_trans_rwtjln='$noTrans' AND cm='$cm'";
					$arr=$this->queryAction($sql,'R');
				}elseif ($cek=='02')
				{
					$sql="SELECT 
	...
	public function cetakClicked($sender,$param)
    {
		...
		if ($cek=='01')
		{
			$sql="UPDATE tbt_rad_penjualan SET st_ambil='1' WHERE no_trans_rwtjln='$noTrans' AND cm='$cm'";
			$this->queryAction($sql,'C');
		}elseif ($cek=='02')
		{	
			$sql="UPDATE tbt_rad_penjualan_inap SET st_ambil='1' WHERE no_trans_inap='$noTrans' AND cm='$cm'";
			$this->queryAction($sql,'C');
		}else{
			$sql="UPDATE tbt_rad_penjualan_lain SET st_ambil='1' WHERE no_trans_rwtjln_lain='$noTrans'";
			$this->queryAction($sql,'C');
		}
