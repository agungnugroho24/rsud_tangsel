...
class masterFisioNonPaketAsuransiEdit extends SimakConf
{
	...
	public function onPreRender($param)
	{
		...
		{
			$data = $this->FisioTdkPaketAsuransiDetailRecord;
			$stRawat = $data->st_rawat;
			
			$this->stRawat->SelectedValue = $stRawat ;
			...
			$this->DDKamar->SelectedValue = $data->id_asuransi;
			$this->setViewState('perusAwal',strtolower($data->id_asuransi));
			$this->setViewState('idTdkAwal',strtolower($data->id_tindakan));
			
			$idAsuransi = $data->id_asuransi;
			$idTdk = $data->id_tindakan;
			
			$sql = "SELECT * FROM tbm_fisio_tindakan_paket_asuransi_detail WHERE id_asuransi = '$idAsuransi' AND id_tindakan = '$idTdk' AND st_rawat = '$stRawat' AND id_paket = '0' ";
			foreach($this->queryAction($sql,'S') as $row)
			{
				$this->makeTmpTablePangkat($row['id_tindakan'],FisioTdkRecord::finder()->findByPk($row['id_tindakan'])->nama,'',$row['tarif'],$row['id'])	;
			}
		}
	...
	protected function getFisioTdkPaketAsuransiDetailRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$FisioTdkPaketAsuransiDetailRecord=FisioTdkPaketAsuransiDetailRecord::finder()->findByPk($ID);		
		if(!($FisioTdkPaketAsuransiDetailRecord instanceof FisioTdkPaketAsuransiDetailRecord))
			throw new THttpException(500,'id tidak benar.');
		return $FisioTdkPaketAsuransiDetailRecord;
	}	
	...
	public function TambahClicked($sender,$param)
	{
		if($this->Page->IsValid)
		{
			...
			$idTdkAwal = $this->getViewState('idTdkAwal');
			if($idTdkAwal == $kelas )
				$sql .= " AND id_tindakan <> '$idTdkAwal' ";
			...
			if($arr)
			{
				...
				('','unmaskContent();
					jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_error">Tindakan untuk Asuransi dan Status Perawatan yang dipilih sudah ada dalam database !</p>\',timeout: 3000,dialog:{
						modal: true
					}});');	
	...
	public function simpanClicked()
	{
		if($this->IsValid)
		{
			...
			/*$sql = "SELECT * FROM tbm_fisio_tindakan_paket_asuransi_detail WHERE LOWER(nama) = '$nama' AND id_asuransi = '$kateg' AND st_rawat = '$stRawat'";
			$arr = $this->queryAction($sql,'S');
				...
				if($this->getViewState('nmTable'))
				{
					...
					*/
					...
					foreach($this->queryAction($sql,'S') as $row)
					{
						...
						//DELETE tbm_operasi_kamar_tarif
						if($row['st_delete'] == '1')
						{
							$sql = "DELETE FROM tbm_fisio_tindakan_paket_asuransi_detail WHERE id = '$idAwal' ";	
							$this->queryAction($sql,'C');
						}
						else
						{	
							if($idAwal)
							{
								//UPDATE tbm_fisio_tindakan_paket_asuransi_detail
								$dataTarif = FisioTdkPaketAsuransiDetailRecord::finder()->findByPk($this->Request['ID']);
								$dataTarif->id_tindakan = $row['kelas'];
								$dataTarif->id_paket = '0';
								$dataTarif->id_asuransi = $kateg;	
								$dataTarif->tarif = $row['tarif'];
					...
					$this->getPage()->getClientScript()->registerEndScript
					('','unmaskContent();
						jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_question">Pengeditan tindakan Elektromedik asuransi sukses.</p>\',timeout: 600000,dialog:{
							modal: true,
							buttons: {
								"OK": function() {
	...
	public function prosesKonfirmasi($sender,$param)
	{
		...
		{
			$this->Response->redirect($this->Service->constructUrl('Fisio.masterFisioNonPaketAsuransi'));	
		}
	}
