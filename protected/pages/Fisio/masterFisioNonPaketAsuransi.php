<?php
class masterFisioNonPaketAsuransi extends SimakConf
{   	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
					  tbm_fisio_tindakan_paket_asuransi_detail.id,
					  tbm_fisio_tindakan_paket_asuransi_detail.id_asuransi,
					  tbm_perusahaan_asuransi.nama AS nm_perus,
					  tbm_fisio_tindakan_paket_asuransi_detail.id_tindakan,
					  tbm_fisio_tindakan.nama AS nm_tindakan,
					  tbm_fisio_tindakan_paket_asuransi_detail.tarif
					FROM
					  tbm_fisio_tindakan_paket_asuransi_detail
					  INNER JOIN tbm_fisio_tindakan ON (tbm_fisio_tindakan_paket_asuransi_detail.id_tindakan = tbm_fisio_tindakan.kode)
					  INNER JOIN tbm_perusahaan_asuransi ON (tbm_fisio_tindakan_paket_asuransi_detail.id_asuransi = tbm_perusahaan_asuransi.id)					
					WHERE tbm_fisio_tindakan_paket_asuransi_detail.id_paket = '0'  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_fisio_tindakan.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_fisio_tindakan.nama LIKE '$nama%' ";
			...
			{
				$stRawat = $this->stRawat->SelectedValue;
				$sql .= "AND tbm_fisio_tindakan_paket_asuransi_detail.st_rawat = '$stRawat' ";
			} 
	  ...
	  if ($this->User->IsAdmin)
		{
		 ...
		 // $idKateg = KamarOperasiTarifRecord::finder()->findByPk($ID)->id_kategori_operasi;
		  ...
		  $sql = "DELETE FROM tbm_fisio_tindakan_paket_asuransi_detail WHERE id='$ID' ";
		  $this->queryAction($sql,'C');
		  
		  /*$sql = "SELECT id FROM tbm_operasi_kamar_tarif WHERE id_kategori_operasi='$idKateg'";
		  if(count($this->queryAction($sql,'S')) == 0);
		  {
				$sql = "DELETE FROM tbm_operasi_kategori WHERE id='$idKateg' ";
				$this->queryAction($sql,'C');			  
		  }*/
		  
		  $this->getPage()->getClientScript()->registerEndScript
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Tindakan telah dihapus dari Data Master Tindakan Non Paket Asuransi Elektromedik .</p>\',timeout: 3000,dialog:{
				modal: true
			}});');	
	...
	public function editRow($sender,$param)
    {
		...
		$this->Response->redirect($this->Service->constructUrl('Fisio.masterFisioNonPaketAsuransiEdit',array('pageParent'=>$this->Page->getPagePath(),'ID'=>$ID)));	
    }	
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Fisio.masterFisioNonPaketAsuransiBaru'));		
	}
