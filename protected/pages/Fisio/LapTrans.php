...
{   
    ...
    public function onLoad($param)
	{
		...
		{	
			...
			$sql = "SELECT 
				...
				FROM
				  tbm_fisio_tindakan ORDER BY nama  ";
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT * FROM view_lap_fisio WHERE flag='1' ";
	...
	public function itemCreated($sender,$param)
    {
		...
		if($item->ItemType==='Item' || $item->ItemType==='AlternatingItem' ||$item->ItemType==='EditItem')
        {
			...
			if($tipe_pasien == '0')
			{
				...
				/*
				$tblFisioJual = 'tbt_fisio_penjualan';
				
				$sql = "SELECT 
						GROUP_CONCAT(tbm_fisio_tindakan.nama SEPARATOR ', ') AS nm_tdk
					FROM
					  tbm_fisio_tindakan
					  LEFT JOIN $tblFisioJual ON ( $tblFisioJual.id_tindakan = tbm_fisio_tindakan.kode ) 
					WHERE
						$tblFisioJual.no_reg = '$no_reg' ";
			...
			elseif($tipe_pasien == '1')
			{
				...
				$item->umurColumn->Text = $umur['years'].' th '.$umur['months'].' bln '.$umur['days'].' hr';
				/*
				$tblFisioJual = 'tbt_fisio_penjualan_inap';
			...
			elseif($tipe_pasien == '2')
			{
				...
				/*
				$tblFisioJual = 'tbt_fisio_penjualan_lain';
	...
	{
		//UPDATE tbt_fisio_hasil
		$sql = "SELECT id FROM tbt_fisio_hasil WHERE no_trans = '$noReg' ";
		$arr = $this->queryAction($sql,'S');
		foreach($arr as $row)
		{
			$id = $row['id'];
		...
		if ($jnsPasien=='0')
		{
			$sql="UPDATE tbt_fisio_penjualan SET st_ambil = '1' WHERE no_reg = '$noReg' ";
		}
		elseif ($jnsPasien=='1')
		{
			$sql="UPDATE tbt_fisio_penjualan_inap SET st_ambil = '1' WHERE no_reg = '$noReg' ";
		}
		elseif ($jnsPasien=='2')
		{
			$cm = '';
			$sql="UPDATE tbt_fisio_penjualan_lain SET st_ambil = '1' WHERE no_reg = '$noReg' ";
		}
	...
	{		
		...
		$this->Response->redirect($this->Service->constructUrl('Rad.cetakLapTransRad',
			array(
				'jnsPasien'=>$this->modeInput->SelectedValue,
