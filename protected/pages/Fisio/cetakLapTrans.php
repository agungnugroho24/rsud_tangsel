...
class cetakLapTrans extends SimakConf
{
	...
	{	
		...
		if($jnsPasien == '0')
			{
				$tblTbtRad= 'tbt_fisio_penjualan';
				$tblTbtRawat= 'tbt_rawat_jalan';
				
				 $sql = "SELECT 
					  $tblTbtRad.no_trans,
					  tbd_pasien.nama,
					  tbm_fisio_tindakan.nama AS tindakan,
					  $tblTbtRawat.cm,
					  $tblTbtRad.harga,
					...
					FROM
					  ...
					  INNER JOIN tbm_fisio_tindakan ON ($tblTbtRad.id_tindakan = tbm_fisio_tindakan.kode)
			...
			elseif($jnsPasien == '1')
			{
				$tblTbtRad = 'tbt_fisio_penjualan_inap';
			...
			elseif($jnsPasien == '2')
			{
				$tblTbtRad = 'tbt_fisio_penjualan_lain';
				 ...
				 $sql = "SELECT 
					...
					FROM
					  $tblTbtRad					  
					  INNER JOIN tbd_user ON ($tblTbtRad.operator = tbd_user.nip)					  
					  LEFT JOIN tbm_fisio_tindakan ON ($tblTbtRad.id_tindakan = tbm_fisio_tindakan.kode)";
			}
			...
			if($dokter <> '')			
				$sql .= "AND $tblTbtRad.dokter = '$dokter' ";
			...
			if($tgl <> '')			
				$sql .= "AND $tblTbtRad.tgl = '$tgl' ";
			
			if($tglawal <> '' AND $tglakhir <> '')			
				$sql .= "AND $tblTbtRad.tgl BETWEEN '$tglawal' AND '$tglakhir' ";
			
			if($bln <> '' AND $thn <> '')			
				$sql .= "AND MONTH ($tblTbtRad.tgl)='$bln' AND YEAR($tblTbtRad.tgl)='$thn' ";	
		...
		*/		
		...
		$pdf->Cell(450,8,'('.$this->User->IsUserName.')',0,0,'C','',$this->Service->constructUrl('Rad.LapTrans'));
