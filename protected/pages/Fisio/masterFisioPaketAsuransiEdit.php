...
class masterFisioPaketAsuransiEdit extends SimakConf
{
	...
	public function onPreRender($param)
	{
		...
		{
			$data = $this->FisioTdkPaketAsuransiRecord;
			...
			$this->tarif->Text = floatval($data->tarif);
			...
			$sql = "SELECT * FROM tbm_fisio_tindakan_paket_asuransi_detail WHERE id_asuransi = '$idAsuransi' AND st_rawat = '$stRawat' AND id_paket = '$idAwal' ";
			foreach($this->queryAction($sql,'S') as $row)
			{
				$this->makeTmpTablePangkat($row['id_tindakan'],FisioTdkRecord::finder()->findByPk($row['id_tindakan'])->nama,'','',$row['id'])	;
			}
		}
	...
	protected function getFisioTdkPaketAsuransiRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$FisioTdkPaketAsuransiRecord=FisioTdkPaketAsuransiRecord::finder()->findByPk($ID);		
		if(!($FisioTdkPaketAsuransiRecord instanceof FisioTdkPaketAsuransiRecord))
			throw new THttpException(500,'id tidak benar.');
		return $FisioTdkPaketAsuransiRecord;
	}	
	...
	public function simpanClicked()
	{
		if($this->IsValid)
		{
			...
			$sql = "SELECT * FROM tbm_fisio_tindakan_paket_asuransi WHERE id <> '$idAwal' AND LOWER(nama) = '$nama' AND id_asuransi = '$kateg' AND st_rawat = '$stRawat'";
			$arr = $this->queryAction($sql,'S');
			...
			else
			{
				...
				if($this->getViewState('nmTable'))
				{
					...
					//UPDATE tbm_operasi_kategori
					$data = FisioTdkPaketAsuransiRecord::finder()->findByPk($idAwal);
					...
					foreach($this->queryAction($sql,'S') as $row)
					{
						$idAwalTdk = $row['id_awal'];
					
						//DELETE tbm_operasi_kamar_tarif
						if($row['st_delete'] == '1')
						{
							$sql = "DELETE FROM tbm_fisio_tindakan_paket_asuransi_detail WHERE id = '$idAwalTdk' ";	
							$this->queryAction($sql,'C');
						}
						else
						{	
							if($idAwalTdk)
							{
								...
								$dataTarif = FisioTdkPaketAsuransiDetailRecord::finder()->findByPk($idAwalTdk);
								$dataTarif->id_tindakan = $row['kelas'];
								$dataTarif->id_paket = $idAwal;
							...
							{
								//INSERT tbm_fisio_tindakan_paket_asuransi_detail
					...
					$this->getPage()->getClientScript()->registerEndScript
					('','unmaskContent();
						jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_question">Paket <b>'.ucwords($this->nama->Text).'</b> telah diedit.</p>\',timeout: 600000,dialog:{
							modal: true,
							buttons: {
								"OK": function() {
