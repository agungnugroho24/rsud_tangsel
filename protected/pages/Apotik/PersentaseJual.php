<?php
class PersentaseJual extends SimakConf
{   
	private $sortExp = "id";
    private $sortDir = "ASC";
    private $offset = 0;
	...
	private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
            ...
            $sql = "SELECT * FROM tbm_obat_presentase_jual ";
	...
	public function itemCreated($sender,$param)
    {
		...
		if($item->ItemType==='EditItem')
        {
           ...
           $item->persentase->TextBox->Columns=5;
		   //$item->harga->TextBox->Columns=5;		   
        }       
		
		if($item->ItemType==='Item' || $item->ItemType==='AlternatingItem' || $item->ItemType==='DeleteItem')
        {
			$kel = $item->DataItem['kelompok'];
			$kateg = $item->DataItem['kategori'];
			
			if($kel=='01')
			{
				$item->kelompok->Text = 'Obat';
			}
			elseif($kel=='02')
			{
				$item->kelompok->Text = 'Alkes';
			}
			elseif($kel=='03')
			{
				$item->kelompok->Text = 'BHP';
			}
			
			
			if($kateg=='0')
			{
				$item->kategori->Text = 'Generik';
			}
			elseif($kateg=='1')
			{
				$item->kategori->Text = 'Non Generik';
			}
			else
			{
				$item->kategori->Text = '-';
		...
		//if($this->User->IsAdmin){
			...
			$item->Hapus->Attributes->onclick='if(!confirm(\'Apakah anda yakin ingin menghapus data ini..?\')) return false;';		
		}
		else{
	...
	public function saveItem($sender,$param)
    {
		...
		$persen = $item->persentase->TextBox->Text;
		
		$sql="UPDATE tbm_obat_presentase_jual SET persentase='$persen' WHERE id='$id'";
...

    {
        ...
        if ($this->User->IsAdmin)
		{
			...
			$sql = "DELETE FROM tbt_stok_lain WHERE id_harga='$idHarga' AND id_obat='$idObat' AND tujuan='$tujuan'";
			$this->queryAction($sql,'C');
