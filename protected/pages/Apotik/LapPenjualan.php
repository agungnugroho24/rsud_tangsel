...
{  	
	...
	*/
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			if($this->modeLap->SelectedValue == '0') //laporan penjualan
			{
				...
				*/
				$sql = "SELECT tbm_obat.nama AS obat,
							  ...
							  $tblName.hrg_nett AS beli,
							  $tblName.jumlah AS jumlah,
							  $tblName.cm AS cm,
							   $tblName.dokter AS dokter,
							  $tblName.tgl AS tgl,
							  (($tblName.hrg_nett * 1.1) * $tblName.jumlah) AS beli_tot,
							  ($tblName.total - ($tblName.r_item + $tblName.r_racik + $tblName.bungkus_racik)) AS jual_tot,
							  (($tblName.total - ($tblName.r_item + $tblName.r_racik + $tblName.bungkus_racik)) - ((($tblName.hrg_nett * 1.1) - ($tblName.hrg_nett * tbt_obat_harga.disc / 100 )) * $tblName.jumlah) - ($tblName.hrg_nett * tbm_obat.persentase_dokter / 100 * $tblName.jumlah )) AS profit,
			...
			if($this->modeLap->SelectedValue == '0') //laporan penjualan
				$sql .= " GROUP BY $tblName.id_obat, $tblName.no_reg, $tblName.id_obat, $tblName.expired ";			
			elseif($this->modeLap->SelectedValue == '1') //laporan /R
    ...
    */
	...
	public function RBkelObatChanged()
	{
		...
		if($this->RBkelObat->SelectedValue == 8)//Kel. Narkotika
		{
	...
	{			
		...
		if($this->totalkanJml->Checked === true)
			$totalkanJml = 1;
		else
			$totalkanJml = 0;
