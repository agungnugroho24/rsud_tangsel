...
{   
	...
	{		
		...
		{				
			...
			$this->DDRuangan->DataSource = UnitInternalRecord::finder()->findAll($criteria);
			$this->DDRuangan->dataBind();
			...
			if($this->Request['cm'] !='' && $this->Request['noTrans'] != '' && $this->Request['jnsPasien'] != '')
			{
				...
				$this->checkRegister($this->notrans,$param);
				//$this->checkNoTrans();
			...
			$this->cariPasBtn->Attributes->onclick = 'popup(\'index.php?page=Tarif.CariPasienRwtJlnAktif&parentPage=Apotik.penjualanObat&notransUpdate='.$this->notrans->getClientID().'&jnsPasien='.$jnsPasien.'&mode=1\',\'tes\')';
	...
	public function checkOtorisasi()
    {
		if($this->User->IsAdmin)
		{
			$sql = "SELECT * FROM tbm_destinasi_farmasi ORDER BY nama";
			$this->modeStok->DataSource = DesFarmasiRecord::finder()->findAllBySql($sql);
			$this->modeStok->dataBind();
			$this->modeStok->Enabled = true;
		}
		else
		{
			$sql = "SELECT * FROM tbm_destinasi_farmasi WHERE id='14'";
			$this->modeStok->DataSource = DesFarmasiRecord::finder()->findAllBySql($sql);
			$this->modeStok->dataBind();
			
			$this->modeStok->SelectedValue = '14';
			$this->setViewState('modeStok',$this->modeStok->SelectedValue);
			$this->modeStok->Enabled = false;
			$this->modeStokChanged();
		}
	}
	...
	public function showDokter()
	{
		if($this->DDKlinik->SelectedValue!='')
		{
			...
			if($kelompokPasien=='02' || $kelompokPasien=='07') //jika pasien asuransi/jamper
			{
				...
				{
					$this->clearViewState('plafonObat');
				}	
			}
    ...
    {	
		if($this->IsValid)  // when all validations succeed
        {
			...
			{		
				...
				//jika ada plafon obat asuransi => UPDATE tbt_rawat_jalan.tanggungan_obat
				if($this->getViewState('plafonObat'))
				{	
					$totSementara = $this->getViewState('totSementara');
					$tanggungan_asuransi_obat = $totSementara;
					
					$sql = "UPDATE tbt_rawat_jalan SET tanggungan_asuransi_obat = '$tanggungan_asuransi_obat' WHERE no_trans = '$noTransRwtJln' ";
					$this->queryAction($sql,'C');
				}
			...
			{		
				...
				foreach($arrData as $row)
				{
					$notransTmp = $this->numCounter('tbt_obat_jual_internal',ObatJualInternalRecord::finder(),'43');
