<?php
class LapPenjualan20091023 extends SimakConf
{  	
	private $sortExp = "id";
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			if($rawat == '0')
			{
				if($tujuan<>'')
				{
					if ($tujuan<>'1')
					{
					$sql .=	" '0' AS tipeRawat FROM tbm_obat a,							
								tbt_obat_jual b,
								tbd_pasien c
							WHERE	 							
			...
			elseif($rawat == '1')
			{
				if($tujuan<>'')
				{
					if ($tujuan<>'1')
					{
					$sql .=	" '1' AS tipeRawat FROM tbm_obat a,							
			...
			elseif($rawat == '2')
			{
				if($tujuan<>'')
				{
					if ($tujuan<>'1')
					{
					$sql .=	" '2' AS tipeRawat FROM tbm_obat a,							
			...
			if($cariByNama <> '')			
				if($elemenBy === true){
						$sql .= "AND a.nama LIKE '%$cariByNama%' ";
					}
					else
					{	
						$sql .= "AND a.nama LIKE '$cariByNama%' ";
			...
			if($cariByID <> '')			
				$sql .= "AND a.kode = '$cariByID' ";
						
			if($cariByGol <> '')			
				$sql .= "AND a.gol = '$cariByGol' ";
			
			if($cariByJenis <> '')
			{			
				$sql .= "AND a.kategori = '$cariByJenis' ";
			...
			if($cariByKlas <> '')					
				$sql .= "AND a.klasifikasi = '$cariByKlas' ";		
				
			if($cariByDerivat <> '')	
			{		
				$sql .= "AND a.derivat = '$cariByDerivat' ";
				$sql .= "AND a.derivat=d.id	";		
			}
			
			if($cariByPbf <> '')					
				$sql .= "AND a.pbf = '$cariByPbf' ";				
			
			if($cariByProd <> '')				
				$sql .= "AND a.produsen = '$cariByProd' ";				
			
			if($cariBySat <> '')			
				$sql .= "AND a.satuan = '$cariBySat' ";
