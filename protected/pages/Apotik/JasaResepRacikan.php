<?php
class JasaResepRacikan extends SimakConf
{   
	private $sortExp = "id";
    private $sortDir = "ASC";
    private $offset = 0;
	...
	public function itemCreated($sender,$param)
    {
		...
		if($item->ItemType==='EditItem')
        {
		   ...
		   $item->resep->TextBox->Columns=3;
		   $item->racikan->TextBox->Columns=3;
		   $item->bungkus->TextBox->Columns=3;
        }       
	...
	public function saveItem($sender,$param)
    {
		...
		$update = JasaResepRacikanRecord::finder()->findByPk($id);
		$update->jasa_resep = $item->resep->TextBox->Text; 
		$update->jasa_racikan = $item->racikan->TextBox->Text; 
		$update->bungkus = $item->bungkus->TextBox->Text; 
...

    {
        if ($this->User->IsAdmin)
		{
			...
			$this->Response->redirect($this->Service->constructUrl('Apotik.JasaResepRacikan'));
