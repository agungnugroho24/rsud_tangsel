<?php
class pengambilanResepInap extends SimakConf
{   
    private $sortExp = "id";
    private $sortDir = "ASC";
    private $offset = 0;
	...
	public function pindahButtonClicked($sender, $param)
	{
		...
		$sql = "UPDATE 
					tbt_obat_jual_inap
				SET
					tbt_obat_jual_inap.flag_salinan_resep = 
					CASE 
	...
	public function checkInput()  // fungsi yang ada querynya buat nentuin ke salinan resep apa ngga
        {
           ...
           if(ObatJualInapRecord::finder()->find('cm = ?', $this->formatCm($this->noCM->Text)))
           {			
           ...
           $sql ="SELECT 
                    ...
                    tbt_obat_jual_inap.keterangan_obat,
                    tbt_obat_jual_inap.tgl,
                    tbt_obat_jual_inap.id_obat,
                    tbt_obat_jual_inap.id_kel_racik,
                    tbt_obat_jual_inap.id_trans_pengganti,
                    tbt_obat_jual_inap.id_harga,
                    tbt_obat_jual_inap.id_kemasan,
                    tbt_obat_jual_inap.bungkus_racik,
                    tbt_obat_jual_inap.bhp,
                    tbt_obat_jual_inap.id_bhp,
                    tbt_obat_jual_inap.flag_ambil_resep,
                    tbt_obat_jual_inap.flag_salinan_resep,
                  ...
                  FROM
                    tbt_obat_jual_inap
                  WHERE
                    tbt_obat_jual_inap.no_reg = '$no_trans'";
                ...
                if( $arr = $this->queryAction($sql,'S')) //jika pengecekan no_trans di tbt_obat_jual suskes
		{
                    ...
                    foreach( $arr as $row )
                    {
                        ...
                        $kategori = ObatRecord::finder()->findByPk($row['id_obat'])->kategori;
                        $jenisObat = "Obat";
                        ...
                        // Mencari id_pengganti apabila obat merupakan obat yang digantikan
                        $sql2 = "SELECT id_obat FROM tbt_obat_jual_inap WHERE no_trans ='".$row['id_trans_pengganti']."'";
                        ...
                        // List Obat yang masuk ke Penebusan Resep
                        ...
                        // List Obat yang masuk ke Salinan Resep
    ...
    public function penebusanResep()
    {
        ...
        foreach( $arr as $row )
        {
            ...
            $sql ="UPDATE tbt_obat_jual_inap SET flag_ambil_resep = '1' WHERE id = '".$row['id']."'";
            $this->queryAction($sql,'C');
            $sql2 = "UPDATE tbt_obat_jual_inap SET flag_ambil_resep = '1' WHERE id_trans_pengganti ='".$row['id']."'";
        }
	...
	{  
            ...
            $url = "index.php?page=Apotik.gantiObatInap&id=".$no_trans;
            $this->getPage()->getClientScript()->registerEndScript('',"tesFrame('$url',650,500,'PENGGANTIAN OBAT RAWAT INAP')");
	}
	...
	{	
		...
		foreach($arr as $row)
		{
			...
			$notrans_inap=$row['no_trans_inap'];//notrans tbt_rawat_jalan
			$dokter=$row['dokter'];
			$sumber=$row['sumber'];
			$tujuan=$row['tujuan'];
			$idObat=$row['id_obat'];
	...
	public function cetakClicked($sender,$param)
		...
		foreach($arr as $row)
		{
			...
			$stAsuransi = RwtInapRecord::finder()->findByPk($noTransJln)->st_asuransi;
			if($stAsuransi == '1')
			{
				$hrg_bulat = $hrg;
			...
			{	
				...
				if($r_item > 0 )
				{
					...
					$sql="SELECT * FROM tbt_obat_jual_inap WHERE no_trans_inap = '$notrans_inap' AND id_obat='$idObat' AND id_harga = '$id_harga' AND expired <> '$expired' ";
					$arr=$this->queryAction($sql,'S');
					if(count($arr) > 0)
					{
						foreach($arr as $row)
						{
							...
							$noTransJlnUpdate = $row['no_trans_inap'];
							$r_itemUpdate = $row['r_item'] + $r_item;
							
							$totalRealUpdate = $row['total_real'] + $r_item + $r_racik + $bungkus_racik;
									
							$stAsuransi = RwtInapRecord::finder()->findByPk($noTransJlnUpdate)->st_asuransi;
							if($stAsuransi == '1')
							{
					...
					{
						$sql="SELECT * FROM tbt_obat_jual_inap WHERE no_trans_inap = '$notrans_inap' AND id_obat='$idObat' AND id_harga <> '$id_harga' ";
						$arr=$this->queryAction($sql,'S');
						if(count($arr) > 0)
						{
							foreach($arr as $row)
						...
						{
							$sql="SELECT * FROM tbt_obat_jual_inap WHERE no_trans_inap = '$notrans_inap' AND id_obat <> '$idObat' ";
							$arr=$this->queryAction($sql,'S');
							if(count($arr) > 0)
							{
								foreach($arr as $row)
