<?php
class gantiObatInap extends SimakConf
{	
	...
	public function onPreRender($param)
	{
            ...
            {			
                if($this->Request['id'] != '')
                {
                    ...
                    $sql = "SELECT * FROM tbt_obat_jual_inap WHERE no_trans='".$no_trans."'";
	...
	public function updateClicked()
	{
		...
		$notransTmp = $this->numCounter('tbt_obat_jual_inap',ObatJualInapRecord::finder(),'10'); //nomor transaksi 
		$noReg = $this->numRegister('tbt_obat_jual_inap',ObatJualInapRecord::finder(),'10'); //nomor register atau nomor obat
		...
		//$ObatJual = new ObatJualRecord();
			...
			$sql2 = "INSERT INTO tbt_obat_jual_inap( no_trans, no_trans_inap, no_reg, cm, dokter,tgl,wkt,operator,sumber,tujuan,id_obat,
			...
			VALUES('$no_transTmp','$no_trans_inap','$no_reg','$cm','$dokter','$tgl','$wkt','$operator',
			...
			$sql3 = "UPDATE tbt_obat_jual_inap SET id_trans_pengganti='".$no_transTmp."' WHERE no_trans='".$no_trans."'";
			$this->queryAction($sql3,'C');

			/*
			$sql4 = "UPDATE tbt_obat_jual_inap SET flag_ambil_resep='1' WHERE no_trans='".$no_trans."'";
