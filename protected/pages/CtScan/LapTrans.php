...
{   
    ...
    public function onLoad($param)
	{
		...
		{	
			...
			$sql = "SELECT 
				...
				FROM
				  tbm_ctscan_tindakan ORDER BY nama  ";
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT * FROM view_lap_ctscan WHERE flag='1' ";
	...
	public function itemCreated($sender,$param)
    {
		...
		if($item->ItemType==='Item' || $item->ItemType==='AlternatingItem' ||$item->ItemType==='EditItem')
        {
			...
			if($tipe_pasien == '0')
			{
				...
				/*
				$tblCtScanJual = 'tbt_ctscan_penjualan';
				
				$sql = "SELECT 
						GROUP_CONCAT(tbm_ctscan_tindakan.nama SEPARATOR ', ') AS nm_tdk
					FROM
					  tbm_ctscan_tindakan
					  LEFT JOIN $tblCtScanJual ON ( $tblCtScanJual.id_tindakan = tbm_ctscan_tindakan.kode ) 
					WHERE
						$tblCtScanJual.no_reg = '$no_reg' ";
			...
			elseif($tipe_pasien == '1')
			{
				...
				$item->umurColumn->Text = $umur['years'].' th '.$umur['months'].' bln '.$umur['days'].' hr';
				/*
				$tblCtScanJual = 'tbt_ctscan_penjualan_inap';
			...
			elseif($tipe_pasien == '2')
			{
				...
				$item->umurColumn->Text = PasienLuarRecord::finder()->findByPk($item->DataItem['no_trans_asal'])->umur;
				/*
				$tblCtScanJual = 'tbt_ctscan_penjualan_lain';
	...
	public function ambilClicked($sender,$param)
	{
		...
		{	
			...
			if($jmlData > 0) //jika ada hasil lab yg diambil
			{
				...
				jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">Pengambilan Hasil Rad Sukses <br/><br/></p>\',timeout: 6000,dialog:{
					modal: true,
					buttons: {
						"OK": function() {
							jQuery( this ).dialog( "close" );
	...
	public function prosesAmbil($noReg)
	{
		...
		$sql = "SELECT id FROM tbt_ctscan_hasil WHERE no_trans = '$noReg' ";
		$arr = $this->queryAction($sql,'S');
		foreach($arr as $row)
		{
			$id = $row['id'];
			
			$data = RadHasilRecord::finder()->findByPk($id);
		...
		if ($jnsPasien=='0')
		{
			$sql="UPDATE tbt_ctscan_penjualan SET st_ambil = '1' WHERE no_reg = '$noReg' ";
		}
		elseif ($jnsPasien=='1')
		{
			$sql="UPDATE tbt_ctscan_penjualan_inap SET st_ambil = '1' WHERE no_reg = '$noReg' ";
		}
		elseif ($jnsPasien=='2')
		{
			$cm = '';
			$sql="UPDATE tbt_ctscan_penjualan_lain SET st_ambil = '1' WHERE no_reg = '$noReg' ";
		}
	...
	{		
		...
		$session->open();
		$session['cetakLapTransRad'] = $this->getViewState('sql');
		
		$this->Response->redirect($this->Service->constructUrl('CtScan.cetakLapTransCtScan',
			array(
				'jnsPasien'=>$this->modeInput->SelectedValue,
				...
				'operatorRad'=>$this->DDOperator->SelectedValue,
