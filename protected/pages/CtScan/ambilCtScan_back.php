...
{   
	...
	public function checkRegister($sender,$param)
    {
        ...
        if(CtScanJualRecord::finder()->find('no_trans = ? AND st_ambil=?', $this->notrans->Text,'0'))
		{			
			...
			$sql ="SELECT 
					...
					FROM
			  		...
			  		WHERE tbt_ctscan_penjualan.no_trans='$tmp' ";
	...
	*/
	...
	public function cetakClicked($sender,$param)
    {
		$noTrans=$this->getViewState('notrans');
		$cm=$this->getViewState('cm');
		$sql="UPDATE tbt_ctscan_penjualan SET st_ambil='1' WHERE no_trans='$noTrans' AND cm='$cm'";
		$this->queryAction($sql,'C');	
