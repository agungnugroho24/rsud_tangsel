<?php
class masterCtScan extends SimakConf
{   
	/**
	...
	protected function getDataRows($offset,$rows,$order,$sort,$pil,$cariNama,$cariID,$tipeCari,$cariKategori,$cariKel)
	{
		if($pil == "1")
		{
			$sql = "SELECT 
						   ...
						   b.jenis AS kategori,
						   c.nama AS kelompok
						FROM tbm_ctscan_tindakan a,
						     tbm_ctscan_kategori b,
							 tbm_ctscan_kelompok c
						WHERE	 
							 a.kategori=b.kode
							 AND a.kelompok=c.kode ";
		...
		$page=CtScanTdkRecord::finder()->findAllBySql($sql);
    ...
    */
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('CtScan.masterRadBaru'));		
	}
