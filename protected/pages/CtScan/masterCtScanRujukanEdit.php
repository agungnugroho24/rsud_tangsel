...
class masterRadRujukanEdit extends SimakConf
{
	...
	public function onPreLoad($param)
	{
		...
		{
			$dataRadRujukan=$this->CtScanRujukanRecord;
	...
	*/
	...
	protected function getCtScanRujukanRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$CtScanRujukanRecord=CtScanRujukanRecord::finder()->findByPk($ID);		
		if(!($CtScanRujukanRecord instanceof CtScanRujukanRecord))
			throw new THttpException(500,'id tidak benar.');
		return $CtScanRujukanRecord;
	}	
