<?php
class masterRadKelEdit extends SimakConf
{
	...
	public function onPreLoad($param)
	{
		...
		{
			$KelurahanRecord = $this->CtScanKelRecord;	
	...
	public function simpanClicked($sender,$param)	{			
		...
		if($this->IsValid)  
        {
			...
			{
				$sql = "SELECT nama FROM tbm_ctscan_kelompok WHERE LOWER(nama) = '$nama' ";
	...
	*/
	...
	protected function getCtScanKelRecord()
	{
		// use Active Record to look for the specified username
		$ID=$this->Request['ID'];
		$this->setViewState('ID',$ID);
		$CtScanKelRecord=CtScanKelRecord::finder()->findByPk($ID);		
		if(!($CtScanKelRecord instanceof CtScanKelRecord))
			throw new THttpException(500,'id tidak benar.');
		return $CtScanKelRecord;
	}	
