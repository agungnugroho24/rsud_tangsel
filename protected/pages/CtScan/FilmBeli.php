<?php
class FilmBeli extends SimakConf
{
	private $sortExp = "id";
    private $sortDir = "ASC";
    private $offset = 0;
 	...
 	public function onPreRender($param)
	{
		...
		if(!$this->IsPostBack && !$this->IsCallBack)
		{
			...
			$sql = "SELECT id FROM tbt_film_beli";		
			$data = FilmBeliRecord::finder()->findBySql($sql);
			if($data==NULL)
			{
			...
			{
				//$this->noPO->Text = $this->numUrut('tbt_film_beli',FilmBeliRecord::finder(),'4').'/ZHRH-PO/'.$this->bulanRomawi(date('m')).'/'.date('Y');
			...
			$sqlBarang = "SELECT id, nama FROM tbm_film WHERE id <> '' ";
			$this->setViewState('sqlBarang',$sqlBarang);
			
			$this->DDNamaBrg->DataSource=FilmRecord::finder()->findAllBySql($sqlBarang);
    ...
    {			
		//Mbikin No Urut
		$find = FilmBeliRecord::finder();//::finder();		
		$sql = "SELECT no_po FROM tbt_film_beli order by id desc";
	...
	{	
		if(!$this->DDJenis->SelectedValue=='')
		{
			...
			//$sql = "SELECT id,nama FROM tbm_film_kelompok WHERE id_jenis_barang = '$idJnsBrg' ORDER BY nama )";
			...
			$sqlBarang = "SELECT id, nama FROM tbm_film WHERE id_jenis_barang = '$idJnsBrg'  ";
			$this->setViewState('sqlBarang',$sqlBarang);
		...
		$this->DDNamaBrg->DataSource = FilmRecord::finder()->findAllBySql($sql);
		$this->DDNamaBrg->dataBind(); 	
	...
	{		
		if($this->DDKelompok->SelectedValue != ''){
			...
			{
				$sqlBarang = "SELECT id, nama FROM tbm_film WHERE id_jenis_barang = '$idJnsBrg' AND id_kelompok_barang = '$DDKelompok' ";
				$this->setViewState('sqlBarang',$sqlBarang);
				$sql = $sqlBarang." ORDER BY nama ";
			}
			...
			elseif(!$idJnsBrg && $DDKelompok)
			{
				$sqlBarang = "SELECT id, nama FROM tbm_film WHERE id_kelompok_barang = '$DDKelompok'  ";
				$this->setViewState('sqlBarang',$sqlBarang);
				$sql = $sqlBarang." ORDER BY nama ";
			}
			else
			...
			//$this->DDNamaBrg->DataSource=FilmRecord::finder()->findAll('kategori = ?', $this->DDJenis->SelectedValue);
			$this->DDNamaBrg->DataSource = FilmRecord::finder()->findAllBySql($sql);
			$this->DDNamaBrg->dataBind(); 	
			$this->DDNamaBrg->Enabled=true;
			//$this->DDNamaBrg->focus();
	...
	{		
		...
		else
		{
			...
			$this->DDNamaBrg->DataSource=FilmRecord::finder()->findAll();
			$this->DDNamaBrg->dataBind();
			$this->DDNamaBrg->Enabled=false;
			$this->clearViewState('minLoket');
			$this->clearViewState('maxLoket');
	...
	public function getDummyData($token) 
	{
		if($this->getViewState('sqlBarang'))
		{
			...
			$sql .= " AND tbm_film.nama LIKE '$token%' "; 
			
			if($this->DDJenis->SelectedValue != '')
			{
				$id_jenis_barang = $this->DDJenis->SelectedValue;	
				$sql .= " AND tbm_film.id_jenis_barang = '$id_jenis_barang' "; 
			}
			
			if($this->DDKelompok->SelectedValue != '')
			{
				$id_kelompok_barang = $this->DDKelompok->SelectedValue;	
				$sql .= " AND tbm_film.id_kelompok_barang = '$id_kelompok_barang' "; 
			}
			
			$sql .= " GROUP BY tbm_film.id ORDER BY tbm_film.nama "; 
	...
	public function DDNamaBrgChanged()
	{
		...
		{
			//$minLoket = FilmRecord::finder()->findByPk($idBarang)->min_2;
			//$maxLoket = FilmRecord::finder()->findByPk($idBarang)->max_2;
	...
	{	
        ...
        { 
			...
			$sql = "SELECT no_po FROM tbt_film_beli WHERE no_po = '$noPO' AND st_rkbu_rtbu = '1' "; 
			if($this->queryAction($sql,'S'))
			{
				$this->getPage()->getClientScript()->registerEndScript
				('','unmaskContent();
					jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_error">No. Kontrak : <br/><b>'.$noPO.'</b><br/>sudah ada dalam database!</p>\',timeout: 3000,dialog:{
						modal: true
					}});');		
			}
			else
	...
	{		
		...
		$this->Response->redirect($this->Service->constructUrl('CtScan.cetakFilmBeli',array('noPO'=>$noPO,'tgl'=>$tgl,'jnsBayar'=>$jnsBayar,'jnsBeli'=>$jnsBeli,'pbf'=>$pbf,'nmApotik'=>$nmApotik,'stRkbu'=>'1')));
