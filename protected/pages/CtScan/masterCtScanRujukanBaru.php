 ...
 * @license http://www.intergo.co.id/license/
 */
class masterRadRujukanBaru extends SimakConf
{
	...
	public function checkRM($sender,$param)
    {
		...
		$data = count(CtScanRujukanRecord::finder()->findByPk($this->ID->Text));
		
		if($param->Value == ($data > 0))
            $param->IsValid=false;
			
       // $param->IsValid=(CtScanRujukanRecord::finder()->findByPk($this->ID->Text)===null);		
    }	   
	...
	public function simpanClicked()
	{
		if($this->IsValid)
		{
			$newPoliklinik=new CtScanRujukanRecord();
