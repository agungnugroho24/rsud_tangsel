...
class uploadFile extends SimakConf
{   
	...
	public function bindData()
    {
		for($i=1; $i<=$this->jml->Text; $i++)
		{
			...
			$sql="INSERT INTO $nmTable (nama) VALUES ('')";
			$this->queryAction($sql,'C');
		}
	...
	public function batalClicked($sender,$param)
    {
		...
		$this->getPage()->getClientScript()->registerEndScript
		('','window.parent.maskContent(); window.parent.modalCallback('.$index.'); jQuery.FrameDialog.closeDialog();');
		
		/*
		$this->cariPanel->Enabled = true;
		$this->uploadPanel->Display = 'None';
		$this->jml->Text = '';
		...
		$this->getPage()->getClientScript()->registerEndScript
		('','window.parent.maskContent(); window.parent.modalCallback('.$index.',\'pesan baru\'); jQuery.FrameDialog.closeDialog();');	
		*/
		...
		elseif($foto->HasFile) //ada file yg akan di upload
		{
			if($tipeFile == '1') //tipe foto
			{
				...
				else
				{
					...
					$url = 'protected/pages/CtScan/foto//'.$nmFile;
					$urlTmp ='protected/pages/CtScan/foto/tmp//'.$nmFile;
