...
class masterRadBaru extends SimakConf
{
    ...
    {       
		...
		$param->IsValid=(CtScanTdkRecord::finder()->findByPk($this->ID->Text)===null);
    }
	...
	{			
		if($this->IsValid)  // when all validations succeed
        {
			...
			{
				//INSERT tbm_ctscan_tindakan
				$newCtScanTdkRecord=new CtScanTdkRecord;
				$newCtScanTdkRecord->kategori = $this->DDRadKateg->SelectedValue;
				$newCtScanTdkRecord->kelompok = $this->DDRadKel->SelectedValue;        
				$newCtScanTdkRecord->nama = ucwords($this->nama->Text);
				$newCtScanTdkRecord->save();
				
				//INSERT tbm_lab_tarif
				$sql = "SELECT kode FROM tbm_ctscan_tindakan ORDER BY kode DESC";
				$kodeTdk = CtScanTdkRecord::finder()->findBySql($sql)->kode;
					
				$newLabTarifRecord = new CtScanTarifRecord;
