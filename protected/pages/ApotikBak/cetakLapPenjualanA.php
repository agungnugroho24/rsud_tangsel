	...
	{	
		...
		*/
		...
		if($modeLap == '0')//laporan penjualan
		{
			...
			else
			{
				...
				$pdf->Cell(33,5,'(HNA+PPn)*Jumlah',1,0,'C');
				$pdf->Cell(33,5,'Total Jual',1,0,'C');
		...
		*/			
		...
		if($modeLap == '0') //laporan penjualan
		{
			if($modeNarkotika != '1') //laporan narkotika
			{
				$pdf->Cell(133,5,'GRAND TOTAL',0,0,'R');	
					
				//$pdf->Cell(30,5,'Rp ' .number_format($totBeli,2,',','.'),1,0,'R');	
				//$pdf->Cell(30,5,'Rp ' .number_format($totJual,2,',','.'),1,0,'R');		
				$pdf->Cell(33,5,'Rp ' .number_format($grandTotJual,2,',','.'),1,0,'R');		
				$pdf->Cell(33,5,'Rp ' .number_format( $grandTot,2,',','.'),1,0,'R');
