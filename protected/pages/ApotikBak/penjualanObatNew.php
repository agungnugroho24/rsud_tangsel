...
{   
	...
	private function karyawanAction($notrans)
	{
		...
		*/
		...
		if($kelompokPasien == '04' || $jenisPasien=='3')//Bila kelompok pasien adalah karyawan
		{
			$this->modeKryPas->SelectedValue = '0';				
		}
		else
		{
			$this->modeKryPas->SelectedValue = '3';				
	...
	*/
	...
	public function makeTmpTbl($id_harga,$jmlAmbil)
	{
		...
		if($kelompokPasien == '04') //kelompok pasien adalah karyawan
			{	
				if($this->getViewState('modeKryPas') == '0')//karyawan yg bersangkutan lgsg
				{
					$r_item = 0;
					$r_racik = 0;
				}
				else if($this->getViewState('modeKryPas') == '1')//Keluarga Inti suami, istri anak
				{
					$r_item = 0;
					$r_racik = 0;
				}
				else if($this->getViewState('modeKryPas') == '2')//Diluar keluarga inti
				{
					$r_item = 1000;
					$r_racik = 3000;
				}	
    ...
    {	
		if($this->IsValid)  // when all validations succeed
        {
			...
			{		
				...
				$stAsuransi = RwtjlnRecord::finder()->findByPk($this->notrans->Text)->st_asuransi;
			...
			*/
			...
			if($jnsPasien == '0') //Pasien Rawat Jalan
			{
				...
				$this->Response->redirect($this->Service->constructUrl('Apotik.cetakNotaObatBeta',array('notrans'=>$notransTmp,'cm'=>$cm,'dokter'=>$this->DDDokter->SelectedValue,'jnsPasien'=>$jnsPasien,'kelompokPasien'=>$kelompokPasien,'jmlTagihan'=>$jmlTagihan,'mode'=>$modeByrInap)));
			}
