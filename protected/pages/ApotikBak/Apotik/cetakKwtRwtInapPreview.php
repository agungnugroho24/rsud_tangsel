...
class cetakKwtRwtInapPreview extends SimakConf
{
	...
	{	
		...
		*/	
		...
		$pdf->Cell(120,5,'',0,0,'C');	
		$pdf->Cell(80,8,'('.$operator.')',0,0,'C','',$this->Service->constructUrl('Apotik.BayarKasirRwtInap') . '&purge=1');	
		$pdf->Ln(5);
		...
		*/
		...
		$pdf->Ln(1);
			...
			//------------------------------- Biaya Lain-Lain lab & rad yg st_bayar=0 ----------------------------------//
		$pdf->SetFont('Arial','B',9);
		...
		$sql = "SELECT * FROM view_biaya_lain_lab_rad WHERE cm='$cm' AND no_trans='$notrans' AND flag = 0 AND st_bayar = 0";
		$arr=$this->queryAction($sql,'S');
		foreach($arr as $row)
		{
			$pdf->SetFont('Arial','',8);
