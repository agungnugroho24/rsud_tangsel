<?php
class cetakObatMasuk1 extends SimakConf
{
	...
	{	
		...
		{
			$table = 'tbt_obat_beli';
			$activeRecord = ObatBeliRecord::finder();
		}
		elseif($modeTerima=='1')
		{
			$table = 'tbt_obat_beli_tunda';
			$activeRecord = ObatBeliTundaRecord::finder();
		}
		...
		*/
		...
		$pdf->Cell(0,5,'Telp. (021) 74718440','0',0,'L');
			...
			$pdf->Cell(230,5,'TANDA TERIMA OBAT','0',0,'C','',$this->Service->constructUrl('Farmasi.ObatMasuk'));
			...
			*/
			...
			$pdf->Cell(2,5,':  ',0,0,'L');
			$pdf->Cell(140,5,'Logistik Farmasi',0,0,'L');
			...
			$sql="SELECT 
					  ...
					  $table.pbf,
					  $table.kode,
					  tbt_obat_masuk.jumlah AS jumlah_kecil,
					  tbt_obat_masuk.jumlah_tunda_sisa,
					...
					FROM
					  tbt_obat_masuk
					  INNER JOIN $table ON (tbt_obat_masuk.no_po = $table.no_po)
					WHERE $table.no_po='$noPO'  
					GROUP BY
					  tbt_obat_masuk.tgl_exp
					ORDER BY tbt_obat_masuk.id     ";
			*/
			...
			foreach($arrData as $row)
			{
				...
				$sql2="SELECT 
						...
						WHERE 
							...
							AND tbt_obat_masuk.no_po='$noPO'  
						GROUP BY
						  tbt_obat_masuk.tgl_exp, tbt_obat_masuk.no_po
						ORDER BY
						  tbt_obat_masuk.id DESC ";
				...
				$pdf->Cell(55,5,ObatRecord::finder()->findByPk($row['kode'])->nama,'1',0,'L');
			...
			$pdf->Cell(45,5,'Logistik Farmasi',1,0,'C');
			$pdf->Ln(5);
			$pdf->Cell(200,5,'',0,0,'C');
