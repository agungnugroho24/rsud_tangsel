<?php
class cetakObatMasuk2 extends SimakConf
{
	...
	{	
		...
		*/
		...
		// 
			...
			$pdf->Cell(35,10,'Kemasan','1',0,'C');
			$pdf->Cell(170,5,'Harga','1',0,'C');
			$pdf->Ln(5);
			
			$pdf->Cell(15,5,'','0',0,'C');
			$pdf->Cell(30,5,'Faktur','1',0,'C');
			$pdf->Cell(65,5,'','0',0,'C');
			$pdf->Cell(20,5,'Barang','1',0,'C');
			$pdf->Cell(35,5,'','0',0,'C');
			$pdf->Cell(25,5,'Satuan','1',0,'C');
			$pdf->Cell(30,5,'Total','1',0,'C');
			$pdf->Cell(30,5,'Potongan','1',0,'C');
			$pdf->Cell(30,5,'PPN','1',0,'C');
			$pdf->Cell(25,5,'Materai','1',0,'C');
			$pdf->Cell(30,5,'Keseluruhan','1',0,'C');
			...
			$sql="SELECT
				...
				FROM 
					tbt_obat_masuk 
				WHERE 
					no_po='$noPO' AND no_faktur='$nofak'";
			...
			{
				$j += 1;			
				$idSatuan = ObatRecord::finder()->findByPk($row['id_obat'])->satuan;
				...
				if($row['discount'] > 0) //jika ada => discount harga ppn discount
				{
					...
					$pdf->Cell(35,5,SatuanObatRecord::finder()->findByPk($idSatuan)->nama,'1',0,'L');
					$pdf->Cell(25,5,'Rp ' .number_format($row['hrg_disc'],2,',','.'),1,0,'R');
					$pdf->Cell(30,5,'Rp ' .number_format($total,2,',','.'),1,0,'R');
					$pdf->Cell(30,5,$row['discount'].'%','1',0,'C');
					$pdf->Cell(30,5,'','1',0,'C');
					$pdf->Cell(25,5,'','1',0,'C');
					$pdf->Cell(30,5,'','1',0,'C');
				...
				else //harga ppn 
				{
					...
					$pdf->Cell(20,5,$row['jumlah'],'1',0,'R');
					$pdf->Cell(35,5,SatuanObatRecord::finder()->findByPk($idSatuan)->nama,'1',0,'L');
					$pdf->Cell(25,5,'Rp ' .number_format($row['hrg_nett'],2,',','.'),1,0,'R');
					$pdf->Cell(30,5,'Rp ' .number_format($total,2,',','.'),1,0,'R');
					$pdf->Cell(30,5,'0%','1',0,'C');
					$pdf->Cell(30,5,'','1',0,'C');
					$pdf->Cell(25,5,'','1',0,'C');
					$pdf->Cell(30,5,'','1',0,'C');
					//$pdf->Cell(30,5,'Rp ' .number_format($row['hrg_ppn'],2,',','.'),1,0,'R');
			...
			$pdf->Cell(30,5,'',0,0,'R');		
			$pdf->Cell(30,5,'Rp ' .number_format( $totsem,2,',','.'),1,0,'R');
			//$pdf->Ln(5);	
			
			if ($stPot=='1')
			{
				//$pdf->Cell(110,8,'',0,0,'L');
				//$pdf->Cell(35,5,'Potongan ',0,0,'R');		
				$pdf->Cell(30,5,'Rp ' .number_format( $pot,2,',','.'),1,0,'R');
				//$pdf->Ln(5);					
			}
			else
			{
			...
			if ($mat==1)
			{
				...
				$pdf->Cell(25,5,'Rp ' .number_format( '3000',2,',','.'),1,0,'R');
				//$pdf->Ln(5);	
			}
			elseif ($mat==2)
			{
				//$pdf->Cell(110,8,'',0,0,'L');
				//$pdf->Cell(35,5,'Materai ',0,0,'R');		
				$pdf->Cell(25,5,'Rp ' .number_format( '6000',2,',','.'),1,0,'R');
				//$pdf->Ln(5);	
			}
			else
			{
				//$pdf->Cell(110,8,'',0,0,'L');
				//$pdf->Cell(35,5,'Materai ',0,0,'R');		
				$pdf->Cell(25,5,'Rp ' .number_format( '',2,',','.'),1,0,'R');
				//$pdf->Ln(5);	
			}
			...
			{
				$pdf->Cell(30,5,'Rp ' .number_format( $grandTot + $jmlPpn + 6000,2,',','.'),1,0,'R');	
			}else
			{
