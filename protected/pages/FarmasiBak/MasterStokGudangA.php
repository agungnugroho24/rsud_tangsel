<?php
class MasterStokGudangA extends SimakConf
{   	
	...
	{				
		...
		if(!$this->IsPostBack)
		{
          ...
          $this->tujuanShow->Text=$this->getViewState('tujuan');
          $this->bindGrid();
        }
    ...
    public function setState($var,$param)
	{		
		if($this->getState($var))
        {
          $this->clearViewState('offsetEdit');
        }else{             
          $this->setViewState($var,$param);
        }
	}
    
    public function getState($var)
	{		
		$tmp=$this->getViewState($var);
        return $tmp;
	}
    
	public function DDTujuanChanged()
	{		
		$this->setState('Tujuan',$this->DDTujuan->SelectedValue);	
	}
