<?php
class cetakBayarPbf extends SimakConf
{
	...
	{	
		...
		$sql="SELECT 
			...
			WHERE tbt_bayar_pbf.no_trans='$noTrans' AND tbt_bayar_pbf.st = '1' AND tbt_bayar_pbf.st_tunda='0'
			GROUP BY
		...
		$sql="SELECT 
			...
			FROM
			  ...
			  INNER JOIN tbt_obat_beli ON (tbt_obat_masuk.no_po = tbt_obat_beli.no_po)
			  INNER JOIN tbm_pbf_obat ON (tbt_obat_beli.pbf = tbm_pbf_obat.id)
			WHERE tbt_bayar_pbf.no_trans='$noTrans' AND tbt_bayar_pbf.st = '1' AND tbt_bayar_pbf.st_tunda='1'
			GROUP BY
			  tbt_obat_masuk.no_faktur
			  ";
		$arr=$this->queryAction($sql,'S');//Select row in tabel bro...
		...
		$sql = "SELECT 
				...
				WHERE 
					...
					AND tbt_obat_masuk.st_keuangan = '1' 
		...
		$pdf->Ln(15);
		$pdf->Cell(65,8,'( '.$operator.' )',0,0,'C','',$this->Service->constructUrl('Farmasi.BayarPbf'));	
		$pdf->Cell(65,8,'',0,0,'C');
		$pdf->Cell(65,8,'( Manajer Keuangan )',0,0,'C','',$this->Service->constructUrl('Farmasi.BayarPbf'));
		$pdf->Ln(10);
		...
		$pdf->Ln(15);
		$pdf->Cell(65,8,'',0,0,'C');
		$pdf->Cell(65,8,'( Direktur )',0,0,'C','',$this->Service->constructUrl('Farmasi.BayarPbf'));
		$pdf->Cell(65,8,'',0,0,'C');
