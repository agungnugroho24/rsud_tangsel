<?php
class DaftarCariRwtInapPdf extends SimakConf
{   
	...
	{	
		...
		$cariByKelas=$this->Request['cariByKelas'];
		$cariByKamar=$this->Request['cariByKamar'];
		...
		*/
		...
		if($jmlData!=0)
		{
			...
			$pdf->Cell(0,5,'HASIL PENCARIAN DATA PASIEN RAWAT INAP','0',0,'C','',$this->Service->constructUrl('Pendaftaran.DaftarCariRwtInap'));			
			
			$pdf->Ln(7);
			$pdf->SetFont('Arial','B',9);
			$pdf->Cell(15,10,'No.','RTL',0,'C');
			$pdf->Cell(15,10,'No. RM','RTL',0,'C');		
			$pdf->Cell(40,10,'Nama Lengkap','RTL',0,'C');
			$pdf->Cell(20,5,'Jenis','RTL',0,'C');
			$pdf->Cell(30,10,'Kamar/Ruang','RTL',0,'C');
			$pdf->Cell(20,10,'Kelas','RTL',0,'C');
			$pdf->Cell(50,10,'Dokter','RTL',0,'C');
			$pdf->Ln(5);
			$pdf->Cell(15,5,'','RBL',0,'C');
			$pdf->Cell(15,5,'','RBL',0,'C');		
			$pdf->Cell(40,5,'','RBL',0,'C');
			...
			foreach($arrData as $row)
			{
				...
				$y=$pdf->GetY();
				$x=$pdf->GetX();
				$pdf->MultiCell(30,10,$row['kamar'],'1','C');
				
				$pdf->setY($y);
				$pdf->setX($x+30);
				$y=$pdf->GetY();
				$x=$pdf->GetX();
				$pdf->MultiCell(20,10,$row['kelas'],'1','C');
		...
		else
		{
			...
			$pdf->Cell(0,5,'DATA PASIEN RAWAT INAP TIDAK DITEMUKAN','0',0,'C','',$this->Service->constructUrl('Pendaftaran.DaftarCariRwtInap'));			
			$pdf->Ln(7);
		}
