<?php
class DaftarCariDataSearch extends SimakConf
{
	...
	{	
		...
		*/
		...
		$sql = "SELECT 
				...
				$where GROUP BY tbd_pasien.cm $sort $limit ";
		...
		$sql2 = "SELECT 
				...
				FROM 
				  ...
				  LEFT JOIN tbm_perusahaan_asuransi ON (tbd_pasien.perusahaan = tbm_perusahaan_asuransi.id) 
				  INNER JOIN tbt_rawat_jalan ON (tbd_pasien.cm = tbt_rawat_jalan.cm)
				$where GROUP BY tbd_pasien.cm $sort ";
		...
		*/
		...
		foreach($result as $row)
		{
			...
			$items .= <<<EOD
			{
				id : '{$row['cm']}',cell : ['{$row['cm']}','{$row['nama']}','{$row['nm_penjamin']}','{$row['jkel']}','{$row['tgl_lahir']}','{$row['alamat']}']
			}
EOD;
			//$items .= "}";
			if ($i < $total )
