...
class CariDataUmumModal extends SimakConf
{   
	private $sortExp = "cm ";
    private $sortDir = "ASC";
    private $offset = 0;
	...
	{
		$idProp = $this->DDProp->SelectedValue;
		$this->DDKab->SelectedValue = 'empty';
		$this->DDKab->Enabled = false;
		
		if($idProp != '')
		{
			$sql = "SELECT * FROM tbm_kabupaten WHERE id_propinsi='$idProp' ORDER BY nama";
			$this->DDKab->DataSource = KabupatenRecord::finder()->findAllBySql($sql);
			$this->DDKab->dataBind();
  ...
  public function itemCreated($sender,$param)
	...
	public function pilihClicked($sender,$param)
	{
		...
		$ID = intval($param->CommandParameter);
