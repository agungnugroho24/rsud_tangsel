<?php
class DaftarCariRwtIgd extends SimakConf
{   
	/**
	...
	*/
	protected function getDataRows($offset,$rows,$order,$sort,$pil,$cariNama,$cariCM,$urutBy,$tipeCari,$Company,$cariAlamat,$cariTgl,$cariBln,$cariByDokter)
	{
		if($pil == "1")
		{
			$sql = "SELECT a.cm,
    ...
    */   
	...
	{				
		...
		{					
			...
			//$this->UserGrid->VirtualItemCount=IgdRecord::finder()->count();
			// fetches all data account information 
			
			$jmlData=count($this->queryAction($this->getDataRows($offset,$limit,$orderBy,'','1',$cariByNama,$cariByCM,$urutBy,$elemenBy,$companyBy,$cariByAlamat,$cariByTgl,$cariByBulan,$cariByDokter),'S'));
			
			$this->UserGrid->VirtualItemCount=$jmlData;
			
			$this->jmlDataPas->Text=$jmlData;
			
			$this->UserGrid->DataSource=$this->queryAction($this->getDataRows($offset,$limit,$orderBy,'','2',$cariByNama,$cariByCM,$urutBy,$elemenBy,$companyBy,$cariByAlamat,$cariByTgl,$cariByBulan,$cariByDokter),'S');
    ...
    public function deleteButtonClicked($sender,$param)
    {
        if ($this->User->IsAdmin)
		{
			...
			$this->Response->redirect($this->Service->constructUrl('Pendaftaran.DaftarCariRwtIgd'));
	...
	{		
		...
		$this->UserGrid->DataSource=$this->queryAction($this->getDataRows($offset,$limit,$orderBy,'','1',$cariByNama,$cariByCM,$urutBy,$elemenBy,$companyBy,$cariByAlamat,$cariByTgl,$cariByBulan,$cariByDokter),'S');
	...
	{		
		if($this->CBpdf->Checked==false)
		{
			...
			$jmlData=count($this->queryAction($this->getDataRows($offset,$limit,$orderBy,'','1',$this->cariNama->Text,$this->cariCM->Text,$this->DDUrut->SelectedValue,$this->Advance->Checked,$this->DDKontrak->SelectedValue,$this->cariAlamat->Text,$this->tglMsk->Text,$this->DDBulan->SelectedValue,$this->DDDokter->SelectedValue),'S'));
			
			$this->UserGrid->VirtualItemCount=$jmlData;
						
			$this->jmlDataPas->Text=$jmlData;
			
			$this->UserGrid->DataSource=$this->queryAction($this->getDataRows($offset,$limit,$orderBy,'','1',$this->cariNama->Text,$this->cariCM->Text,$this->DDUrut->SelectedValue,$this->Advance->Checked,$this->DDKontrak->SelectedValue,$this->cariAlamat->Text,$this->tglMsk->Text,$this->DDBulan->SelectedValue,$this->DDDokter->SelectedValue),'S');
		...
		else
		{
			...
			$this->Response->redirect($this->Service->constructUrl('Pendaftaran.DaftarCariRwtIgdPdf',array('cariCM'=>$cariCM,'cariNama'=>$cariNama,'tipeCari'=>$tipeCari,'cariAlamat'=>$cariAlamat,'urutBy'=>$urutBy,'Company'=>$Company,'cariTgl'=>$cariTgl,'cariBln'=>$cariBln,'cariByDokter'=>$cariByDokter)));
		}	
	}
