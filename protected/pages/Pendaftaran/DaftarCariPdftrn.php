           ...
           if($this->jnsPas->SelectedValue == '0' )
			...
			elseif($this->jnsPas->SelectedValue == '1' )
			{
				$sql = "SELECT '1' AS tipe_rawat,
							...
							FROM
							  tbt_rawat_inap
							  LEFT JOIN tbd_pasien ON (tbt_rawat_inap.cm = tbd_pasien.cm)
							  LEFT JOIN tbd_pegawai ON (tbt_rawat_inap.dokter = tbd_pegawai.id)
							  LEFT JOIN tbm_kabupaten ON (tbd_pasien.kabupaten = tbm_kabupaten.id)
							  LEFT JOIN tbm_kelompok ON (tbt_rawat_inap.penjamin = tbm_kelompok.id)
							  LEFT JOIN tbm_kamar_kelas ON (tbt_rawat_inap.kelas = tbm_kamar_kelas.id)	
							  LEFT JOIN tbm_ruang ON (tbt_rawat_inap.kamar = tbm_ruang.id)						  								
  ...
  {	
	...
	protected function dtgSomeData_ItemCreated($sender,$param)
	{
		...
		{	
			if($this->jnsPas->SelectedValue == '0' )
			{
				...
				elseif($idKlinik=='15')//HAJI
					...
					$sql = "SELECT * FROM tbd_pegawai WHERE kelompok='1' AND poliklinik='$idKlinik' ORDER BY nama ";
			...
			if($this->User->IsAdmin){
				...
				$item->Hapus->Button->Attributes->onclick='if(!confirm(\'Apakah anda yakin ingin'.$tipe.' menghapus data ini..?\')) return false;';			
			}else{
		...
		{	
			...
			{				
				...
				*/
				//$this->dtgSomeData->Columns[8]->Visible = false; //dokter
				$this->dtgSomeData->Columns[9]->Visible = false; //klinik
    ...
    protected function dtgSomeData_UpdateCommand($sender,$param)
    {
		...
		if($this->jnsPas->SelectedValue == '1' )
		{
			...
			if ($this->User->IsAdmin || $this->User->IsPower)
			{
    ...
    protected function dtgSomeData_DeleteCommand($sender,$param)
    {
		...
		if ($this->User->IsAdmin)
		{
			...
			elseif($this->jnsPas->SelectedValue == '1')
			{
				RwtInapRecord::finder()->deleteByPk($ID);
			}
		}
		else
		{
	...
	{		
		...
		/*if($id_klinik == '04')//Bila POLI GIGI
		{
			$this->Response->redirect($this->Service->constructUrl('Tarif.cetakKartuStatusGigi',array('cm'=>$cm,'notrans'=>$noTrans,'poli'=>$id_klinik,'dokter'=>$dokter,'pen'=>$pen,'shift'=>$shift,'notrans'=>$noTrans,'mode'=>'01')));	
		}
		elseif($id_klinik == '06')//Bila IGD
		{
			$this->Response->redirect($this->Service->constructUrl('Tarif.cetakKartuStatusIgd',array('cm'=>$cm,'notrans'=>$noTrans,'poli'=>$id_klinik,'dokter'=>$dokter,'pen'=>$pen,'shift'=>$shift,'notrans'=>$noTrans,'mode'=>'01')));
		}
		else
		{
				$this->Response->redirect($this->Service->constructUrl('Tarif.cetakKartuStatusPoli',array('cm'=>$cm,'notrans'=>$noTrans,'poli'=>$id_klinik,'dokter'=>$dokter,'pen'=>$pen,'shift'=>$shift,'notrans'=>$noTrans,'mode'=>'01')));
		}	*/
