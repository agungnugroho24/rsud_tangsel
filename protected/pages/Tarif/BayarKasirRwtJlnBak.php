...
{   
	...
	public function checkNoTrans()
    {
		...
		{	
			...
			else
			{
				...
				if($jmlHarga >0 || $jmlHarga != NULL) //jika ada transaksi tdk rwtjln/lab/rad
				{
				...
				else //jika tidak ada transaksi tdk rwtjln/lab/rad
				{
					...
					$this->noTransCtrl->Visible=false;
					$this->showSecond->Visible=false;
					$this->errMsg->Text='Tidak ada transaksi Tindakan Rawat Jalan / Lab / Rad!';
					$this->notrans->Text = '';
					$this->notrans->focus();				
					$this->citoCheck->Enabled=true;
				}				
    ...
    {	
		...
		$this->Response->redirect($this->Service->constructUrl('Tarif.cetakKwtRwtJln',array('notrans'=>$notrans,'cm'=>$cm,'nama'=>$nama,'jmlTagihan'=>$jmlTagihan,'sisaByr'=>$sisaByr,'jmlBayar'=>$jmlBayar,'petugasPoli'=>$this->getViewState('petugasPoli'),'jnsPasien'=>$jnsPasien,'jnsTrans'=>$jnsTrans)));
