<?php
class cetakLapJasmedRwtJln extends SimakConf
{
	...
	{	
		...
		$session->open();
		$sql = $session['cetakLapJasmedRwtJln'];
		$sql .= ' ORDER BY cm';
		...
		$pdf->Cell(30,5,'Tarif',1,0,'C');
		$pdf->Cell(30,5,'Sarana RS',1,0,'C');
		$pdf->Cell(30,5,'Dokter',1,0,'C');	
		$pdf->Ln(5);
		...
		foreach($arrData as $row)
		{
			...
			$pdf->Cell(30,5,number_format($row['tarif'],2,',','.'),1,0,'R');
			$pdf->Cell(30,5,number_format($row['kesra'],2,',','.'),1,0,'R');
			$pdf->Cell(30,5,number_format($row['jasmed_dokter'],2,',','.'),1,0,'R');	
			
			$tot_tarif += $row['tarif'];
			$tot_kesra += $row['kesra'];
			$tot_dok += $row['jasmed_dokter'];
			//$totProfit += $profit;
			$pdf->Ln(5);
		}				
		
		$pdf->SetFont('Arial','B',9);
		$pdf->Cell(185,5,'GRAND TOTAL',1,0,'C');		
		$pdf->Cell(30,5,number_format($tot_tarif,2,',','.'),1,0,'R');
		$pdf->Cell(30,5,number_format($tot_kesra,2,',','.'),1,0,'R');
		$pdf->Cell(30,5,number_format($tot_dok,2,',','.'),1,0,'R');	
		...
		$pdf->Cell(30,7,'',1,0,'R');
		$pdf->Ln(7);
		$pdf->Cell(170,7,'B. Jasa Medis Dokter',1,0,'L');		
		$pdf->Cell(30,7,'Rp. '.number_format($tot_dok,2,',','.'),1,0,'R');
		$pdf->Cell(30,7,'',1,0,'R');
