<?php
class BhpTindakan extends SimakConf
{   
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "SELECT 
						tbm_bhp_tindakan.id,
						tbm_bhp_tindakan.nama,
						tbm_bhp_tindakan.tarif,
						if(tbm_bhp_tindakan.st='0','BHP','Sewa Alkes') AS st
					FROM 
						tbm_bhp_tindakan 
					WHERE tbm_bhp_tindakan.id <> ''  ";
			
			if($this->cariNama->Text != '')
			{
				$nama = $this->cariNama->Text;		
				if($this->Advance->Checked === true){
					$sql .= "AND tbm_bhp_tindakan.nama LIKE '%$nama%' ";
				}
				else
				{	
					$sql .= "AND tbm_bhp_tindakan.nama LIKE '$nama%' ";
			...
			if($this->jenis->SelectedValue != '')
			{
				$jenis = $this->jenis->SelectedValue;
				$sql .= "AND tbm_bhp_tindakan.st = '$jenis' ";
			}
	  ...
	  if ($this->User->IsAdmin)
		{
		  ...
		  $sql = "DELETE FROM tbm_bhp_tindakan WHERE id = '$ID'";
		 ...
		 ('','unmaskContent();
			jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_info">BHP Tindakan '.$nama.'</b> telah dihapus dari database.<br/><br/></p>\',timeout: 4000,dialog:{
				modal: true,
				buttons: {
					"OK": function() {
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Tarif.BhpTindakanBaru'));		
	}
