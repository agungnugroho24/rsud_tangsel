<?php
class DaftarCariRwtJln extends SimakConf
{   
	...
	private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
            ...
            $sql = "SELECT tbd_pasien.cm AS cm,
						   ...
						   tbt_rawat_jalan.no_trans AS trans,
			...
			if($this->getViewState('cariByJkel') <> '')			
			{
				$jkel = $this->getViewState('cariByJkel');
				$sql .= "AND tbd_pasien.jkel = '$jkel' ";		
			}			
			...
			if($this->getViewState('cariBySapaan') <> '')			
			{
				$sapaan = $this->getViewState('cariBySapaan');
				$sql .= "AND tbd_pasien.satuation = '$sapaan' ";		
			}			
			...
			if($this->getViewState('cariByUrut') <> '')
			{
				...
				$sql .= "AND tbd_pasien.kelompok = '$urut' ";
			}	
			if($this->getViewState('cariByCompany') <> '')
			{
				$company = $this->getViewState('cariByCompany');
				$urut = $this->getViewState('cariByUrut');
				$sql .= "AND tbd_pasien.perusahaan = '$company' AND tbm_perusahaan.id_kel = '$urut'";						
			}
			$sql .= " GROUP BY tbt_rawat_jalan.cm ";      			
			...
			//----------Prepare Lazy Loading dummy data ----///			
			if (!$this->getViewState('nmTable'))
			{			
				$nmTable = $this->setNameTable('nmTable');
				$query="CREATE TABLE $nmTable (id INT (2) auto_increment,
											 ...
											 tdk_total FLOAT(11) NOT NULL,	
											 obat_total FLOAT(11) NOT NULL,
											 lab_total FLOAT(11) NOT NULL,
											 rad_total FLOAT(11) NOT NULL,
											 fis_total FLOAT(11) NOT NULL,
											 grand_tot FLOAT(11) NOT NULL,											 
			...
			foreach($arr as $row)
			{
				$cm = $row['cm'];
				$pasien = $row['nama'];
				$noTransRwtjln = $row['trans'];
				$tgl = $row['tgl'];
				$dokter = $row['dokter'];
				$klinik = $row['klinik'];				
				$v = "SELECT sum(total) FROM tbt_kasir_rwtjln WHERE no_trans_rwtjln='$noTransRwtjln' GROUP BY no_trans_rwtjln";
				$w = "SELECT sum(total) FROM tbt_obat_jual WHERE no_trans_rwtjln='$noTransRwtjln' GROUP BY no_trans_rwtjln";
				$x = "SELECT sum(harga) AS total FROM tbt_lab_penjualan WHERE no_trans_rwtjln='$noTransRwtjln' GROUP BY no_trans_rwtjln";
				$y = "SELECT sum(harga) AS total FROM tbt_rad_Penjualan WHERE no_trans_rwtjln='$noTransRwtjln' GROUP BY no_trans_rwtjln";
				$z = "SELECT sum(harga) AS total FROM tbt_fisio_Penjualan WHERE no_trans_rwtjln='$noTransRwtjln' GROUP BY no_trans_rwtjln";
				
				$sql = "SELECT * FROM $nmTable WHERE no_trans_rwtjln='$noTransRwtjln'";
				if(!$arr = $this->queryAction($sql,'S'))//if found do update not INSERT!!
				{
					if($a = $this->queryAction($v,'S'))					
						foreach($a as $rowa)
						{
							$tindakan = $rowa['total'];
							$sql = "INSERT INTO (no_trans_rwtjln,
									 tgl,
									 cm,
									 nama,
									 klinik,
					...
					else if($b = $this->queryAction($w,'S'))
					{					
						foreach($b as $rowb)
						{
							$tindakan = $rowb['total'];
					...
					else if($c = $this->queryAction($x,'S'))
					{					
						foreach($c as $rowc)
						{
							$tindakan = $rowc['total'];
					...
					else if($d = $this->queryAction($y,'S'))
					{					
						foreach($d as $rowd)
						{
							$tindakan = $rowd['total'];
					...
					else if($e = $this->queryAction($z,'S'))
					{					
						foreach($e as $rowe)
						{
							$tindakan = $rowe['total'];
				...
				else//Bila tem_table sudah ada isinya
				{
					/* Search and uodate total tindakan dari tabel kasir rawat jalan */					
					if($a = $this->queryAction($v,'S'))					
						foreach($a as $rowa)
						{
							$tindakan = $rowa['total'];
							$sql = "UPDATE $nmTable SET tdk_total='$tindakan' WHERE no_trans_rwtjln='$noTransRwtjln'";
							$this->queryAction($sql,'C');
						}
					}
					else if($b = $this->queryAction($w,'S'))
					{					
						...
						{
							$tindakan = $rowb['total'];
							$sql = "UPDATE $nmTable SET obat_total='$tindakan' WHERE no_trans_rwtjln='$noTransRwtjln'";
							$this->queryAction($sql,'C');
						}
					}
					else if($c = $this->queryAction($x,'S'))
					{					
						...
						{
							$tindakan = $rowc['total'];
							$sql = "UPDATE $nmTable SET lab_total='$tindakan' WHERE no_trans_rwtjln='$noTransRwtjln'";
							$this->queryAction($sql,'C');
						}
					}	
					else if($d = $this->queryAction($y,'S'))
					{					
						...
						{
							$tindakan = $rowd['total'];
							$sql = "UPDATE $nmTable SET rad_total='$tindakan' WHERE no_trans_rwtjln='$noTransRwtjln'";
							$this->queryAction($sql,'C');
						}
					}
					else if($e = $this->queryAction($z,'S'))
					{					
						foreach($e as $rowe)
							...
							$tindakan = $rowe['total'];
							$sql = "UPDATE $nmTable SET fisio_total='$tindakan' WHERE no_trans_rwtjln='$noTransRwtjln'";
							$this->queryAction($sql,'C');
						}
					}
				}//End of logika bila temp_table sudah ada isinya					
				
			}//End of fr each table sumber
			
			$sql = "SELECT tgl, cm, nama, klinik, dokter, tdk_tot, obat_tot, lab_tot, rad_tot, fis_tot";
    ...
    protected function dtgSomeData_UpdateCommand($sender,$param)
    {
		...
		//$tmpVar=$this->authApp('2');//Bila tidak ada hak utk modul aplikasi keuangan
		$tmpVar=$this->authApp('0');//Bila tidak ada hak utk modul aplikasi keuangan
		if($tmpVar == "True")
		{			
			if($dataKasir=KasirRwtJlnRecord::finder()->find('no_trans_rwtjln = ?',$this->dtgSomeData->DataKeys[$item->ItemIndex]))
			{
				//Update data dokter yang ada di tbt_rawat_jalan
				$oSomeData = RwtjlnRecord::finder()->findByPk($this->dtgSomeData->DataKeys[$item->ItemIndex]);
				$oSomeData->dokter = $dokter;
				$oSomeData->save();
				
				//Bila data dokter tsb sudah masuk pembayaran/kasir
				$dataKasir->dokter = $dokter;
				$dataKasir->save();
			}
			else
			{
	...
	{	
		...
		{		
			...
			if($this->DDSapaan->SelectedValue){ 		
				$this->setViewState('cariBySapaan', $this->DDSapaan->SelectedValue);	
			}else{
				$this->clearViewState('cariBySapaan');	
			}
			...
			if($this->jkel){ 		
				$jkel = $this->collectSelectionResult($this->jkel);
				$this->setViewState('cariByJkel', $jkel);	
			}else{
				$this->clearViewState('cariByJkel');	
			}
	...
	public function DDSapaanChanged($sender,$param)
	{		
		$this->cariClicked();
	}
	...
	public function JkelChanged($sender,$param)
	{		
		$this->cariClicked();
	}
