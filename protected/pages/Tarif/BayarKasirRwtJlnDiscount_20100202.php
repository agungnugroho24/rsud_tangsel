<?php
class BayarKasirRwtJlnDiscount_20100202 extends SimakConf
{   
	...
	public function rekapBayar()
    {
		if($this->getViewState('tmpJml') || $this->getViewState('tmpJml')==0)
		{
			...
			{
				if($jnsPasien == '0' || $jnsPasien == '2' || $jnsPasien == '3') //pasien rawat jalan atau bebas
				{
	...
	public function bindGridApotik()
    {
		...
		{	
			...
			if($kelompokPasien == '04' && $stAsuransi == '1') //kelompok pasien karyawan & st_asuransi berlaku
			{
				...
				$sqlBhp="SELECT id,tgl,id_bhp,bhp FROM tbt_obat_jual_karyawan WHERE no_trans_rwtjln='$notrans' AND bhp <> '0' AND flag = '0' ORDER BY id";				
			...
			else
			{
				...
				$sqlBhp="SELECT id,tgl,id_bhp,bhp FROM tbt_obat_jual  WHERE no_trans_rwtjln='$notrans' AND bhp <> '0' AND flag = '0'  ORDER BY id";
			}
			...
			if($jnsTrans == '1') //Transaksi Apotik
			{
				...
				$sqlApotik = " SELECT 
					...
					if($jnsPasien == '2')//Pasien bebas
					{
						...
						$sqlBhp="SELECT id,tgl,id_bhp,bhp FROM tbt_obat_jual_lain WHERE no_trans = '$notrans' AND bhp <> '0' AND flag = '0' ORDER BY id";
					}
					elseif($jnsPasien == '3')//Pasien bebas karyawan
					{
						$sqlApotik .= " tbt_obat_jual_lain_karyawan ";
						
						$sqlBhp="SELECT id,tgl,id_bhp,bhp FROM tbt_obat_jual_lain_karyawan WHERE no_trans = '$notrans' AND bhp <> '0' AND flag = '0' ORDER BY id";
					}
	...
	public function bindGridAmbulan()
    {
		$cm=$this->notrans->Text;
		$kelompokPasien = PasienRecord::finder()->findByPk($this->notrans->Text)->kelompok;
		...
		{	
			...
			$sqlApotik = "SELECT
									tbt_inap_ambulan.id AS id,
									tbt_inap_ambulan.tgl AS tgl,
									tbm_ambulan.nama AS tujuan,
									CONCAT(tbt_inap_ambulan.catatan,'  ',tbt_inap_ambulan.lainnya) AS catatan,
									tbt_inap_ambulan.tarif AS tarif
								FROM
									tbm_ambulan
									Inner Join tbt_inap_ambulan ON tbt_inap_ambulan.tujuan = tbm_ambulan.id
								WHERE 
									no_trans='$notrans'
									AND flag = '0'
									AND jns_pasien=0 ";
			...
			else
			{
				$jmlHargaApotik = 0;
				$this->apotikRwtJlnGrid->Visible = false;
				$this->apotikMsg->Text = 'Tidak Ada Transaksi Ambulan';			
			}	
		}/*
		elseif($jnsPasien == '1') //pasien rawat inap
		{
	...
	public function detailNonInap()
    {
		...
		{	
			...
			$jmlHargaAsli = $jmlHargaApotik+$jmlHargaAmbulan;
		...
		elseif($jnsPasien == '2' || $jnsPasien == '3') //pasien rawat bebas atau bebas karyawan
		{
			$jnsTrans = $this->collectSelectionResult($this->jnsTransBebas);
			if($jnsTrans == '1') //Transaksi Apotik
			{
	...
	public function kryChanged()
	{
		...
		$this->jmlTransLabChanged();
		$this->jmlTransRadChanged();
		$this->jmlTransFisioChanged();
	}
	
	public function jmlTransLabChanged()
    {
		$jmlTransLab = $this->jmlTransLab->Text;		
		$cek=$this->getViewState('krycheck');
		if($jmlTransLab > 0)
		{
			...
			if($jmlTransLab >= 60000 && $jmlTransLab < 250000)
			{
				//$this->admLab->Text = 10000;
				$trf=10000;
			}
			
			if($jmlTransLab >= 250000)
			{
	...
	public function jmlTransFisioChanged()
    {
		...
		if($jmlTransFisio > 0)
		{
			...
			if($jmlTransFisio >= 60000 && $jmlTransFisio < 250000)
			{
				//$this->admFisio->Text = 10000;
				$trf = 10000;
			}
			
			if($jmlTransFisio >= 250000)
			{
		...
		elseif($jnsPasien == '2' || $jnsPasien == '3') //pasien bebas atau bebas karyawan
		{
			...
			if($jnsTrans == '0') //Transaksi Non Apotik
			{
				...
				if($this->jmlTransLab->Text != '' )
				{
					...
					if($jnsPasien == '2')
					{ $transRwtJln->st_karyawan ='0'; }
					elseif($jnsPasien == '3')
					{ $transRwtJln->st_karyawan ='1'; }
			...
			elseif($jnsTrans == '1') //Transaksi Apotik
			{
				...
				if(count($arr)>0) //jika ada transaksi apotik, UPDATE flag di tbt_obat_jual_lain
				{
					if($jnsPasien == '2')
					{ 
						$sql="UPDATE tbt_obat_jual_lain SET no_trans_rwtjln_lain='$notrans', flag='1',operator_kasir='$nipTmp' WHERE no_trans ='$noTransRwtBebas' AND flag='0' ";		
					}
					elseif($jnsPasien == '3')
					{
						$sql="UPDATE tbt_obat_jual_lain_karyawan SET no_trans_rwtjln_lain='$notrans', flag='1',operator_kasir='$nipTmp' WHERE no_trans ='$noTransRwtBebas' AND flag='0' ";		
					}
