<?php
class cetakKwtRwtInapDiscountNoKop extends SimakConf
{
	...
	{	
		...
		*/	
		...
		$pdf->Cell(45,10,'',0,0,'L');
		$pdf->Cell(2,10,'',0,0,'L');
		$pdf->Cell(63,10,'Total Setelah Discount',0,0,'L');
		$pdf->Cell(2,10,':  ',0,0,'L');
		...
		*/
		...
		$sql = "SELECT * FROM view_obat_alkes WHERE cm='$cm' AND no_trans_inap='$notrans' AND flag=1 AND st_bayar=0";
			$arr=$this->queryAction($sql,'S');
			foreach($arr as $row)
			{
				$pdf->SetFont('Arial','',8);
		...
		$sql = "SELECT jml FROM view_biaya_lain_lab_rad WHERE jenis = 'lab' AND cm='$cm' AND no_trans='$notrans' AND flag = 1 AND st_bayar = 0";
		$arr=$this->queryAction($sql,'S');
		foreach($arr as $row)
		{
			$totLab += $row['jml'];
		...
		$sql = "SELECT * FROM view_biaya_lain_lab_rad WHERE jenis = 'lab' AND cm='$cm' AND no_trans='$notrans' AND flag = 1 AND st_bayar = 0";
		$arr=$this->queryAction($sql,'S');
		foreach($arr as $row)
		{
			$pdf->SetFont('Arial','',8);
		...
		$sql = "SELECT jml FROM view_biaya_lain_lab_rad WHERE jenis = 'rad' AND cm='$cm' AND no_trans='$notrans' AND flag = 1 AND st_bayar = 0";
		$arr=$this->queryAction($sql,'S');
		foreach($arr as $row)
		{
			$totRad += $row['jml'];
		...
		$sql = "SELECT * FROM view_biaya_lain_lab_rad WHERE jenis = 'rad' AND cm='$cm' AND no_trans='$notrans' AND flag = 1 AND st_bayar = 0";
		$arr=$this->queryAction($sql,'S');
		foreach($arr as $row)
		{
			$pdf->SetFont('Arial','',8);
		...
		$totLain = $totAskep + $totAskepOK + $totAskeb + $totOksigen + $totAmbulan;
		...
		if(InapAskepOkRecord::finder()->find('no_trans=? AND flag=?',$notrans,'1'))				
		{
			$sql = "SELECT nama,tgl,tarif 
						   FROM tbt_inap_askep_ok			
						   WHERE no_trans = '$notrans'
		...
		if(InapAskebRecord::finder()->find('no_trans=? AND flag=?',$notrans,'1'))				
		{
			$sql = "SELECT nama,tgl,tarif 
						   FROM tbt_inap_askeb			
						   WHERE no_trans = '$notrans'
		...
		if(AmbulanRecord::finder()->find('no_trans=?',$noTrans))				
		{
			...
			foreach($arr as $row)
			{
				...
				$pdf->Cell(25,5,$this->convertDate($row['tgl'],'3'),1,0,'C');
				$pdf->SetFont('Arial','',9);
				$pdf->Cell(45,5,'Ambulan',1,0,'L');
				$pdf->Cell(50,5,'',1,0,'L');
				$pdf->Cell(42,5,$row['tujuan'],1,0,'L');
				$pdf->Cell(30,5,'Rp. '.number_format($row['jumlah'],2,',','.'),1,0,'R');
				$pdf->Ln(5);
			}
		}
		...
		if(RwtInapRecord::finder()->find('no_trans=? AND st_alih=?',$notrans,'1'))			
		{
			...
			*/
			...
			foreach($arr as $row)
			{
				...
				$sql = "SELECT dokter FROM tbt_kasir_rwtjln WHERE no_trans_rwtjln = '$noTransJln' AND st_flag = '1'";
