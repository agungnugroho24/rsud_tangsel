...
{   
	...
	// ---------- datagrid helper fucktions --------
    public function getSortExpression() {
        if ($this->getViewState('sortExpression',null)!==null) {
            return $this->getViewState('sortExpression');
        }
        // set default in case there's nonok 'sortExpression' key in viewstate
    ...
    public function getSortDirection() {
        ...
        // set default in case there's nodonk 'sortDirection' key in viewstate
    ...
    {
        // clear data in sexion because we need to get a new page from db
	...
	private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
			...
			$sql = "select 
						...
						tbm_kelompok.id AS kelompok,
						IF ((substr(tbm_nama_tindakan.nama,1,4)='Jasa'),(0.9 * tbt_kasir_rwtjln.total),(0.8 * tbt_kasir_rwtjln.total)) AS jasmed_dok,
	...
	{	
		...
		$session->open();
		$session['cetakLapJasmedDok'] = $this->getViewState('sql');
