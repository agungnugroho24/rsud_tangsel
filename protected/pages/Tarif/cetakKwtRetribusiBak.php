...
class cetakKwtRetribusi extends SimakConf
{
	...
	{	
		...
		*/
		...
		$pdf=new reportKwitansi('P','mm','kwt_antrian');
		$pdf->AliasNbPages(); 
		$pdf->AddPage();
		$pdf->SetLeftMargin(5);
		...
		$pdf->Cell(0,5,'KUITANSI PEMBAYARAN RETRIBUSI','0',0,'C','',$this->Service->constructUrl('Tarif.BayarKasirRwtJlnRetribusi'));
		...
		$pdf->Cell(30,10,'Pasien',0,0,'L');
		$pdf->Cell(2,10,':  ',0,0,'L');
		$pdf->Cell(30,10,PasienRecord::finder()->findByPk($cm)->nama,0,0,'L');
		$pdf->Ln(5);
		
		$kelompok = PasienRecord::finder()->findByPk($cm)->kelompok;
		
		$pdf->Cell(30,10,'Penjamin',0,0,'L');
		$pdf->Cell(2,10,':  ',0,0,'L');
		if($kelompok != '')
		{
			if($kelompok == '01')
			...
			$penjamin = PerusahaanRecord::finder()->findByPk(PasienRecord::finder()->findByPk($cm)->perusahaan)->nama;
		}
		else
		{
			$penjamin = 'Umum';
		...
		$pdf->Cell(80,8,'Kota Tangerang Selatan, '.substr($tgl_kasir,8,2) . ' ' . $this->namaBulan(substr($tgl_kasir,5,2)) . ' ' . substr($tgl_kasir,0,4),0,0,'C');
		$pdf->Ln(5);
		...
		$pdf->Cell(120,5,'',0,0,'C');	
		$pdf->Cell(80,8,'('.$operator.')',0,0,'C','',$this->Service->constructUrl('Tarif.BayarKasirRwtJlnRetribusi'));	
		$pdf->Ln(5);
