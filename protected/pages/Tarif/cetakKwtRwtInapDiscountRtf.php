<?php
class cetakKwtRwtInapDiscountRtf extends SimakConf
{
...
$table->writeToCell(1, 2, 'Telp. (021) 74718440', new Font(10,'Arial'), $parHead );
			...
			if($this->textToNumber($jsObat) > 0)
			{
				...
				$sql = "SELECT * FROM view_obat_alkes WHERE cm='$cm' AND no_trans_inap='$notrans' AND flag=1 AND st_bayar=0 ORDER BY tgl";
				$arr=$this->queryAction($sql,'S');
				if(count($arr) > 0)
				{	
					$table = &$sect->addTable();			
			...
			$sql = "SELECT jml FROM view_biaya_lain_lab_rad WHERE jenis = 'fisio' AND cm='$cm' AND no_trans='$notrans' AND flag = 1 AND st_bayar = 0";
			$arr=$this->queryAction($sql,'S');
			foreach($arr as $row)
			{
				$totFisio += $row['jml'];
			...
			if($totFisio > 0)
			{
				...
				$sql = "SELECT * FROM view_biaya_lain_lab_rad WHERE jenis = 'fisio' AND cm='$cm' AND no_trans='$notrans' AND flag = 1 AND st_bayar = 0";
				$arr=$this->queryAction($sql,'S');
				if(count($arr) > 0)
				{	
					$table = &$sect->addTable();			
