<?php
class cetakKwtRwtInapDiscount extends SimakConf
{
	...
	{	
		...
		*/
		...
		//$pdf->Cell(2,10,':  ',0,0,'L');
		//$pdf->Cell(30,10,'Rp. '.number_format($jsKamar,2,',','.'),0,0,'L');
		$pdf->Ln(10);
		...
		*/
		...
		$pdf->Cell(45,10,'',0,0,'L');
		$pdf->Cell(2,10,'',0,0,'L');
		$pdf->Cell(63,10,'Jumlah bayar',0,0,'L');
		$pdf->Cell(2,10,':  ',0,0,'L');
		...
		*/	
		...
		$sql = "SELECT jml FROM view_biaya_lain_lab_rad WHERE jenis = 'fisio' AND cm='$cm' AND no_trans_asal='$notrans' AND flag = 1 AND st_bayar = 0";
		$arr=$this->queryAction($sql,'S');
		foreach($arr as $row)
		{
			$totFisio += $row['jml'];
		...
		{			
			...
			$sql = "SELECT * FROM view_biaya_lain_lab_rad WHERE jenis = 'fisio' AND cm='$cm' AND no_trans_asal='$notrans' AND flag = 1 AND st_bayar = 0";
			$arr=$this->queryAction($sql,'S');
			foreach($arr as $row)
			{
				$pdf->SetFont('Arial','',8);
