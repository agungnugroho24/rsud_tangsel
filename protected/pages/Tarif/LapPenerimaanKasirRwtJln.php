...
{   
    ...
    public function onLoad($param)
	{
		...
		{	
			...
			$sql = "SELECT id, nama FROM tbm_perusahaan_asuransi WHERE st='0' ";
	...
	public function selectionChangedKelompok()
	{
		...
		else //Penjamin Non Umum
		{
			...
			if($this->DDKelompok->SelectedValue != '01')
			{
	...
	public function itemCreated($sender,$param)
    {
		...
		if($item->ItemType==='Item' || $item->ItemType==='AlternatingItem' ||$item->ItemType==='EditItem')
        {
			...
			{					
				...
				$item->retColumn->Text = number_format(KasirPendaftaranRecord::finder()->findBySql($sql)->tarif,2,',','.');
				...
				$total = $ret + $adm + $poli + $totalObat + $totalLab + $totalRad + $totalFisio; 
