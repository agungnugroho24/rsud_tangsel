<?php
class AsetBeliRtpbu extends SimakConf
{
	private $sortExp = "id";
    private $sortDir = "ASC";
    private $offset = 0;
	...
	{	
        ...
        { 
			...
			$sql = "SELECT no_po FROM tbt_aset_beli WHERE no_po = '$noPO' AND st_rkbu_rtbu = '1' "; 
			if($this->queryAction($sql,'S'))
			{
				$this->getPage()->getClientScript()->registerEndScript
				('','unmaskContent();
					jQuery.WsGrowl.show({title: \'\', content:\'<p class="msg_error">No. Kontrak RTBU/RTPBU:<br/><b>'.$noPO.'</b><br/>sudah ada dalam database!</p>\',timeout: 3000,dialog:{
						modal: true
					}});');		
			}
			else
	...
	{		
		...
		$this->Response->redirect($this->Service->constructUrl('Asset.cetakAsetBeli',array('noPO'=>$noPO,'tgl'=>$tgl,'jnsBayar'=>$jnsBayar,'jnsBeli'=>$jnsBeli,'pbf'=>$pbf,'nmApotik'=>$nmApotik,'stRkbu'=>'1')));
