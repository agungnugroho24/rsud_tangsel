...
class AsetMasuk extends SimakConf
{ 
	public function onPreLoad($param)
	{
		...
		if(!$this->IsPostBack && !$this->IsCallBack){
			...
			/*
			Set initial stok for tbt_stok_aset
			$sql = "SELECT id,kode FROM tbt_aset_harga ORDER BY kode";	
			...
			{		
				...
				$sql = "INSERT INTO tbt_stok_aset (id_barang,
												   id_harga,
												   sumber,
												   tujuan,
												   jumlah) 
			...
			{
				$kode =  $row['kode'];
				$sql = "SELECT count(kode) AS jml, kode FROM tbt_aset_harga WHERE kode=$kode";
	...
	public function checkNoPO($noPO)
	{
		...
		{
			$sql = "SELECT no_po FROM tbt_aset_beli WHERE no_po='$noPO' AND flag='0'";
		...
		{
			$sql = "SELECT no_po FROM tbt_aset_beli_tunda WHERE no_po='$noPO' AND flag='0'";
		}
	...
	public function ambilDataObatBeli($noPO)
	{
		if($this->modeTerima->SelectedValue == '0')//penerimaan baru
		{	
			$sql = "SELECT * FROM tbt_aset_beli WHERE no_po='$noPO' AND flag='0' ";
		...
		elseif($this->modeTerima->SelectedValue == '1')//penerimaan tunda
		{
			$sql = "SELECT * FROM tbt_aset_beli_tunda WHERE no_po='$noPO' AND flag='0' ORDER BY id DESC";
		}
	...
	{		
		if (!$this->getViewState('nmTable'))
		{
			...
			//insert kode,pbf,jml dari tbt_aset_beli dengan No.PO yg ditentukan ke tblTemp
			...
			foreach($arr as $row)
			{
				...
				//$sql="SELECT hrg_ppn FROM tbt_aset_harga WHERE kode='$kode' ";
				
				$sql = "SELECT MAX(id) AS id FROM tbt_aset_harga WHERE kode='$kode'";
				$idMax = HrgAsetRecord::finder()->findBySql($sql)->id;
			
				$sql="SELECT hrg_netto FROM tbt_aset_harga WHERE kode='$kode' AND id = '$idMax'  ";
				$arr=$this->queryAction($sql,'R');
				foreach($arr as $row)
				{
					if($st_pembelian == '0') //pembelian dari PBF
	...
	public function cetakClicked($sender,$param)
    {
		...
		$newAsetMasukEmbel= new AsetMasukEmbelRecord();
		$newAsetMasukEmbel->no_po=$noPO;
		$newAsetMasukEmbel->jml_fktr=$this->jmlFktr->text;
		$newAsetMasukEmbel->no_fktr=$this->noFktr->text;
		$newAsetMasukEmbel->tgl_fktr=$tglFaktur;
		$newAsetMasukEmbel->tgl_jth_tempo=$tglJthTempo;
		$newAsetMasukEmbel->tgl_trima_brg=$tglTerima;
		$newAsetMasukEmbel->materai=$cek;			
		$newAsetMasukEmbel->ppn=$ppn;
		$newAsetMasukEmbel->pot=$pot;						
		
		$newAsetMasukEmbel->Save();
		...
		{	
			...
			$jmlSatBesar = BarangRecord::finder()->findByPk($id_barang)->jml_satuan_besar;			
			...
			{
				//insert tbt_aset_masuk
				$newAsetMasuk= new AsetMasukRecord();
				$newAsetMasuk->no_po=$noPO;
				$newAsetMasuk->no_faktur=$noFak;			
				$newAsetMasuk->tgl_faktur=$tglFaktur;
				$newAsetMasuk->tgl_jth_tempo=$tglJthTempo;						
				$newAsetMasuk->tgl_terima=$tglTerima;
				$newAsetMasuk->id_barang=$id_barang;
				$newAsetMasuk->jumlah=$jmlSatKecil;
				$newAsetMasuk->jumlah_tunda_sisa=$jmlTundaSatKecil;			
				$newAsetMasuk->discount=$disc;
				$newAsetMasuk->hrg_nett=$hrg_nett;
				$newAsetMasuk->hrg_disc=$hrg_disc;
				$newAsetMasuk->hrg_ppn=$hrg_ppn;
				$newAsetMasuk->hrg_ppn_disc=$hrg_ppn_disc;
				$newAsetMasuk->sumber='01';
				//$newAsetMasuk->no_batch=$batch;
				//$newAsetMasuk->tgl_exp=$tgl_exp;
				$newAsetMasuk->st_keuangan='0';
				$newAsetMasuk->petugas=$operator;
				$newAsetMasuk->st_tunda_sisa=$st_tunda_sisa;
				$newAsetMasuk->thn_pengadaan=$thn_pengadaan;
				$newAsetMasuk->depresiasi=$depresiasi;
				$newAsetMasuk->st_rkbu_rtbu=$st_rkbu_rtbu;
				
				$newAsetMasuk->Save();
			}
			elseif($this->modeTerima->SelectedValue == '1')
			{
				//$sql = "SELECT * FROM tbt_aset_masuk WHERE no_po='$noPO' AND id_barang = '$id_barang' AND hrg_nett = '$hrg_nett' AND tgl_exp = '$tgl_exp' ";
				//$sql = "SELECT * FROM tbt_aset_masuk WHERE no_po='$noPO' AND id_barang = '$id_barang' AND hrg_nett = '$hrg_nett' AND MONTH(tgl_exp) = '$blnExp' AND YEAR(tgl_exp) = '$thnExp' ";
				$sql = "SELECT * FROM tbt_aset_masuk WHERE no_po='$noPO' AND id_barang = '$id_barang' AND hrg_nett = '$hrg_nett' AND thn_pengadaan = '$thn_pengadaan' AND depresiasi = '$depresiasi' ";
				$arr = $this->queryAction($sql,'S');
				
				if($arr)
				{
					//update tbt_aset_masuk
					$data = AsetMasukRecord::finder()->findBySql($sql);
			...
			if($st_tunda_sisa == '1')
			{	
				$sql = "SELECT MAX(id) AS id FROM tbt_aset_masuk";
				$id_barang_masuk = AsetMasukRecord::finder()->findBySql($sql)->id;
				
				//insert tbt_aset_beli_tunda
				$data = newAsetBeliTundaRecord();
				$data->id_barang_masuk = $id_barang_masuk;
				$data->no_po = $noPO;
				$data->tgl_po =AsetBeliRecord::finder()->find('no_po=? AND kode=?',array($noPO,$id_barang))->tgl_po;
				$data->waktu =AsetBeliRecord::finder()->find('no_po=? AND kode=?',array($noPO,$id_barang))->waktu;
				$data->pbf =AsetBeliRecord::finder()->find('no_po=? AND kode=?',array($noPO,$id_barang))->pbf;
				$data->kode = $id_barang;
				$data->jumlah = $jml_terima_tunda;
				$data->jumlah_kecil = $jmlTundaSatKecil;
				$data->catatan =AsetBeliRecord::finder()->find('no_po=? AND kode=?',array($noPO,$id_barang))->catatan;
				$data->petugas=$operator;
				$data->flag = '0';
				$data->nm_apotik_luar =AsetBeliRecord::finder()->find('no_po=? AND kode=?',array($noPO,$id_barang))->nm_apotik_luar;
				$data->st_pembelian = $stBeli;
				$data->Save();
			}
				//Cek hrg_ppn di tbt_aset_harga, jika beda dengan $hrg_ppn=$row['hrg'] => INSERT hrg_ppn,hrg_netto,tgl di tbt_aset_harga
				//$sql="SELECT hrg_ppn FROM tbt_aset_harga WHERE kode='$id_barang' ";
				//ini ngambil idHarga paling baru mas
				$sql = "SELECT MAX(id) AS id FROM tbt_aset_harga WHERE kode='$id_barang'";
				$id = HrgAsetRecord::finder()->findBySql($sql)->id;
				...
				foreach($arr as $row)
				{
					if($row['hrg_netto'] <> $hrg_nett) //harga berubah
					{
						...
						//$sql = "SELECT SUM(jumlah) AS jumlah FROM tbt_stok_aset WHERE id_barang = '$id_barang' AND id_harga = '$id' AND expired = '$tgl_exp' ";
						$sql = "SELECT SUM(jumlah) AS jumlah FROM tbt_stok_aset WHERE id_barang = '$id_barang' AND id_harga = '$id' AND thn_pengadaan = '$thn_pengadaan' AND depresiasi = '$depresiasi' ";
						
						$jmlStok = StokAsetRecord::finder()->findBySql($sql)->jumlah;
						...
						*/
							...
							*/
							...
							$sql="SELECT 
								  ...
								  WHERE 
									...
									AND id_harga='$id' 
									AND tujuan = '1'
									AND thn_pengadaan = '$thn_pengadaan' AND depresiasi = '$depresiasi' ";
						...
						{
							// INSERT hrg_ppn,hrg_netto,tgl di tbt_aset_harga
							$sql="INSERT INTO 
									tbt_aset_harga 
									(kode,sumber,hrg_netto,hrg_ppn,hrg_netto_disc,hrg_ppn_disc,tgl,st_pembelian)
								VALUES
									('$id_barang','01','$hrg_nett','$hrg_ppn','0','0','$dateNow','$stBeli') ";
							$this->queryAction($sql,'C');
				...
				//jika Discount!=0, update hrg_netto_disc & hrg_ppn_disc di tbt_aset_harga
				if($disc!=0)
				{
					$sql="UPDATE 
								tbt_aset_harga 
				...
				{
					// INSERT tbt_stok_aset
					$sql="INSERT INTO 
							tbt_stok_aset 
							(id_barang,id_harga,jumlah,sumber,tujuan,thn_pengadaan,depresiasi)
						VALUES
							('$id_barang','$id','$jmlSatKecil','01','1','$thn_pengadaan','$depresiasi') ";
					$this->queryAction($sql,'C');
				}
		}
		...
		if($this->modeTerima->SelectedValue == '0')
		{
			//Update status di tbt_aset_beli 
			$sql="UPDATE tbt_aset_beli SET flag='1' WHERE no_po='$noPO' AND st_rkbu_rtbu = '$st_rkbu_rtbu' ";
		}
		elseif($this->modeTerima->SelectedValue == '1')
		{
			//Update status di tbt_aset_beli_tunda jadi
			$sql="UPDATE tbt_aset_beli_tunda SET flag='1' WHERE no_po='$noPO' AND st_rkbu_rtbu = '$st_rkbu_rtbu' ";
		}
		...
		//$this->test->Text = $tes;
		$this->Response->redirect($this->Service->constructUrl('Asset.cetakAsetMasuk1',array('noPO'=>$noPO,'tgl'=>$tglFaktur,'noFak'=>$noFak,'mat'=>$cek,'ppn'=>$ppn,'pot'=>$pot,'modeTerima'=>$this->modeTerima->SelectedValue,'nmTable'=>$nmTable,'st_rkbu_rtbu'=>$st_rkbu_rtbu)));
	}
	...
	public function prevClicked($sender,$param)
    {
		...
		/*
		//Update status di tbt_aset_beli jadi
		$sql="UPDATE tbt_aset_beli SET flag='1' WHERE no_po='$noPO' ";
		$this->queryAction($sql,'C');
			
		$this->queryAction($nmTable,'D');*/
		//$this->test->Text = $tes;
		$this->Response->redirect($this->Service->constructUrl('Asset.cetakAsetMasukEdit',array('noPO'=>$noPO,'tgl'=>$tglFaktur,'noFak'=>$noFak,'mat'=>$cek,'ppn'=>$ppn,'pot'=>$pot,'tab'=>$nmTable)));
	}	
}
?>
