...
{   
	...
	{				
		...
		{	
			...
			/*
			$this->DDJenis->DataSource=GolBarangRecord::finder()->findAll();
	...
	{	
		...
		{
			$klas = $this->DDKelompok->SelectedValue;	
			$this->DDDerivat->DataSource=DerivatBarangRecord::finder()->findAll('klas_id = ?', $klas);
			
			if(count(DerivatBarangRecord::finder()->findAll('klas_id = ?', $klas)) > 0)
			{
				$this->DDDerivat->dataBind(); 	
	...
	public function editRow($sender,$param)
    {
		...
		$this->Response->redirect($this->Service->constructUrl('Asset.MasterBarangEdit',array('pageParent'=>$this->Page->getPagePath(),'ID'=>$ID)));	
    }	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
            ...
            $sql = "SELECT 
					  tbm_barang.id,
					  IF(tbm_barang.id_jenis_barang = '1','Barang Bergerak','Barang Tidak Bergerak') AS nm_jenis,
					  tbm_barang.id_kelompok_barang,
					  tbm_barang_kelompok.nama AS nm_kelompok,
					  tbm_barang.nama,
					  tbm_barang.merk,
					  tbm_barang.spesifikasi,
					  tbm_barang.deskripsi,
					  tbm_barang.thn_pembuatan,
					  tbm_barang.thn_pengadaan
					FROM
					  tbm_barang
			...
			if($this->cariNama->Text <> '')	
			{
				...
				if($this->Advance->Checked === true)
					$sql .= "AND tbm_barang.nama LIKE '%$cariNama%' ";
				else
					$sql .= "AND tbm_barang.nama LIKE '$cariNama%' ";
			}			
			...
			{
				$merk = $this->merk->Text;	
				$sql .= "AND tbm_barang.merk LIKE '%$merk%' ";
			}		
