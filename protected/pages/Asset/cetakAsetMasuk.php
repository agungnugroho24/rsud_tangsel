<?php
class cetakAsetMasuk extends SimakConf
{
	...
	{	
		...
		$sql="SELECT pbf FROM tbt_aset_beli WHERE no_po='$noPO' GROUP BY  no_po";
		$idPbf =AsetBeliRecord::finder()->findBySql($sql)->pbf;
		$nmPbf = PbfBarangRecord::finder()->findByPk($idPbf)->nama;	
		$alamatPbf = PbfBarangRecord::finder()->findByPk($idPbf)->alamat;		
		...
		*/
		...
		foreach($arrData as $row)
		{
			...
			if($row['discount'] > 0) //jika ada => discount harga ppn discount
			{
				...
				$pdf->Cell(65,5,BarangRecord::finder()->findByPk($row['id_barang'])->nama,'1',0,'L');
		...
		$pdf->Cell(65,8,'( Direktur )',0,0,'C','',$this->Service->constructUrl('Asset.ObatBeli'));
		$pdf->Cell(65,8,'( Manajer Keuangan )',0,0,'C','',$this->Service->constructUrl('Asset.ObatBeli'));
		$pdf->Cell(60,8,'( '.$operator.' )',0,0,'C','',$this->Service->constructUrl('Asset.ObatBeli'));	
