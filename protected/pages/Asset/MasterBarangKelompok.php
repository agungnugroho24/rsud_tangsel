<?php
class MasterBarangKelompok extends SimakConf
{   
    ...
    public function deleteButtonClicked($sender,$param)
    {
        if ($this->User->IsAdmin)
		{
			...
			if($ID <> 8)
				BarangKelompokRecord::finder()->deleteByPk($ID);	
	...
	public function editRow($sender,$param)
    {
		...
		$this->Response->redirect($this->Service->constructUrl('Asset.MasterBarangKelompokEdit',array('pageParent'=>$this->Page->getPagePath(),'ID'=>$ID)));	
    }	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
            ...
            $sql = "SELECT 
					  tbm_barang_kelompok.id,
					  tbm_barang_kelompok.nama,
					  IF(tbm_barang_kelompok.id_jenis_barang = '1','Barang Bergerak','Barang Tidak Bergerak') AS nm_jenis
					FROM
					  tbm_barang_kelompok						
					WHERE 
						tbm_barang_kelompok.id<>'' ";
			...
			if($this->cariNama->Text <> '')	
			{
				...
				if($this->Advance->Checked === true)
					$sql .= "AND tbm_barang_kelompok.nama LIKE '%$cariNama%' ";
				else
					$sql .= "AND tbm_barang_kelompok.nama LIKE '$cariNama%' ";
			}			
			...
			if($this->DDJenis->SelectedValue != '')	
			{
				...
				$sql .= "AND tbm_barang_kelompok.id_jenis_barang = '$DDJenis' ";
			}
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Asset.MasterBarangKelompokBaru'));		
	}
