<?php
class cetakBbkAset extends SimakConf
{
	...
	{	
		...
		$tujuan=DestinasiAsetRecord::finder()->findByPk($this->Request['tujuan'])->nama;
		...
		*/
		...
		$pdf->Cell(0,5,'SURAT BUKTI ASET KELUAR','0',0,'C','',$this->Service->constructUrl('Asset.bbkAset') . '&purge=1&nmTable=' . $nmTable);
		$pdf->Ln(5);		
		$pdf->SetFont('Arial','',9);
		...
		{
			$no++;
			$nmObat=BarangRecord::finder()->findByPk($row['kode'])->nama;
			$sumMaster=DestinasiAsetRecord::finder()->findByPk($row['sumber'])->nama;
			$sumSekunder=DestinasiAsetRecord::finder()->findByPk($row['tujuan'])->nama;
		...
		$pdf->Ln(10);
		$pdf->Cell(80,8,'( '.$petugas.' )',0,0,'C','',$this->Service->constructUrl('Asset.bbkAset') . '&purge=1&nmTable=' . $nmTable);		
		$pdf->Cell(30,8,'',0,0,'C');		
		$pdf->Cell(80,8,'( '.$penerima.' )',0,0,'C');
		...
		//$pdf->Cell(53,8,'('.$operator.')',0,0,'C','',$this->Service->constructUrl('Asset.bbkAset') . '&purge=1&nmTable=' . $nmTable);		
		//$pdf->Ln(5);
		$pdf->Output();
