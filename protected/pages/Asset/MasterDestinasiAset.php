<?php
class MasterDestinasiAset extends SimakConf
{   
	...
	{				
		...
		{	
			...
			$this->DDKelompok->DataSource = DestinasiAsetRecord::finder()->findAll();
	...
	{	
		if(!$this->DDJenis->SelectedValue=='')
		{
			...
			$sql = "SELECT id,nama FROM tbm_destinasi_aset WHERE id_jenis_barang = '$gol' ORDER BY nama )";
			$this->DDKelompok->DataSource = DestinasiAsetRecord::finder()->findAllBySql($sql);
			$this->DDKelompok->dataBind(); 	
    ...
    public function deleteButtonClicked($sender,$param)
    {
        if ($this->User->IsAdmin)
		{
			...
			DestinasiAsetRecord::finder()->deleteByPk($ID);	
	...
	public function editRow($sender,$param)
    {
		...
		$this->Response->redirect($this->Service->constructUrl('Asset.MasterDestinasiAsetEdit',array('pageParent'=>$this->Page->getPagePath(),'ID'=>$ID)));	
    }	
    ...
    private function bindGrid()
    {
        ...
        if (!$session->contains("SomeData")) 
        {
            ...
            $sql = "SELECT 
					  tbm_destinasi_aset.id,
					  tbm_destinasi_aset.nama
					FROM
					  tbm_destinasi_aset						
					WHERE 
						tbm_destinasi_aset.id<>'' ";
			...
			if($this->cariNama->Text <> '')	
			{
				...
				if($this->Advance->Checked === true)
					$sql .= "AND tbm_destinasi_aset.nama LIKE '%$cariNama%' ";
				else
					$sql .= "AND tbm_destinasi_aset.nama LIKE '$cariNama%' ";
			}			
			...
			if($this->DDJenis->SelectedValue != '')	
			{
				...
				$sql .= "AND tbm_destinasi_aset.id_jenis_barang = '$DDJenis' ";
			}
	...
	{		
		$this->Response->redirect($this->Service->constructUrl('Asset.MasterDestinasiAsetBaru'));		
	}
