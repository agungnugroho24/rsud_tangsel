<?php
class bbkAset extends SimakConf
{   
	...
	{				
		...
		{					
			...
			$sqlNmObat="SELECT tbm_barang.id,tbm_barang.nama 
					FROM tbt_stok_aset 
					INNER JOIN tbm_barang ON (tbt_stok_aset.id_barang = tbm_barang.id)
					WHERE tbt_stok_aset.sumber = '01' AND tbt_stok_aset.tujuan = '1' AND tbt_stok_aset.jumlah > '0'
					ORDER BY tbm_barang.nama";
		...
		{			
			...
			{		
				$sql = "SELECT 
					...
					WHERE COLUMNS.TABLE_SCHEMA = 'simak_belitung'
					AND COLUMNS.TABLE_NAME = '$nmTable' ";
			...
			$this->Response->redirect($this->Service->constructUrl('Asset.bbkAset',array('goto'=>'1')));
		}	
	...
	protected function prosesClicked()
	{
		if($this->IsValid)  // when all validations succeed
        {
			...
			$this->DDSumMaster->DataSource=DestinasiAsetRecord::finder()->find('id <> ?',$tmp);
			$this->DDSumMaster->dataBind();*/
			$sql="SELECT id, nama FROM tbm_destinasi_aset WHERE id <> '$tmp' ";
	...
	{
		if($this->getViewState('sqlAset'))
		{
			$sql = $this->getViewState('sqlAset');
			$sql .= " AND tbm_barang.nama LIKE '$token%' GROUP BY tbt_stok_aset.id_barang
					ORDER BY
					  tbm_barang.nama "; 
		...
		if($this->DDSumMaster->SelectedValue != '')
		{
			...
			$sql = "SELECT
					...
					FROM
					  ...
					  INNER JOIN tbm_barang ON (tbt_stok_aset.id_barang = tbm_barang.id)
					WHERE
					  tbt_stok_aset.tujuan = '$tmp' 
					  AND jumlah > 0 ";
			
			$sqlAset = $sql;
					  
			$sql .= " GROUP BY tbt_stok_aset.id_barang
					ORDER BY
					  tbm_barang.nama";
			...
			if(count($this->queryAction($sql,'S')) > 0) //jika data obat ada untuk sumber obat tersedia
			{
				...
				$this->DDnmObat->dataBind();
				$this->errMsg->Text = "";
				$this->setViewState('sqlAset',$sqlAset);
			...
			{	
				...
				$this->DDnmObat->Enabled = false;
				$this->nama->Enabled=false;
				$this->errMsg->Text = "Maaf, belum ada aset untuk sumber aset yg dipilih";
				$this->clearViewState('sqlAset');
				$this->Page->CallbackClient->focus($this->DDSumMaster);
			}	
		}
		else
	...
	protected function DDnmObatChanged()
	{
		...
		else
		{
			...
			//$idBarang=$this->DDnmObat->SelectedValue;
			$this->jmlAmbil->Text='';
			...
			if($this->modeDistribusi->SelectedValue == '1') //mode distribusi barang rusak
			{
				if($this->thnPengadaan->Text != '')
					$thnPengadaan = $this->thnPengadaan->Text;
				else
					$thnPengadaan = '';
				...
				$sql = "SELECT
					...
					FROM
					  tbt_stok_aset
					WHERE
					  tbt_stok_aset.tujuan = '$idTujuan'
					  AND tbt_stok_aset.id_barang = '$idBarang' 
					  AND tbt_stok_aset.thn_pengadaan = '$thnPengadaan' 
					GROUP BY tbt_stok_aset.thn_pengadaan";
			}
			else
			{
				...
				$sql = "SELECT
					...
					WHERE
					  tbt_stok_aset.tujuan = '$idTujuan'
					  AND tbt_stok_aset.id_barang = '$idBarang' 
					GROUP BY tbt_stok_aset.id_barang";
			}
			
			
			//$this->jmlStokCtrl->Display='Dynamic';
			//$this->showSql->text=$sql;
			if(StokAsetRecord::finder()->findBySql($sql))
			{
				...
				$idDestinasi = DestinasiAsetRecord::finder()->findByPk($idTujuanDistribusi)->id;
				//$fieldMax = 'max_'.$idDestinasi;
				
				//$maxLoket = BarangRecord::finder()->findByPk($idBarang)->$fieldMax;
				...
				//{					
					...
					$jmlStokAset = StokAsetRecord::finder()->findBySql($sql)->jumlah;
					$this->jmlAmbil->Text = $jmlStokAset;
					//$this->jmlAmbil->Text = $maxLoket - $jmlStokAset;
					//$this->setViewState('maxLoket',$maxLoket - $jmlStokAset);
				//}
				//else
				//{
			...
			{		
				if($this->thnPengadaan->Text != '')
				{
					$this->getPage()->getClientScript()->registerEndScript
						('','jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>'.$this->ambilTxt($this->DDnmObat).' dengan tahun pengadaan : <br/>'.$this->thnPengadaan->Text.' tidak ditemukan !</p>\',timeout: 3000,dialog:{modal: true}});');
				}
						
				$this->thnPengadaan->Text = '';
				$this->Page->CallbackClient->focus($this->thnPengadaan);	
				$this->jmlStok->Text='0';
				////$this->jmlStokCtrl->Display='Dynamic';
    ...
    {    
		...
		$tmp=$this->DDSumMaster->SelectedValue;
		$idBarang=$this->DDnmObat->SelectedValue;
		$sql="SELECT * FROM tbt_stok_aset WHERE id_barang='$idBarang' AND sumber='01' AND tujuan='$tmp' ";
		$jmlGudang=StokAsetRecord::finder()->findBySql($sql);   
	...
	public function prosesAmbilClicked()
    {
		...
		if($jmlTotalStok >= $jmlAmbil) //jika jumlah total stok mencukupi
		{
			if($this->modeDistribusi->SelectedValue == '0') //mode distribusi normal
			{
				...
				{
					$sql = "SELECT 
								id,jumlah, id_harga,expired,thn_pengadaan,depresiasi
							FROM 
								tbt_stok_aset 
							WHERE 
								id_barang='$idBarang'
						...
						if($row['jumlah'] <> 0)
						{
							if($row['jumlah'] > $this->getViewState('jmlKekurangan'))
							{
								if($this->getViewState('jmlKekurangan') > 0)
								{
									...
									$this->makeTmpTbl($id_tbt_stok_aset,$id_harga,$jmlAmbil,$expired,$thn_pengadaan,$depresiasi);
								}
							}
							else
							{
			...
			elseif($this->modeDistribusi->SelectedValue == '1') //mode distribusi rusak
			{
				...
				$sql = "SELECT 
						...
						WHERE 
							id_barang='$idBarang'
							AND tujuan ='$idTujuan'
							AND thn_pengadaan ='$thnPengadaan'
						ORDER BY id_harga DESC";
	...
	public function ambilClicked()
    {
		...
		{
			$nmTable = $this->getViewState('nmTable');
			$sql="SELECT SUM(jumlah) AS jumlah FROM $nmTable WHERE kode = '$idBarang' GROUP BY kode ORDER BY id ";
	...
	protected function makeTmpTbl($id_tbt_stok_aset,$id_harga,$jmlAmbil,$expired,$thn_pengadaan,$depresiasi)
	{
		if($this->IsValid)  // when all validations succeed
        {
			...
			if (!$this->getViewState('nmTable'))
			{
				...
				$sql="CREATE TABLE $nmTable (id INT (2) auto_increment, 									 
											 ...
											 id_tbt_stok_aset VARCHAR(20) NOT NULL,	
											 expired DATE DEFAULT '0000-00-00',	
											 thn_pengadaan CHAR(4) DEFAULT NULL,
											 depresiasi FLOAT DEFAULT 0,
											 PRIMARY KEY (id)) ENGINE = MEMORY";
				
				$this->queryAction($sql,'C');//Create new tabel bro...					
				
				$sql="INSERT INTO 
						$nmTable(kode,sumber,tujuan,jumlah,id_harga,id_tbt_stok_aset,expired,thn_pengadaan,depresiasi) 
					VALUES ('$idBarang','$sumber','$tujuan','$jmlAmbil','$id_harga','$id_tbt_stok_aset','$expired','$thn_pengadaan','$depresiasi')";
	...
	{	
		...
		if($this->modeDistribusi->SelectedValue == '1') //mode distribusi barang rusak
			$nobbk=$this->numCounter(BbkBarangRusakRecord::finder());
		else
			$nobbk=$this->numCounter(BbkBarangRecord::finder());
		...
		foreach($arrData as $row)
		{	
			$id_tbt_stok_aset=$row['id_tbt_stok_aset'];	
			$idBarang=$row['kode'];
			$sumber=$row['sumber'];
			$id_harga=$row['id_harga'];
			$jumlah=$row['jumlah'];
			...
			{
				//$sql="SELECT * FROM tbt_stok_aset WHERE id_barang='$idBarang' AND sumber='01' AND tujuan='$tujuan' AND id_harga='$id_harga'";
				
				$sql="SELECT * FROM tbt_stok_aset WHERE id_barang='$idBarang' AND tujuan='$tujuan' AND id_harga='$id_harga' AND thn_pengadaan='$thn_pengadaan' AND depresiasi='$depresiasi' ";
				
				$data=StokAsetRecord::finder()->findBySql($sql);
				if($data===NULL)
				{
					//data baru untuk tbt_stok_aset
					$dataStokAset=new StokAsetRecord;
					$dataStokAset->id_barang=$row['kode'];
					//$dataStokAset->sumber=$row['sumber'];
					$dataStokAset->sumber='01'; //sumber 01 adalah sumber pendanaan dari APBD
					$dataStokAset->tujuan=$tujuan;
					$dataStokAset->jumlah=$row['jumlah'];
					$dataStokAset->id_harga=$row['id_harga'];
					$dataStokAset->expired=$row['expired'];
					$dataStokAset->thn_pengadaan=$row['thn_pengadaan'];
					$dataStokAset->depresiasi=$row['depresiasi'];
							
					//save ke tbt_stok_aset
					$dataStokAset->save();
				}
				else
				{
					//update jumlah di tbt_stok_aset
					$data->jumlah = $jumlah + $data->getColumnValue('jumlah');
					$data->save();
				}
				
				//data baru untuk tbt_bbk_barang
				$dataBbkObat=new BbkBarangRecord;
				//$dataBbkObat->no_bbk=$this->numCounter('tbt_bbk_barang',BbkBarangRecord::finder(),'50');
				$dataBbkObat->no_bbk=$nobbk;
				$dataBbkObat->kode_barang=$idBarang;
			...
			elseif($this->modeDistribusi->SelectedValue >= '1') //mode distribusi barang rusak
			{
				...
				$dataBbkObat=new BbkBarangRusakRecord;
				$dataBbkObat->no_bbk=$nobbk;
				$dataBbkObat->kode_barang=$idBarang;
				$dataBbkObat->jumlah=$row['jumlah'];
				$dataBbkObat->tgl=date("Y-m-d");
				...
				$dataBbkObat->hrg_ppn = HrgAsetRecord::finder()->findByPk($id_harga)->hrg_ppn;
				$dataBbkObat->expired = $expired;
				$dataBbkObat->thn_pengadaan = $thn_pengadaan;
				$dataBbkObat->depresiasi = $depresiasi;
				$dataBbkObat->st_mutasi = $this->modeDistribusi->SelectedValue;
			...
			//$sql="SELECT * FROM tbt_stok_aset WHERE id_barang='$idBarang' AND sumber='01' AND tujuan='$sumMas' AND id_harga='$id_harga'";
			
			$sql="SELECT * FROM tbt_stok_aset WHERE id = '$id_tbt_stok_aset' AND  id_barang='$idBarang' AND tujuan='$sumMas' AND id_harga='$id_harga' AND thn_pengadaan='$thn_pengadaan' AND depresiasi='$depresiasi' ";
		...
		//$this->clearViewState('nmTable');//Clear the view state
		//$this->Response->redirect($this->Service->constructUrl('Asset.bbkAset'));
		
		$this->Response->redirect($this->Service->constructUrl('Asset.cetakBbkAset',array('nobbk'=>$nobbk,'petugas'=>$this->ambilTxt($this->DDPetugas),'tujuan'=>$tujuan,'penerima'=>$this->ambilTxt($this->DDPenerima),'table'=>$this->getViewState('nmTable'))));
	}
	...
	{	
		...
		//$this->Response->redirect($this->Service->constructUrl('Asset.bbkAset') . '&purge=1&nmTable=' . $nmTable);
