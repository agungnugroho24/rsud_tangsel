<?php
class TdkParamedis extends SimakConf
{   
...
$this->umur->Text = $umur['years'];
    ...
    {	
		...
		foreach($arrData as $row)
		{
			...
			$newInapTdkParamedis= new InapTdkMedisRecord();
			$newInapTdkParamedis->no_trans = $this->getViewState('notrans');
			$newInapTdkParamedis->cm=$this->formatCm($this->cm->Text);
			$newInapTdkParamedis->medis = $idParamedis;
			$newInapTdkParamedis->id_tdk = $idTdk;
			$newInapTdkParamedis->tgl = $tgl;
			$newInapTdkParamedis->wkt = $wkt;
			$newInapTdkParamedis->operator = $this->User->IsUserNip;
			$newInapTdkParamedis->catatan = $row['catatan'];
			$newInapTdkParamedis->tarif = $tarifStandar + ($tarifStandar * $persetaseKelas / 100);
			$newInapTdkParamedis->Save();	
		...
		$this->errMsg->Text = ' 
			<script type="text/javascript">
					unmaskContent();
					jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>Rekam Tindakan Paramedis Sukses.</p>\',timeout: 200000,dialog:{
