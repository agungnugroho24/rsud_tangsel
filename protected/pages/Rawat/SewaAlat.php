<?php
class SewaAlat extends SimakConf
{   
	...
	{				
		...
		{					
			...
			$sql="SELECT * FROM tbm_sewa_alat ORDER BY nama";
			$arrData=$this->queryAction($sql,'S');//Select row in tabel bro...
			$this->DDdokter->DataSource=$arrData;	
			$this->DDdokter->dataBind();
			
			$sql="SELECT id, nama FROM tbm_sewa_alat ORDER BY nama";
			$data=$this->queryAction($sql,'S');//Select row in tabel bro...
			$this->DDSewaAlat->DataSource=$data;	
			$this->DDSewaAlat->dataBind();
		}	
...
$this->umur->Text = $umur['years'];
			...
			$this->getPage()->getClientScript()->registerEndScript('','document.all.'.$this->DDSewaAlat->getClientID().'.focus();');	
		}
		else
		{			
	...
	public function tambahClicked()
    {
		if($this->Page->IsValid)
        {
			...
			$idTdk = $this->DDSewaAlat->SelectedValue;
			...
			if(InapSewaAlatRecord::finder()->find('no_trans=? AND id_tdk=? AND tgl=?',$noTransRwtInap,$idTdk,date('Y-m-d'))) //konfirmasi muncul
			{
				$this->lapTrans();
			}
			else
	...
	public function makeTmpTbl()
    {
			...
			$id_tdk = $this->DDSewaAlat->SelectedValue;
			$nama_tdk = $this->ambilTxt($this->DDSewaAlat);			
			$catatan = $this->catatan->Text;
			$tarif = SewaAlatRecord::finder()->findByPk($id_tdk)->tarif;
			...
			if(count($arrData) > 0)//data sudah ada
			{
				...
				<script type="text/javascript">
					jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>Sewal Alat '.$nama_tdk.' sudah ditambahkan sebelumnya !</p>\',timeout: 3000,dialog:{modal: true}});
				</script>';
			...
			$this->DDSewaAlat->SelectedValue = 'empty';
			$this->catatan->Text = '';
    ...
    {	
		...
		$this->nmTdk->Text = $this->ambilTxt($this->DDSewaAlat);
		...
		$sql="SELECT 
			  tbt_inap_sewa_alat.wkt,
			  tbm_sewa_alat.nama AS nm_tdk
			FROM
			  tbt_inap_sewa_alat
			  INNER JOIN tbm_sewa_alat ON (tbt_inap_sewa_alat.id_tdk = tbm_sewa_alat.id)
			WHERE
			  tbt_inap_sewa_alat.no_trans = '$noTransRwtInap'
			  AND tbt_inap_sewa_alat.id_tdk = '$idTdk' 
			  AND tbt_inap_sewa_alat.tgl = '$tgl'
			ORDER BY			  
			  tbt_inap_sewa_alat.wkt";
    ...
    {	
		...
		foreach($arrData as $row)
		{
			...
			$newInapSewaAlat= new InapSewaAlatRecord();
			$newInapSewaAlat->no_trans = $this->getViewState('notrans');
			$newInapSewaAlat->cm=$this->formatCm($this->cm->Text);
			$newInapSewaAlat->dokter = $idDokter;
			$newInapSewaAlat->id_tdk = $idTdk;
			$newInapSewaAlat->tgl = $tgl;
			$newInapSewaAlat->wkt = $wkt;
			$newInapSewaAlat->operator = $this->User->IsUserNip;
			$newInapSewaAlat->catatan = $row['catatan'];
			$newInapSewaAlat->tarif = $tarifStandar + ($tarifStandar * $persetaseKelas / 100);
			$newInapSewaAlat->Save();	
		...
		$this->errMsg->Text = ' 
			<script type="text/javascript">
					unmaskContent();
					jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>Rekam Sewal Alat Sukses.</p>\',timeout: 200000,dialog:{
    ...
    {
		$this->Response->redirect($this->Service->constructUrl('Rawat.SewaAlat'));
	}
