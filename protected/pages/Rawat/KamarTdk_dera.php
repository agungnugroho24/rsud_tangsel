...
{   
	...
	public function clearDD()
	{
		...
		$sql="SELECT * FROM tbd_pegawai WHERE kelompok='7'";
		$arrData=$this->queryAction($sql,'S');//Select row in tabel bro...
	...
	public function ChangedTdk()
	{
		if($this->DDtindakan->SelectedValue!='')
		{
			...
			else
			{
				...
				if($b == '04')
				{					
					$this->pelakuResusitasiCtrl->Visible=true;
					$this->bidanCtrl->Visible=false;
					$this->bidanValidator->Enabled=true;
					$this->resusitasiCtrl->Visible=false;
					$this->modeResusitasi->SelectedIndex = -1 ;
					
				}
				else
				{
					$this->pelakuResusitasiCtrl->Visible=false;
					$this->bidanCtrl->Visible=true;
					$this->bidanValidator->Enabled=false;
					$this->resusitasiCtrl->Visible=false;
				}
			}
		}
		...
		$this->clearDD();
		$this->clearCB();
		$this->CBcito->Checked = false;
	}
	
	public function modeResusitasiChanged()
    {
		$modeResusitasi = $this->collectSelectionResult($this->modeResusitasi);
		if($modeResusitasi == '0')
		{
			$this->resusitasiCtrl->Visible=true;
			$this->bidanCtrl->Visible=false;
		}
		elseif($modeResusitasi == '1')
		{
			$this->resusitasiCtrl->Visible=false;
			$this->bidanCtrl->Visible=true;
			$this->bidanValidator->Enabled=true;			
	...
	public function clearCB()
    {
		...
		$this->CBresusitasi->Checked=false;
    ...
    {	
		...
		$st=OperasiNamaRecord::finder()->findByPK($this->DDtindakan->SelectedValue)->st;
		if($st==0 && $this->CBcito->Checked==true) //jika non persalinan dan cito di pilih => tarif ditambah 25%
		{
			$tarif = $tarif + ($tarif * 25 / 100);
		}
		...
		$st=OperasiNamaRecord::finder()->findByPK($this->DDtindakan->SelectedValue)->st;
			...
			else
			{
				...
				if($this->DDtindakan->SelectedValue == '04')//jika operasi caessar maka ada biaya resusitasi
				{
					$modeResusitasi = $this->collectSelectionResult($this->modeResusitasi);
					if($modeResusitasi == '0') //pelaku resusitasi = dokter
					{
						switch ($id_kelas) 
						{
							case 1:
						...
						{
							$newOperasi->petugas_resusitasi=$this->DDresusitasi->SelectedValue;
							$newOperasi->tarif_petugas_resusitasi=$tarifResus;
						}
					}
					elseif($modeResusitasi == '1') //pelaku resusitasi = bidan
					{
						switch ($id_kelas) 
						{
							case 1:
						...
						{
							$newOperasi->petugas_resusitasi=$this->DDbidan->SelectedValue;
							$newOperasi->tarif_petugas_resusitasi=$tarifResus;
						}		
					}
				}
				else //operasi persalinan bukan caessar
				{
					if($this->DDbidan->SelectedValue!='')
					{
		...
		else
		{
			...
			$tipeRujukan = $this->getViewState('tipeRujukan');
			$tarifSewaOK = $this->getViewState('tarifKamarOK');
			
			if($oprST == '0')//Non Kebidanan atau Persalinan
			{	
				if($tipeRujukan == '1') //rujukan karyawan
				{
					$tarifSewaOK += 200000;
				}
				elseif($tipeRujukan == '2') //rujukan luar/medis
				{
					$tarifSewaOK += 300000;
				}
			}
		...
		*/	
		...
		else
		{
			...
			//insert ke tbt_inap_operasi_billing_luar jika ada salah satu dokter/paramedis/bidan luar
			$this->billingLuarSave();
		}
	...
	public function billingLuarSave()
    {	
		$id_kelas=RwtInapRecord::finder()->find('cm = ?',$this->cm->Text)->kelas;	
		$id_opr=$this->DDtindakan->SelectedValue;			
		$nipTmp=$this->User->IsUserNip;
		...
		$newOperasi= new OperasiBillingLuarRecord();
		...
		else
		{
			...
			if($this->DDtindakan->SelectedValue == '04')//jika operasi caessar maka ada biaya resusitasi
			{
				...
				if($modeResusitasi == '0') //pelaku resusitasi = dokter
				{
					...
					if($this->CBresusitasi->Checked==true) //Dokter resusitasi luar dipilih
					{
						if($this->DDresusitasi->SelectedValue!='')
						{
							$newOperasi->petugas_resusitasi=$this->DDresusitasi->SelectedValue;
		...
		if($this->getViewState('kategST') == "0"){ //jika non rujukan
			$tarifSewaOK = $this->getViewState('tarifKamarOK');
		}
		else //jika rujukan
		{
