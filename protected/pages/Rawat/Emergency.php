<?php
class Emergency extends SimakConf
{   
	...
	{				
		...
		{					
			...
			$sql="SELECT id, nama FROM tbm_emergency ORDER BY nama";
...
$this->umur->Text = $umur['years'];
	...
	public function tambahClicked()
    {
		if($this->Page->IsValid)
        {
			...
			if(InapEmergencyRecord::finder()->find('no_trans=? AND dokter=? AND id_tdk=? AND tgl=?',$noTransRwtInap,$idDokter,$idTdk,date('Y-m-d'))) //konfirmasi muncul
			{
				$this->lapTrans();
			}
			else
	...
	public function makeTmpTbl()
    {
			...
			$tarif = EmergencyRecord::finder()->findByPk($id_tdk)->tarif;
    ...
    {	
		...
		$sql="SELECT 
			  tbt_inap_emergency.wkt,
			  tbd_pegawai.nama,
			  tbm_emergency.nama AS nm_tdk
			FROM
			  tbt_inap_emergency
			  INNER JOIN tbd_pegawai ON (tbt_inap_emergency.dokter = tbd_pegawai.id)
			  INNER JOIN tbm_emergency ON (tbt_inap_emergency.id_tdk = tbm_emergency.id)
			WHERE
			  tbt_inap_emergency.no_trans = '$noTransRwtInap' 
			  AND tbt_inap_emergency.dokter = '$idDokter' 
			  AND tbt_inap_emergency.id_tdk = '$idTdk' 
			  AND tbt_inap_emergency.tgl = '$tgl'
			ORDER BY			  
			  tbt_inap_emergency.wkt";
    ...
    {	
		...
		foreach($arrData as $row)
		{
			...
			$newInapEmergency= new InapEmergencyRecord();
			$newInapEmergency->no_trans = $this->getViewState('notrans');
			$newInapEmergency->cm=$this->formatCm($this->cm->Text);
			$newInapEmergency->dokter = $idDokter;
			$newInapEmergency->id_tdk = $idTdk;
			$newInapEmergency->tgl = $tgl;
			$newInapEmergency->wkt = $wkt;
			$newInapEmergency->operator = $this->User->IsUserNip;
			$newInapEmergency->catatan = $row['catatan'];
			$newInapEmergency->tarif = $tarifStandar + ($tarifStandar * $persetaseKelas / 100);
			$newInapEmergency->Save();	
		...
		$this->errMsg->Text = ' 
			<script type="text/javascript">
					unmaskContent();
					jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>Rekam Tindakan Emergency Sukses.</p>\',timeout: 200000,dialog:{
    ...
    {
		$this->Response->redirect($this->Service->constructUrl('Rawat.Emergency'));
	}
