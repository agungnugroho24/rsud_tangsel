<?php
class BiayaLainLain extends SimakConf
{   
	...
	{				
		...
		{					
			...
			$sql="SELECT * FROM tbm_biaya_lain ORDER BY nama";
			$arrData=$this->queryAction($sql,'S');//Select row in tabel bro...
			$this->DDdokter->DataSource=$arrData;	
			$this->DDdokter->dataBind();
			
			$sql="SELECT id, nama FROM tbm_biaya_lain ORDER BY nama";
...
$this->umur->Text = $umur['years'];
    ...
    {	
		...
		foreach($arrData as $row)
		{
			...
			$newInapOksigenPas= new InapBiayaLainRecord();
		...
		$this->errMsg->Text = ' 
			<script type="text/javascript">
					unmaskContent();
					jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>Rekam Biaya Lain-Lain Sukses.</p>\',timeout: 200000,dialog:{
    ...
    {
		$this->Response->redirect($this->Service->constructUrl('Rawat.BiayaLainLain'));
	}
