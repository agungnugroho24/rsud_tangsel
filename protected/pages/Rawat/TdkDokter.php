<?php
class TdkDokter extends SimakConf
{   
...
$this->umur->Text = $umur['years'];
    ...
    {	
		...
		foreach($arrData as $row)
		{
			...
			$newInapTdkDokter->cm=$this->formatCm($this->cm->Text);
			$newInapTdkDokter->dokter = $idDokter;
			$newInapTdkDokter->id_tdk = $idTdk;
			$newInapTdkDokter->tgl = $tgl;
			$newInapTdkDokter->wkt = $wkt;
			$newInapTdkDokter->operator = $this->User->IsUserNip;
			$newInapTdkDokter->catatan = $row['catatan'];
			$newInapTdkDokter->tarif = $tarifStandar + ($tarifStandar * $persetaseKelas / 100);
		...
		$this->errMsg->Text = ' 
			<script type="text/javascript">
					unmaskContent();
					jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>Rekam Tindakan Dokter Sukses.</p>\',timeout: 200000,dialog:{
