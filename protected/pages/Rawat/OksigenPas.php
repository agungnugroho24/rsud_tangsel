<?php
class OksigenPas extends SimakConf
{   
	...
	{				
		...
		{					
			...
			$sql="SELECT * FROM tbm_inap_tarif_oksigen ORDER BY nama";
			$arrData=$this->queryAction($sql,'S');//Select row in tabel bro...
			$this->DDdokter->DataSource=$arrData;	
			$this->DDdokter->dataBind();
			
			$sql="SELECT id, nama FROM tbm_inap_tarif_oksigen ORDER BY nama";
...
$this->umur->Text = $umur['years'];
	...
	public function tambahClicked()
    {
		if($this->Page->IsValid)
        {
			...
			if(InapOksigenRecord::finder()->find('no_trans=? AND id_tdk=? AND tgl=?',$noTransRwtInap,$idTdk,date('Y-m-d'))) //konfirmasi muncul
			{
				$this->lapTrans();
			}
			else
	...
	public function makeTmpTbl()
    {
			...
			$tarif = InapTarifOksigenRecord::finder()->findByPk($id_tdk)->tarif;
			...
			if(count($arrData) > 0)//data sudah ada
			{
				...
				<script type="text/javascript">
					jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>Pemakaian '.$nama_tdk.' sudah ditambahkan sebelumnya !</p>\',timeout: 3000,dialog:{modal: true}});
				</script>';
    ...
    {	
		...
		$sql="SELECT 
			  tbt_inap_oksigen.wkt,
			  tbm_inap_tarif_oksigen.nama AS nm_tdk
			FROM
			  tbt_inap_oksigen
			  INNER JOIN tbm_inap_tarif_oksigen ON (tbt_inap_oksigen.id_tdk = tbm_inap_tarif_oksigen.id)
			WHERE
			  tbt_inap_oksigen.no_trans = '$noTransRwtInap'
			  AND tbt_inap_oksigen.id_tdk = '$idTdk' 
			  AND tbt_inap_oksigen.tgl = '$tgl'
			ORDER BY			  
			  tbt_inap_oksigen.wkt";
    ...
    {	
		...
		foreach($arrData as $row)
		{
			...
			$newInapOksigenPas= new InapOksigenRecord();
			$newInapOksigenPas->no_trans = $this->getViewState('notrans');
			$newInapOksigenPas->cm=$this->formatCm($this->cm->Text);
			//$newInapOksigenPas->dokter = $idDokter;
			$newInapOksigenPas->id_tdk = $idTdk;
			$newInapOksigenPas->tgl = $tgl;
			$newInapOksigenPas->wkt = $wkt;
			$newInapOksigenPas->operator = $this->User->IsUserNip;
			$newInapOksigenPas->catatan = $row['catatan'];
			$newInapOksigenPas->tarif = $tarifStandar + ($tarifStandar * $persetaseKelas / 100);
			$newInapOksigenPas->Save();	
		...
		$this->errMsg->Text = ' 
			<script type="text/javascript">
					unmaskContent();
					jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>Rekam Pemakaian Oksigen Sukses.</p>\',timeout: 200000,dialog:{
    ...
    {
		$this->Response->redirect($this->Service->constructUrl('Rawat.OksigenPas'));
	}
