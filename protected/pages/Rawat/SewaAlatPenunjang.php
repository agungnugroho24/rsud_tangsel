<?php
class SewaAlatPenunjang extends SimakConf
{   
	...
	{				
		...
		{					
			...
			$sql="SELECT * FROM tbm_sewa_alat_penunjang ORDER BY nama";
			$arrData=$this->queryAction($sql,'S');//Select row in tabel bro...
			$this->DDdokter->DataSource=$arrData;	
			$this->DDdokter->dataBind();
			
			$sql="SELECT id, nama FROM tbm_sewa_alat_penunjang ORDER BY nama";
			$data=$this->queryAction($sql,'S');//Select row in tabel bro...
			$this->DDSewaAlatPenunjang->DataSource=$data;	
			$this->DDSewaAlatPenunjang->dataBind();
		}	
...
$this->umur->Text = $umur['years'];
			...
			$this->getPage()->getClientScript()->registerEndScript('','document.all.'.$this->DDSewaAlatPenunjang->getClientID().'.focus();');	
		}
		else
		{			
	...
	public function tambahClicked()
    {
		if($this->Page->IsValid)
        {
			...
			$idTdk = $this->DDSewaAlatPenunjang->SelectedValue;
			...
			if(InapSewaAlatPenunjangRecord::finder()->find('no_trans=? AND id_tdk=? AND tgl=?',$noTransRwtInap,$idTdk,date('Y-m-d'))) //konfirmasi muncul
			{
				$this->lapTrans();
			}
			else
	...
	public function makeTmpTbl()
    {
			...
			$id_tdk = $this->DDSewaAlatPenunjang->SelectedValue;
			$nama_tdk = $this->ambilTxt($this->DDSewaAlatPenunjang);			
			$catatan = $this->catatan->Text;
			$tarif = SewaAlatPenunjangRecord::finder()->findByPk($id_tdk)->tarif;
			...
			if(count($arrData) > 0)//data sudah ada
			{
				...
				<script type="text/javascript">
					jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>Sewal Alat Penunjang '.$nama_tdk.' sudah ditambahkan sebelumnya !</p>\',timeout: 3000,dialog:{modal: true}});
				</script>';
			...
			$this->DDSewaAlatPenunjang->SelectedValue = 'empty';
			$this->catatan->Text = '';
    ...
    {	
		...
		$this->nmTdk->Text = $this->ambilTxt($this->DDSewaAlatPenunjang);
		...
		$sql="SELECT 
			  tbt_inap_sewa_alat_penunjang.wkt,
			  tbm_sewa_alat_penunjang.nama AS nm_tdk
			FROM
			  tbt_inap_sewa_alat_penunjang
			  INNER JOIN tbm_sewa_alat_penunjang ON (tbt_inap_sewa_alat_penunjang.id_tdk = tbm_sewa_alat_penunjang.id)
			WHERE
			  tbt_inap_sewa_alat_penunjang.no_trans = '$noTransRwtInap'
			  AND tbt_inap_sewa_alat_penunjang.id_tdk = '$idTdk' 
			  AND tbt_inap_sewa_alat_penunjang.tgl = '$tgl'
			ORDER BY			  
			  tbt_inap_sewa_alat_penunjang.wkt";
    ...
    {	
		...
		foreach($arrData as $row)
		{
			...
			$newInapSewaAlatPenunjang= new InapSewaAlatPenunjangRecord();
			$newInapSewaAlatPenunjang->no_trans = $this->getViewState('notrans');
			$newInapSewaAlatPenunjang->cm=$this->formatCm($this->cm->Text);
			$newInapSewaAlatPenunjang->dokter = $idDokter;
			$newInapSewaAlatPenunjang->id_tdk = $idTdk;
			$newInapSewaAlatPenunjang->tgl = $tgl;
			$newInapSewaAlatPenunjang->wkt = $wkt;
			$newInapSewaAlatPenunjang->operator = $this->User->IsUserNip;
			$newInapSewaAlatPenunjang->catatan = $row['catatan'];
			$newInapSewaAlatPenunjang->tarif = $tarifStandar + ($tarifStandar * $persetaseKelas / 100);
			$newInapSewaAlatPenunjang->Save();	
		...
		$this->errMsg->Text = ' 
			<script type="text/javascript">
					unmaskContent();
					jQuery.WsGrowl.show({title: \'Konfirmasi\', content:\'<p>Rekam Sewal Alat Penunjang Sukses.</p>\',timeout: 200000,dialog:{
    ...
    {
		$this->Response->redirect($this->Service->constructUrl('Rawat.SewaAlatPenunjang'));
	}
